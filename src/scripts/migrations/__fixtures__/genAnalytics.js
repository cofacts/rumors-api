...
const lastDay = new Date('2020-01-01');
const allDates = Array(365).fill(0).map((v, i) => new Date(lastDay - i * 1000 * 60 * 60 * 24));

let analytics = {}
for (const key in fixturesToLoad) {
  const [, index, doc, id] = key.split('/');
  if (index === 'replies' || index === 'articles') {
    const count = Math.ceil(Math.random() * 5);
    for (const date of _.sortBy(_.sampleSize(allDates, count))) {
      const dateStr = date.toISOString();
      const docType = index.replace('ies', 'y').replace('s', '');
      analytics[`/analytics/doc/${docType}_${id}_${dateStr.substr(0, 10)}`] = {
        date: `${dateStr}T00:00:00.000+8:00`,
        docId: id,
        type: doc,
        stats: {
          webUser: Math.round(Math.random() * 10),
          webVisit: Math.round(Math.random() * 10),
          lineUser: Math.round(Math.random() * 10),
          lineVisit: Math.round(Math.random() * 10),
        },
        docUserId: fixturesToLoad[key].userId
      }
    }
  }
}
