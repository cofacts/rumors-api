const fixtures = {
  fixturesToLoad: {
    '/users/doc/CBUT_user_0': {
      createdAt: '2019-02-18T06:28:55.720Z',
      updatedAt: '2019-02-18T06:28:55.720Z',
      email: 'cnMsf0z@example.com',
      name: 'uH2VDAgiwf UC6V4',
      facebookId: 'CBUT_facebook_0',
    },
    '/users/doc/CBUT_user_1': {
      createdAt: '2019-10-15T05:11:15.972Z',
      updatedAt: '2019-10-15T05:11:15.972Z',
      email: '8xeGKHZOda@example.com',
      name: 'QcGDf68rq D3ZWh',
      facebookId: 'CBUT_facebook_1',
    },
    '/users/doc/CBUT_user_2': {
      createdAt: '2020-07-06T05:46:06.931Z',
      updatedAt: '2020-07-06T05:46:06.931Z',
      email: 'GFsBpOzw1t@example.com',
      name: 'fzAlW7 lTLr8U',
      facebookId: 'CBUT_facebook_2',
    },
    '/users/doc/CBUT_user_3': {
      createdAt: '2017-08-07T04:03:29.995Z',
      updatedAt: '2017-08-07T04:03:29.995Z',
      email: 'WNEhSxYx@example.com',
      name: 'nUWqA 9zu04Q',
      facebookId: 'CBUT_facebook_3',
    },
    '/users/doc/CBUT_user_4': {
      createdAt: '2019-03-30T03:46:39.014Z',
      updatedAt: '2019-03-30T03:46:39.014Z',
      email: '-49hG@example.com',
      name: '0W8kdJ Vg9XsuVhy',
      facebookId: 'CBUT_facebook_4',
    },
    '/users/doc/CBUT_user_5': {
      createdAt: '2018-12-25T10:06:57.131Z',
      updatedAt: '2018-12-25T10:06:57.131Z',
      email: 'hsnKHanBb5@example.com',
      name: 'dTPZcxn4W aWHchdvji',
      facebookId: 'CBUT_facebook_5',
    },
    '/users/doc/CBUT_user_6': {
      createdAt: '2019-02-12T03:40:04.690Z',
      updatedAt: '2019-02-12T03:40:04.690Z',
      email: 'X6M6CqP6-@example.com',
      name: '2PK0m KbAK-wu',
      facebookId: 'CBUT_facebook_6',
    },
    '/users/doc/CBUT_user_7': {
      createdAt: '2019-06-17T03:14:26.547Z',
      updatedAt: '2019-06-17T05:39:45.935Z',
      email: 'mtSHa9pQR@example.com',
      name: '1waeHG H5nlk5w',
      facebookId: 'CBUT_facebook_7',
    },
    '/users/doc/CBUT_user_8': {
      createdAt: '2018-12-02T05:48:31.102Z',
      updatedAt: '2018-12-02T05:48:31.102Z',
      email: '9bScmbJ@example.com',
      name: 'B4ywca jlF0w',
      facebookId: 'CBUT_facebook_8',
    },
    '/users/doc/CBUT_user_9': {
      createdAt: '2020-05-05T09:14:15.364Z',
      updatedAt: '2020-05-05T09:14:15.364Z',
      email: 'k-MJ48Wozf@example.com',
      name: 'AvqX7 SXguFvvotD',
      facebookId: 'CBUT_facebook_9',
    },
    '/users/doc/CBUT_user_10': {
      createdAt: '2020-03-12T14:45:03.825Z',
      updatedAt: '2020-03-12T14:45:03.825Z',
      email: 'LOv6ae2@example.com',
      name: '5aDOfh nMM705Ml',
      facebookId: 'CBUT_facebook_10',
    },
    '/users/doc/CBUT_user_11': {
      createdAt: '2020-02-07T15:27:05.303Z',
      updatedAt: '2020-02-07T15:27:05.303Z',
      email: 'ZRc1WF1ih@example.com',
      name: 'KDnWGKCis HUQADXS5dP',
      facebookId: 'CBUT_facebook_11',
    },
    '/users/doc/CBUT_user_12': {
      createdAt: '2019-05-14T02:20:27.275Z',
      updatedAt: '2019-05-14T02:20:27.275Z',
      name: 'BPZWZW BufLuDil',
      facebookId: 'CBUT_facebook_12',
    },
    '/users/doc/CBUT_user_13': {
      createdAt: '2019-06-18T15:20:54.339Z',
      updatedAt: '2019-06-18T15:20:54.339Z',
      email: 'DmjXIw@example.com',
      name: 'EXJbf brCVMftg',
      facebookId: 'CBUT_facebook_13',
    },
    '/users/doc/CBUT_user_14': {
      createdAt: '2020-09-26T21:38:58.624Z',
      updatedAt: '2020-09-26T21:38:58.624Z',
      email: 'Ci_HpyP6m@example.com',
      name: '8peFNlZdWT aSAqUlJ',
      facebookId: 'CBUT_facebook_14',
    },
    '/users/doc/CBUT_user_15': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_16': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_20': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_23': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_35': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_38': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_40': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_45': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_47': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_48': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_49': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_53': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_54': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_57': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_60': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_61': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_63': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_65': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_68': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_69': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_71': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_73': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_74': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_79': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_84': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_85': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_88': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/users/doc/CBUT_user_90': {
      createdAt: '2010-01-01T00:00:00Z',
    },
    '/articlecategoryfeedbacks/doc/CBUT_article_0__CBUT_category_0__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_0',
      categoryId: 'CBUT_category_0',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-09-19T07:07:05.785Z',
      updatedAt: '2020-09-19T07:07:05.785Z',
    },
    '/articlecategoryfeedbacks/doc/CBUT_article_1__CBUT_category_1__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_1',
      categoryId: 'CBUT_category_1',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-07-25T03:32:50.190Z',
      updatedAt: '2020-07-25T03:32:50.190Z',
      comment: 'irrelevant',
    },
    '/articlecategoryfeedbacks/doc/CBUT_article_2__CBUT_category_1__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_2',
      categoryId: 'CBUT_category_1',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-07-25T03:31:47.629Z',
      updatedAt: '2020-07-25T03:31:47.629Z',
      comment: 'irrelevant',
    },
    '/articlecategoryfeedbacks/doc/CBUT_article_3__CBUT_category_1__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_3',
      categoryId: 'CBUT_category_1',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-07-25T03:35:36.459Z',
      updatedAt: '2020-07-25T03:35:36.459Z',
      comment: 'irrelevant',
    },
    '/articlecategoryfeedbacks/doc/CBUT_article_4__CBUT_category_1__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_4',
      categoryId: 'CBUT_category_1',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-07-25T04:14:22.278Z',
      updatedAt: '2020-07-25T04:14:22.278Z',
      comment: 'irrelevant',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_5__CBUT_reply_0__CBUT_user_16__DEVELOPMENT_FRONTEND': {
      createdAt: '2017-11-19T09:40:11.292Z',
      appId: 'DEVELOPMENT_FRONTEND',
      userId: 'CBUT_user_16',
      updatedAt: '2017-11-19T09:40:11.292Z',
      articleId: 'CBUT_article_5',
      replyId: 'CBUT_reply_0',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_6__CBUT_reply_1__CBUT_user_17__DEVELOPMENT_BACKEND': {
      createdAt: '2018-01-11T08:29:22.045Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_17',
      updatedAt: '2018-01-11T08:29:22.045Z',
      articleId: 'CBUT_article_6',
      replyId: 'CBUT_reply_1',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_7__CBUT_reply_2__CBUT_user_18__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_7',
      replyId: 'CBUT_reply_2',
      userId: 'CBUT_user_18',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2018-10-29T14:32:04.786Z',
      updatedAt: '2018-10-29T14:32:04.786Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_8__CBUT_reply_3__CBUT_user_19__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_8',
      replyId: 'CBUT_reply_3',
      userId: 'CBUT_user_19',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2019-06-25T02:42:31.105Z',
      updatedAt: '2019-06-25T02:42:31.636Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_9__CBUT_reply_4__CBUT_user_20__WEBSITE': {
      articleId: 'CBUT_article_9',
      replyId: 'CBUT_reply_4',
      userId: 'CBUT_user_20',
      appId: 'WEBSITE',
      createdAt: '2020-09-10T11:45:55.227Z',
      updatedAt: '2020-09-10T11:45:55.227Z',
      comment: '',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_10__CBUT_reply_5__CBUT_user_21__DEVELOPMENT_BACKEND': {
      articleId: 'CBUT_article_10',
      replyId: 'CBUT_reply_5',
      userId: 'CBUT_user_21',
      appId: 'DEVELOPMENT_BACKEND',
      createdAt: '2020-02-27T09:18:07.301Z',
      updatedAt: '2020-02-27T09:18:07.301Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_11__CBUT_reply_6__CBUT_user_22__DEVELOPMENT_BACKEND': {
      createdAt: '2018-01-24T05:28:35.650Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_22',
      updatedAt: '2018-01-24T05:28:35.650Z',
      articleId: 'CBUT_article_11',
      replyId: 'CBUT_reply_6',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_12__CBUT_reply_7__CBUT_user_23__WEBSITE': {
      articleId: 'CBUT_article_12',
      replyId: 'CBUT_reply_7',
      userId: 'CBUT_user_23',
      appId: 'WEBSITE',
      createdAt: '2020-06-28T08:33:54.411Z',
      updatedAt: '2020-06-28T08:33:54.411Z',
      comment: '',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_13__CBUT_reply_8__CBUT_user_24__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_13',
      replyId: 'CBUT_reply_8',
      userId: 'CBUT_user_24',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2020-09-26T13:53:15.665Z',
      updatedAt: '2020-09-26T13:53:24.365Z',
      comment: '',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_14__CBUT_reply_9__CBUT_user_25__DEVELOPMENT_BACKEND': {
      articleId: 'CBUT_article_14',
      replyId: 'CBUT_reply_9',
      userId: 'CBUT_user_25',
      appId: 'DEVELOPMENT_BACKEND',
      createdAt: '2018-03-29T06:42:21.545Z',
      updatedAt: '2018-03-29T06:42:21.545Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_15__CBUT_reply_10__CBUT_user_15__DEVELOPMENT_FRONTEND': {
      createdAt: '2018-02-07T18:09:49.594Z',
      appId: 'DEVELOPMENT_FRONTEND',
      userId: 'CBUT_user_15',
      updatedAt: '2018-02-07T18:09:49.594Z',
      articleId: 'CBUT_article_15',
      replyId: 'CBUT_reply_10',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_16__CBUT_reply_11__CBUT_user_26__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_16',
      replyId: 'CBUT_reply_11',
      userId: 'CBUT_user_26',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2020-03-10T12:02:07.419Z',
      updatedAt: '2020-03-10T12:02:08.394Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_17__CBUT_reply_12__CBUT_user_15__WEBSITE': {
      articleId: 'CBUT_article_17',
      replyId: 'CBUT_reply_12',
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      createdAt: '2020-04-05T09:33:19.299Z',
      updatedAt: '2020-04-05T09:33:19.299Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_18__CBUT_reply_13__CBUT_user_27__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_18',
      replyId: 'CBUT_reply_13',
      userId: 'CBUT_user_27',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2019-02-20T05:11:06.611Z',
      updatedAt: '2019-02-20T05:11:06.611Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_19__CBUT_reply_14__CBUT_user_20__WEBSITE': {
      articleId: 'CBUT_article_19',
      replyId: 'CBUT_reply_14',
      userId: 'CBUT_user_20',
      appId: 'WEBSITE',
      createdAt: '2020-06-27T23:11:24.480Z',
      updatedAt: '2020-06-27T23:11:24.480Z',
      comment: '',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_20__CBUT_reply_15__CBUT_user_20__WEBSITE': {
      articleId: 'CBUT_article_20',
      replyId: 'CBUT_reply_15',
      userId: 'CBUT_user_20',
      appId: 'WEBSITE',
      createdAt: '2020-09-26T05:54:08.502Z',
      updatedAt: '2020-09-26T05:54:08.502Z',
      comment: '',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_21__CBUT_reply_16__CBUT_user_28__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_21',
      replyId: 'CBUT_reply_16',
      userId: 'CBUT_user_28',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2019-04-06T06:45:23.016Z',
      updatedAt: '2019-04-06T06:45:23.016Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_22__CBUT_reply_17__CBUT_user_29__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_22',
      replyId: 'CBUT_reply_17',
      userId: 'CBUT_user_29',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2018-11-11T23:43:02.406Z',
      updatedAt: '2018-11-11T23:43:02.406Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_23__CBUT_reply_18__CBUT_user_30__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_23',
      replyId: 'CBUT_reply_18',
      userId: 'CBUT_user_30',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2019-01-22T11:01:54.027Z',
      updatedAt: '2019-01-22T11:01:54.027Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_24__CBUT_reply_19__CBUT_user_31__DEVELOPMENT_BACKEND': {
      articleId: 'CBUT_article_24',
      replyId: 'CBUT_reply_19',
      userId: 'CBUT_user_31',
      appId: 'DEVELOPMENT_BACKEND',
      createdAt: '2019-11-20T22:23:54.285Z',
      updatedAt: '2019-11-20T22:23:54.285Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_25__CBUT_reply_20__CBUT_user_32__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_25',
      replyId: 'CBUT_reply_20',
      userId: 'CBUT_user_32',
      appId: 'RUMORS_LINE_BOT',
      createdAt: '2018-10-02T04:01:02.366Z',
      updatedAt: '2018-10-02T04:01:02.366Z',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_26__CBUT_reply_21__CBUT_user_16__DEVELOPMENT_FRONTEND': {
      createdAt: '2017-11-22T12:27:20.578Z',
      appId: 'DEVELOPMENT_FRONTEND',
      userId: 'CBUT_user_16',
      updatedAt: '2017-11-22T12:27:20.578Z',
      articleId: 'CBUT_article_26',
      replyId: 'CBUT_reply_21',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_27__CBUT_reply_22__CBUT_user_17__DEVELOPMENT_BACKEND': {
      createdAt: '2017-08-11T16:55:48.227Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_17',
      updatedAt: '2017-08-11T16:55:48.227Z',
      articleId: 'CBUT_article_27',
      replyId: 'CBUT_reply_22',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_28__CBUT_reply_23__CBUT_user_33__DEVELOPMENT_BACKEND': {
      createdAt: '2018-01-31T13:52:33.249Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_33',
      updatedAt: '2018-01-31T13:52:33.249Z',
      articleId: 'CBUT_article_28',
      replyId: 'CBUT_reply_23',
    },
    '/articlereplyfeedbacks/doc/CBUT_article_29__CBUT_reply_24__CBUT_user_23__WEBSITE': {
      articleId: 'CBUT_article_29',
      replyId: 'CBUT_reply_24',
      userId: 'CBUT_user_23',
      appId: 'WEBSITE',
      createdAt: '2020-09-19T17:04:20.167Z',
      updatedAt: '2020-09-19T17:04:20.167Z',
      comment: '',
    },
    '/articles/doc/CBUT_article_30': {
      references: [
        {
          createdAt: '2019-03-18T15:31:50.099Z',
          appId: 'DEVELOPMENT_BACKEND',
          type: 'URL',
          userId: 'CBUT_user_34',
        },
      ],
      userId: 'CBUT_user_34',
      tags: [],
      createdAt: '2019-03-18T15:31:50.099Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-03-19T14:57:01.116Z',
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_25',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-03-19T14:57:01.116Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-23T07:28:16.667Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-05-23T07:28:16.667Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'DEVELOPMENT_BACKEND',
      lastRequestedAt: '2019-03-18T15:31:50.146Z',
      text: 'Newsgeist is a horrible event. People are doing nothing here.',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-03-18T15:31:50.099Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_31': {
      references: [
        {
          createdAt: '2017-01-20T00:58:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-20T00:58:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-09-13T13:44:48.950Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_26',
          userId: 'CBUT_user_38',
          status: 'NORMAL',
          updatedAt: '2017-09-13T13:44:48.950Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T11:42:10.824Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_3',
          status: 'NORMAL',
          updatedAt: '2020-05-21T11:42:10.824Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-20T00:58:00.000Z',
      text:
        '南投新景點～富寮里動物園，佔地18公頃，內有狐獴、侏儒山羊、兔子、鸚鵡、天鵝、紅環、灰冠鶴、驢、騾、小馬、羊駝.......等，1百多種動物(持續增加ing)，...',
      hyperlinks: [
        {
          summary:
            '部落格關閉中\n                                    \n                                \n   ...',
          title: '痞客邦 訊息提示',
          url: 'http://fbuon2881.pixnet.net/blog/post/220868495',
        },
      ],
      updatedAt: '2017-01-20T00:58:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_32': {
      references: [
        {
          createdAt: '2018-06-15T07:34:36.490Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_39',
        },
      ],
      userId: 'CBUT_user_39',
      tags: [],
      createdAt: '2018-06-15T07:34:36.490Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-07-10T16:17:54.867Z',
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_27',
          userId: 'CBUT_user_40',
          status: 'NORMAL',
          updatedAt: '2018-07-10T16:17:54.867Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-23T04:34:30.811Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_4',
          status: 'NORMAL',
          updatedAt: '2020-05-23T04:34:30.811Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-06-15T07:34:36.502Z',
      text:
        '讚！國民黨有史以來打民進黨蔡英文最好的一支廣告.既傳神又生動，大家努力的傳出去https://m.youtube.com/watch?v=VVHf9ni09m4&feature=youtu.be',
      normalArticleReplycount: 0,
      hyperlinks: [
        {
          summary:
            'https://www.facebook.com/events/217719084982022/ 請多多邀請橘綠藍朋友加入或共同參加主辦人行列,要求""媒體自重...',
          title:
            '國民黨馬英九有史以來打民進黨蔡英文最好的一支廣告.既傳神又生動',
          url: 'https://m.youtube.com/watch?v=VVHf9ni09m4&feature=youtu.be',
        },
      ],
      updatedAt: '2018-06-15T07:34:36.490Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_33': {
      references: [
        {
          createdAt: '2019-05-16T15:09:42.254Z',
          appId: 'WEBSITE',
          type: 'URL',
          permalink: 'https://www.facebook.com/jnliao/videos/2701707013234650/',
          userId: 'CBUT_user_15',
        },
      ],
      userId: 'CBUT_user_15',
      tags: [],
      createdAt: '2019-05-16T15:09:42.254Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-05-16T15:11:30.728Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_28',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2019-05-16T15:11:30.728Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T23:59:04.380Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_5',
          status: 'NORMAL',
          updatedAt: '2020-05-21T23:59:04.380Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'WEBSITE',
      lastRequestedAt: '2019-05-16T15:09:42.281Z',
      text: '可惡至極的政府亂搞教育政策！這可以秀出來教嗎？這要禍害下一代！',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-05-16T15:09:42.254Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_34': {
      references: [
        {
          createdAt: '2017-01-18T02:58:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-18T02:58:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-06-12T11:50:12.540Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_29',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-06-12T11:50:12.540Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T12:03:06.153Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_6',
          status: 'NORMAL',
          updatedAt: '2020-05-21T12:03:06.153Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-18T02:58:00.000Z',
      text:
        'l  請大家注意！  l\n\n中壢地區的泰豐輪胎廠方失火，燒出來的煙都是戴奧辛，它非常非常毒，請大家及小朋友們出門可以的話戴口罩，下雨的話也不要淋雨，以上請各位長...',
      hyperlinks: [],
      updatedAt: '2017-01-18T02:58:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_15': {
      references: [
        {
          createdAt: '2018-01-07T16:35:06.585Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_41',
      tags: [],
      createdAt: '2018-01-07T16:35:06.585Z',
      articleReplies: [
        {
          createdAt: '2018-02-07T17:33:53.027Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_10',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2018-02-07T17:33:53.027Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T22:39:16.144Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_7',
          status: 'NORMAL',
          updatedAt: '2020-05-19T22:39:16.144Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-01-07T16:35:06.599Z',
      text:
        'http://health.businessweekly.com.tw/AArticleM.aspx?ID=ARTL000065544&p=2',
      hyperlinks: [
        {
          summary:
            '7.「支氣管擴張劑」：有些孩子的咳嗽，是因為病毒感染之後呼吸道過度反應而造成，如果發現孩子咳嗽超過一個星期以上，可以在看病時，跟醫師討論看看是否需要使用「支氣管...',
          title:
            '孩子半夜咳不停？問題可能在冷氣！小兒科醫師建議的冷氣溫度這樣調',
          url:
            'http://health.businessweekly.com.tw/AArticleM.aspx?ID=ARTL000065544&p=2',
        },
      ],
      updatedAt: '2018-01-07T16:35:06.585Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_35': {
      references: [
        {
          createdAt: '2017-12-24T23:48:52.794Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_42',
      tags: [],
      createdAt: '2017-12-24T23:48:52.794Z',
      articleReplies: [
        {
          createdAt: '2017-12-27T14:55:54.650Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_30',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-12-27T14:55:54.650Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T00:00:47.748Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_7',
          status: 'NORMAL',
          updatedAt: '2020-05-20T00:00:47.748Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-12-24T23:48:52.805Z',
      text:
        '支持手遊 www.yxc8.cn 安全服務交易平臺上來交易！瀏覽器打開網頁先上去註冊個屬於自己的帳戶，然後我去掛單你需要的價格的貨物，使用點數卡在線充值選擇貨物完成下單。',
      hyperlinks: [
        {
          summary: null,
          title: null,
          url: 'www.yxc8.cn',
        },
      ],
      updatedAt: '2017-12-24T23:48:52.794Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_26': {
      references: [
        {
          createdAt: '2017-11-19T09:35:45.073Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_43',
      tags: [],
      createdAt: '2017-11-19T09:35:45.073Z',
      articleReplies: [
        {
          createdAt: '2017-11-19T10:08:18.039Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_21',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-11-19T10:08:18.039Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T14:26:26.065Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-20T14:26:26.065Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-11-19T09:35:45.084Z',
      text: '肺炎疫苗接種，終身受用',
      hyperlinks: [],
      updatedAt: '2017-11-19T09:35:45.073Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_36': {
      references: [
        {
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_44',
      tags: [],
      createdAt: '2017-03-05T00:13:37.379Z',
      articleReplies: [
        {
          createdAt: '2017-06-19T14:54:52.147Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 28,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_31',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2017-06-19T14:54:52.147Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T13:41:30.327Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_7',
          status: 'NORMAL',
          updatedAt: '2020-05-20T13:41:30.327Z',
        },
      ],
      replyRequestCount: 7,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-06-09T03:12:55.885Z',
      text:
        '(受傷)\n原來汽車機車強制險不是\n只有賠償意外傷亡200萬, \n這個最重要一定要去了解！這篇報導…您不看是您的損失！\n\nhttps://youtu.be/7BV...',
      hyperlinks: [
        {
          summary:
            '觀賞更多影片請至https://goo.gl/rvS1UC  歡迎分享 訂閱\n心靈舒果FB：https://www.facebook.com/anytime99...',
          title:
            '心靈舒果--  您非知道不可的生活常識有汽車 機車強制險的好友們．一定要看一下 要牢記起來',
          url: 'https://youtu.be/7BVazOgwsXk',
        },
      ],
      updatedAt: '2017-03-05T00:13:37.379Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_8': {
      references: [
        {
          createdAt: '2017-08-01T08:45:13.720Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_46',
      tags: [],
      createdAt: '2017-08-01T08:45:13.720Z',
      articleReplies: [
        {
          createdAt: '2017-08-01T13:53:32.747Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 8,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_3',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2017-08-01T13:53:32.747Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T03:24:12.458Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T03:24:12.458Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-08-01T08:45:13.734Z',
      text:
        '天啊! 原來只要這個動作，發黑香蕉瞬間變回新鮮香蕉?!! 實在太神奇了!一定要分享出去!!\nhttp://goez1.com/p46719.asp\n',
      hyperlinks: [
        {
          summary:
            '這是不是太誇張了！我相信你的下巴一定也跟小編一樣掉到地上了，如果你還是不相信，請看完整操作影片！ 現在你再也可以不用因為浪費了香蕉而後悔不已了！ 快將這個神奇的妙招分享出去讓朋友們大開眼界吧！',
          title:
            '天啊! 原來只要這個動作，發黑香蕉瞬間變回新鮮香蕉?!! 實在太神奇了!一定要分享出去!! | EZGOE',
          url: 'http://goez1.com/p46719.asp',
        },
      ],
      updatedAt: '2017-08-01T08:45:13.720Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_22': {
      references: [
        {
          createdAt: '2017-01-16T14:55:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-16T14:55:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-03-27T14:21:52.625Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 10,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_32',
          userId: 'CBUT_user_35',
          status: 'DELETED',
          updatedAt: '2018-11-03T16:59:09.975Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-11-03T16:14:49.213Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_33',
          userId: 'CBUT_user_15',
          status: 'DELETED',
          updatedAt: '2018-11-03T16:17:11.162Z',
        },
        {
          negativeFeedbackCount: 2,
          createdAt: '2018-11-03T16:17:08.188Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 145,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_34',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2018-11-03T16:17:08.188Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-11-07T06:06:02.530Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_35',
          userId: 'CBUT_user_47',
          status: 'DELETED',
          updatedAt: '2018-11-07T06:08:54.149Z',
        },
        {
          negativeFeedbackCount: 1,
          createdAt: '2018-11-07T06:17:00.482Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 22,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_17',
          userId: 'CBUT_user_47',
          status: 'NORMAL',
          updatedAt: '2018-11-07T06:17:00.482Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-11-15T13:54:13.264Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 36,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_36',
          userId: 'CBUT_user_48',
          status: 'NORMAL',
          updatedAt: '2018-11-15T13:54:13.264Z',
        },
      ],
      normalArticleReplyCount: 3,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T12:28:44.104Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_9',
          status: 'NORMAL',
          updatedAt: '2020-05-21T12:28:44.104Z',
        },
      ],
      replyRequestCount: 7,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-16T14:55:00.000Z',
      text:
        '昨天ㄧ位媽媽去地檢署按鈴申告，很多家長去聲援，你看看影片， 她小六大兒子被強迫跟另外一個女生練習自慰， 她抗議給小孩上一些亂七八糟的性解放教育，被霸凌，這會不會...',
      hyperlinks: [],
      updatedAt: '2017-01-16T14:55:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_25': {
      references: [
        {
          createdAt: '2017-01-16T12:47:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-16T12:47:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-01-09T15:19:00.000Z',
          negativeFeedbackCount: 7,
          replyType: 'RUMOR',
          positiveFeedbackCount: 27,
          appId: 'BOT_LEGACY',
          replyId: 'CBUT_reply_37',
          userId: 'CBUT_user_35',
          status: 'DELETED',
          updatedAt: '2017-10-15T16:05:15.495Z',
        },
        {
          createdAt: '2017-10-15T16:05:28.186Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 53,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_38',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-10-15T16:05:28.186Z',
        },
        {
          negativeFeedbackCount: 5,
          createdAt: '2018-10-01T16:02:38.679Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 185,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_20',
          userId: 'CBUT_user_49',
          status: 'NORMAL',
          updatedAt: '2018-10-01T16:02:38.679Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T12:30:16.948Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_3',
          status: 'NORMAL',
          updatedAt: '2020-05-21T12:30:16.948Z',
        },
      ],
      replyRequestCount: 2,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-16T12:47:00.000Z',
      text:
        '今晚開始就實施擴大臨檢\n請各位朋友千萬不要酒後駕車\n警察局擴大臨檢日期：\n10/13：19~23時\n10/16：20~24時\n10/20：21~01時\n10/2...',
      hyperlinks: [],
      updatedAt: '2017-01-16T12:47:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_5': {
      references: [
        {
          createdAt: '2017-11-14T23:42:12.624Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_50',
      tags: [],
      createdAt: '2017-11-14T23:42:12.624Z',
      articleReplies: [
        {
          createdAt: '2017-11-15T05:23:58.758Z',
          negativeFeedbackCount: 1,
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_0',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-11-15T05:23:58.758Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T06:53:00.369Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T06:53:00.369Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-11-14T23:42:12.633Z',
      text:
        '眼睛周圍有一粒一粒的要如何改善？非常多人都有！分享出去功德無量！\nhttp://goez1.com/p97252.asp\n',
      hyperlinks: [
        {
          summary:
            '我們有時候會發現眼睛周圍會突然冒出一些白色顆粒狀的小疙瘩，看起很是讓人困擾，其實這個就是脂肪粒了洗臉的時候對著鏡子總是要嘆氣，別人看著自己的臉時也特別不自信，不...',
          title:
            '眼睛周圍有一粒一粒的要如何改善？非常多人都有！分享出去功德無量！ | EZGOE',
          url: 'http://goez1.com/p97252.asp',
        },
      ],
      updatedAt: '2017-11-14T23:42:12.624Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_37': {
      references: [
        {
          createdAt: '2017-01-11T08:52:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-11T08:52:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-12-14T04:09:00.115Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_39',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-12-14T04:09:00.115Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T15:07:00.163Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T15:07:00.163Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-11T08:52:00.000Z',
      text:
        '小兒用藥安全-小兒用藥的一些基本觀念 https://www.parenting.com.tw/article/5059770-小兒用藥安全-小兒用藥的一些基本觀念/',
      hyperlinks: [
        {
          summary:
            '不管是中藥或是西藥，沒有任何一種藥是沒有副作用的。如果有少數中藥總是強調天然植物提煉，完全不提其副作用，那麼這是一種欺騙的行為。什麼才是正確的用藥觀念呢？讓黃醫...',
          title: '小兒用藥安全-小兒用藥的一些基本觀念/親子天下',
          url:
            'https://www.parenting.com.tw/article/5059770-小兒用藥安全-小兒用藥的一些基本觀念/',
        },
      ],
      updatedAt: '2017-01-11T08:52:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_7': {
      references: [
        {
          createdAt: '2018-10-23T09:21:35.323Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_51',
        },
        {
          createdAt: '2018-10-30T01:50:21.966Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_52',
        },
      ],
      userId: 'CBUT_user_51',
      tags: [],
      createdAt: '2018-10-23T09:21:35.323Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-10-23T11:51:05.419Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 10,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_40',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2018-10-23T11:51:05.419Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-10-23T17:31:53.669Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 10,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_41',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2018-10-23T17:31:53.669Z',
        },
        {
          negativeFeedbackCount: 6,
          createdAt: '2018-10-24T13:19:03.745Z',
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 4,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_42',
          userId: 'CBUT_user_53',
          status: 'NORMAL',
          updatedAt: '2018-10-24T13:19:03.745Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-10-25T16:17:06.303Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 10,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_2',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2018-10-25T16:17:06.303Z',
        },
        {
          negativeFeedbackCount: 1,
          createdAt: '2018-11-22T15:55:57.510Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 6,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_43',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2018-11-22T15:55:57.510Z',
        },
      ],
      normalArticleReplyCount: 5,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T09:35:53.201Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_1',
          status: 'NORMAL',
          updatedAt: '2020-05-22T09:35:53.201Z',
        },
      ],
      replyRequestCount: 4,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-12-05T13:22:22.965Z',
      text:
        '大法官已釋憲，如果年底公投沒達到500萬票，明年五月台灣將成為亞洲第一個同性可合法結婚的國家。\n\n外國同性戀藉結婚來台治療愛滋病，都是健保支付，全民買單。\n\n學...',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2018-10-30T01:50:21.966Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_16': {
      references: [
        {
          createdAt: '2020-03-03T15:37:27.556Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_55',
        },
      ],
      userId: 'CBUT_user_55',
      tags: [],
      createdAt: '2020-03-03T15:37:27.556Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-03-04T03:39:58.171Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 60,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_11',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2020-03-04T03:39:58.171Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T04:06:44.784Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-22T04:06:44.784Z',
        },
      ],
      replyRequestCount: 7,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-03-04T03:08:52.875Z',
      text:
        '各位同事，您可能還記得，當我是加利福尼亞大學聖地亞哥分校的病理學教授時，我是世界上最早研究冠狀病毒的分子病毒學家之一（1970年代）。我是第一個證明該病毒包含的...',
      hyperlinks: [],
      updatedAt: '2020-03-03T15:37:27.556Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_38': {
      references: [
        {
          createdAt: '2020-02-21T02:02:54.455Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_56',
        },
      ],
      userId: 'CBUT_user_56',
      tags: [],
      createdAt: '2020-02-21T02:02:54.455Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-02-21T02:49:47.098Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 44,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_44',
          userId: 'CBUT_user_57',
          status: 'NORMAL',
          updatedAt: '2020-02-21T02:49:47.098Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-08T14:19:31.631Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_45',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2020-04-08T14:19:31.631Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T01:28:20.202Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-05-22T01:28:20.202Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-02-21T02:02:55.271Z',
      text:
        '意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨。這回意大利下的石頭雨就讓人見識了大自然的不可預測，不可控制，不可防範的天災，也再次見識了極端天氣之恐怖！\n...',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2020-02-21T02:02:54.455Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_28': {
      references: [
        {
          createdAt: '2017-08-29T03:38:35.524Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_58',
      tags: [],
      createdAt: '2017-08-29T03:38:35.524Z',
      articleReplies: [
        {
          createdAt: '2017-08-29T04:03:40.183Z',
          negativeFeedbackCount: 4,
          replyType: 'RUMOR',
          positiveFeedbackCount: 156,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_23',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-08-29T04:03:40.183Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-12-12T04:33:27.040Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 76,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_46',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2018-12-12T04:33:27.040Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-08-28T02:48:00.678Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 5,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_47',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2019-08-28T02:48:00.678Z',
        },
      ],
      normalArticleReplyCount: 3,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T19:08:49.495Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-20T19:08:49.495Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-08-29T03:38:35.539Z',
      text:
        '台北馬偕醫院傳來訊息：\n\n資深醫生（林清風）傳來：「請大家以後千萬不要再吃生魚片了！」\n因為最近已經發現- 好多病人因為吃了生魚片，胃壁附著《海獸胃腺蟲》，大小...',
      hyperlinks: [],
      updatedAt: '2017-08-29T03:38:35.524Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_2': {
      references: [
        {
          createdAt: '2020-04-17T13:11:33.426Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_59',
        },
      ],
      userId: 'CBUT_user_59',
      tags: [],
      createdAt: '2020-04-17T13:11:33.426Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-28T13:51:19.455Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_48',
          userId: 'CBUT_user_60',
          status: 'NORMAL',
          updatedAt: '2020-04-28T13:51:19.455Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-05T09:41:19.619Z',
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_49',
          userId: 'CBUT_user_61',
          status: 'NORMAL',
          updatedAt: '2020-05-05T09:41:19.619Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 1,
          createdAt: '2020-05-23T16:12:16.897Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_1',
          status: 'NORMAL',
          updatedAt: '2020-05-23T16:12:16.897Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-07-25T03:31:27.224Z',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          userId: 'CBUT_user_15',
          categoryId: 'CBUT_category_10',
          status: 'NORMAL',
          updatedAt: '2020-07-25T03:31:27.224Z',
        },
      ],
      replyRequestCount: 3,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-04-25T11:34:30.749Z',
      text:
        '13:23 金克寧 分享轉傳：\n\n兩位地方公共衛生局局長：Grant Colfax和Sara Cody，分別給他們取個中文名字吧：格蘭特和薩拉。\n\n格蘭特是舊金...',
      hyperlinks: [],
      updatedAt: '2020-04-17T13:11:33.426Z',
      normalArticleCategoryCount: 2,
    },
    '/articles/doc/CBUT_article_1': {
      references: [
        {
          createdAt: '2020-04-06T04:35:45.919Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_62',
        },
      ],
      userId: 'CBUT_user_62',
      tags: [],
      createdAt: '2020-04-06T04:35:45.919Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-06T07:42:40.317Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 45,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_50',
          userId: 'CBUT_user_63',
          status: 'NORMAL',
          updatedAt: '2020-04-06T07:42:40.317Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 1,
          createdAt: '2020-05-21T09:08:36.433Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_1',
          status: 'NORMAL',
          updatedAt: '2020-05-21T09:08:36.433Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-07-25T03:32:27.942Z',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          userId: 'CBUT_user_15',
          categoryId: 'CBUT_category_10',
          status: 'NORMAL',
          updatedAt: '2020-07-25T03:32:27.942Z',
        },
      ],
      replyRequestCount: 15,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-04-06T07:42:18.890Z',
      text:
        '你有沒有發現今天的記者會、陳時中等所有人都戴上了口罩，這認為意味著什麼？\n\n現在無症狀卻可能有傳染力的隱性感染者，是最讓人驚恐又難以防範的\n真的不要挑戰自己的運...',
      hyperlinks: [],
      updatedAt: '2020-04-06T04:35:45.919Z',
      normalArticleCategoryCount: 2,
    },
    '/articles/doc/CBUT_article_3': {
      references: [
        {
          createdAt: '2019-12-24T02:03:16.857Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_64',
        },
      ],
      userId: 'CBUT_user_64',
      tags: [],
      createdAt: '2019-12-24T02:03:16.857Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-12-24T02:41:34.902Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_51',
          userId: 'CBUT_user_65',
          status: 'DELETED',
          updatedAt: '2019-12-24T02:45:39.069Z',
        },
        {
          negativeFeedbackCount: 1,
          createdAt: '2019-12-24T02:55:42.332Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 44,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_52',
          userId: 'CBUT_user_65',
          status: 'NORMAL',
          updatedAt: '2019-12-24T02:55:42.332Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-12-25T16:46:47.886Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 9,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_53',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2019-12-25T16:46:47.886Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 1,
          createdAt: '2020-05-23T15:39:19.038Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_1',
          status: 'NORMAL',
          updatedAt: '2020-05-23T15:39:19.038Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-07-25T03:36:19.070Z',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          userId: 'CBUT_user_15',
          categoryId: 'CBUT_category_4',
          status: 'NORMAL',
          updatedAt: '2020-07-25T03:36:19.070Z',
        },
      ],
      replyRequestCount: 8,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2019-12-24T02:55:11.261Z',
      text:
        '民進黨已經通過了毒品條例了\n\n校園內可以擁有50克的毒品，才被視同販毒。\n\n台灣除了是(同性戀島)\n(愛滋病島)現在又增加了一個(毒品島)了\n校園毒品零容忍遭封...',
      normalArticleReplycount: 0,
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.chinatimes.com/realtimenews/20191217002605-260407',
          summary:
            '立法委員沈智慧在立法院提案，「毒品危害防制條例第四條、第六條及第三十一條之一條文修正草案」，沈智慧曾兩度上台發言，拜託民進黨支持，能讓法案通過，但結果還是被封殺...',
          title: '校園毒品零容忍遭封殺 藍委提嚴刑峻法被綠委阻攔',
          url: 'https://www.chinatimes.com/realtimenews/20191217002605-260407',
        },
      ],
      updatedAt: '2019-12-24T02:03:16.857Z',
      normalArticleCategoryCount: 2,
    },
    '/articles/doc/CBUT_article_4': {
      references: [
        {
          createdAt: '2019-05-30T13:03:09.773Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_66',
        },
      ],
      userId: 'CBUT_user_66',
      tags: [],
      createdAt: '2019-05-30T13:03:09.773Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-06-02T01:40:59.876Z',
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_54',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-06-02T01:40:59.876Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 1,
          createdAt: '2020-05-24T00:58:22.690Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_1',
          status: 'NORMAL',
          updatedAt: '2020-05-24T00:58:22.690Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2019-05-30T13:03:09.792Z',
      text:
        '(moon)A，戀人\n\n他是愛你的，你在他眼中你是是完美的戀人，你的優點缺點他都能夠接受。\n情人眼裡出西施，說的就是他，你們在一起是他最為榮幸的，他覺得你是他一...',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-05-30T13:03:09.773Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_39': {
      references: [
        {
          createdAt: '2020-04-26T07:32:15.732Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_67',
        },
      ],
      userId: 'CBUT_user_67',
      tags: [],
      createdAt: '2020-04-26T07:32:15.732Z',
      normalArticleCategoryCount: 1,
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-26T15:36:41.569Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_55',
          userId: 'CBUT_user_68',
          status: 'NORMAL',
          updatedAt: '2020-04-26T15:36:41.569Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-27T09:34:49.517Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_56',
          userId: 'CBUT_user_69',
          status: 'NORMAL',
          updatedAt: '2020-04-27T09:34:49.517Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T06:54:21.155Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_7',
          status: 'NORMAL',
          updatedAt: '2020-05-19T06:54:21.155Z',
        },
      ],
      replyRequestCount: 2,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-04-27T09:34:01.196Z',
      text:
        'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
      hyperlinks: [
        {
          normalizedUrl:
            'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
          summary:
            'Skip to content\n\n\t\n\t\n\n\t\n\t\t\n\n\t\t\t\n\n\n\t\n\n\t\n\t\n\t\t\n\n\n\n\n朝鲜全体党员、人民军将领以及人民：\n\n\n\n　　朝鲜劳动党中央委员...',
          title: '朝鲜全体党员、人民军将领以及人民：',
          url:
            'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
        },
      ],
      updatedAt: '2020-04-26T07:32:15.732Z',
    },
    '/articles/doc/CBUT_article_21': {
      references: [
        {
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_70',
      tags: [],
      createdAt: '2017-03-13T14:12:52.385Z',
      articleReplies: [
        {
          createdAt: '2017-03-21T16:25:34.320Z',
          negativeFeedbackCount: 12,
          replyType: 'RUMOR',
          positiveFeedbackCount: 307,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_16',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-03-21T16:25:34.320Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-08-14T05:27:33.035Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 13,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_57',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2019-08-14T05:27:33.035Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T13:04:27.210Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-20T13:04:27.210Z',
        },
      ],
      replyRequestCount: 27,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-03-21T15:32:51.522Z',
      text:
        '各位朋友請注意，\n\n即日起不可食用泰國製之各類罐頭食品，包含臺灣品牌泰國生産者，尤其是泰國罐裝水果食品。\n泰國200多名愛滋病感染者，在患者的首腦指揮下，湧進泰...',
      hyperlinks: [],
      updatedAt: '2017-03-13T14:12:52.385Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_40': {
      references: [
        {
          createdAt: '2017-05-27T04:06:18.243Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_17',
      tags: [],
      createdAt: '2017-05-27T04:06:18.243Z',
      articleReplies: [
        {
          createdAt: '2017-06-20T17:29:47.809Z',
          negativeFeedbackCount: 1,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_58',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2017-06-20T17:29:47.809Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-07-22T03:51:29.633Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_59',
          userId: 'CBUT_user_71',
          status: 'NORMAL',
          updatedAt: '2020-07-22T03:51:29.633Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T05:37:25.077Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-20T05:37:25.077Z',
        },
      ],
      replyRequestCount: 3,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-06-13T13:42:06.389Z',
      text:
        '蜂蜜正確吃法 https://youtu.be/6Mh0EKBfGYg               這是一篇很棒的建議。',
      hyperlinks: [
        {
          summary:
            '選購保健食品，還在看小綠人標章嗎？\n你知道出過問題的味全油品也通過小綠人認證嗎？！\n\n經過人體雙盲研究的保健效果才真正有效！\n\n江守山醫師精選12大保健天然保健...',
          title: '《吃對天然保健食品2天然篇》蜂蜜的神奇保健力',
          url: 'https://youtu.be/6Mh0EKBfGYg',
        },
      ],
      updatedAt: '2017-05-27T04:06:18.243Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_23': {
      references: [
        {
          createdAt: '2018-12-28T13:18:06.346Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_72',
        },
      ],
      userId: 'CBUT_user_72',
      tags: [],
      createdAt: '2018-12-28T13:18:06.346Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-12-28T13:27:11.334Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 124,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_18',
          userId: 'CBUT_user_73',
          status: 'NORMAL',
          updatedAt: '2018-12-28T13:27:11.334Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-07-02T04:09:52.096Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 4,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_60',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2019-07-02T04:09:52.096Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-07-23T08:06:47.683Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 15,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_61',
          userId: 'CBUT_user_74',
          status: 'NORMAL',
          updatedAt: '2019-07-23T08:06:47.683Z',
        },
      ],
      normalArticleReplyCount: 3,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T15:00:40.117Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-22T15:00:40.117Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-12-28T13:18:06.372Z',
      text:
        '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效!：https://youtu.be/EI_oExo38fA',
      normalArticleReplycount: 0,
      hyperlinks: [
        {
          summary:
            '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效! 值得收藏！在一次煮玉米粥時，我用叉子在沸騰的粥裡翻攪，想看看粥好了沒有。\n由於想問題，...',
          title:
            '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效!',
          url: 'https://youtu.be/EI_oExo38fA',
        },
      ],
      updatedAt: '2018-12-28T13:18:06.346Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_17': {
      references: [
        {
          createdAt: '2020-04-03T01:27:41.969Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_75',
        },
      ],
      userId: 'CBUT_user_75',
      tags: [],
      createdAt: '2020-04-03T01:27:41.969Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-03T04:24:14.814Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_12',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2020-04-03T04:24:14.814Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-08T14:05:42.107Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_62',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2020-04-08T14:05:42.107Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-24T01:29:04.833Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-24T01:29:04.833Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-04-03T01:27:41.997Z',
      text:
        '🇮🇹這就是-義大利🇮🇹\n一對*約定今年完婚*的\n情侣.女友感染新冠病毒在醫院搶救.男友探病時断然脱掉防護服⋯他們在3/25日-相繼往生！\n😭看得讓人腸斷心碎😭  ',
      hyperlinks: [],
      updatedAt: '2020-04-03T01:27:41.969Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_10': {
      references: [
        {
          createdAt: '2020-02-27T03:27:49.869Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_76',
        },
      ],
      userId: 'CBUT_user_76',
      tags: [],
      createdAt: '2020-02-27T03:27:49.869Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-02-27T09:11:33.769Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 4,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_5',
          userId: 'CBUT_user_68',
          status: 'NORMAL',
          updatedAt: '2020-02-27T09:11:33.769Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-24T02:14:20.097Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_0',
          status: 'NORMAL',
          updatedAt: '2020-05-24T02:14:20.097Z',
        },
      ],
      replyRequestCount: 2,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2020-02-27T07:50:54.261Z',
      text:
        '\n什麼是「中國腳鐐」呢？\n為何全世界會願意套上這個“中國腳鐐”呢？\n一個簡單的答案就是：沒有更好的選擇。\n※※※※※※\n數字是這樣告訴我們的：台灣出口最大的國家...',
      hyperlinks: [],
      updatedAt: '2020-02-27T03:27:49.869Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_41': {
      references: [
        {
          createdAt: '2019-04-17T06:56:36.132Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_77',
        },
      ],
      userId: 'CBUT_user_77',
      tags: [],
      createdAt: '2019-04-17T06:56:36.132Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-04-18T11:30:15.508Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_63',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-04-18T11:30:15.508Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T00:11:03.854Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_5',
          status: 'NORMAL',
          updatedAt: '2020-05-22T00:11:03.854Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2019-04-17T06:56:36.153Z',
      text: '日本人砍台灣樹林比台灣人砍的還要多?',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-04-17T06:56:36.132Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_42': {
      references: [
        {
          createdAt: '2019-04-15T16:38:11.456Z',
          appId: 'WEBSITE',
          type: 'URL',
          permalink:
            'https://www.facebook.com/NeilPeng.tw/photos/a.289826441057121/2633838049989270/?type=3&theater',
          userId: 'CBUT_user_35',
        },
      ],
      userId: 'CBUT_user_35',
      tags: [],
      createdAt: '2019-04-15T16:38:11.456Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-04-15T16:40:37.610Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_64',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-04-15T16:40:37.610Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T07:47:55.647Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_4',
          status: 'NORMAL',
          updatedAt: '2020-05-19T07:47:55.647Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'WEBSITE',
      lastRequestedAt: '2019-04-15T16:38:11.473Z',
      text:
        '一名網友在「韓國瑜加油團」臉書社團發文指出，經在高雄市政府工作的朋友告知，韓國瑜此行訪美，川普特別透過秘密管道向高市府傳達求見意願，還強調之所以會找上韓國瑜，是...',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-04-15T16:38:11.456Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_19': {
      references: [
        {
          createdAt: '2019-11-10T06:18:52.830Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_78',
        },
      ],
      userId: 'CBUT_user_78',
      tags: [],
      createdAt: '2019-11-10T06:18:52.830Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-11-18T07:31:32.991Z',
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_65',
          userId: 'CBUT_user_79',
          status: 'NORMAL',
          updatedAt: '2019-11-18T07:31:32.991Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-11-18T13:31:29.671Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_14',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-11-18T13:31:29.671Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T20:41:58.708Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T20:41:58.708Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2019-11-10T06:18:52.860Z',
      text:
        '电視新聞報導医生檢查女記者的肺部，發現常使用手機的人，肺部迅速老化，那位女記者才二十多歲，肺部老化，相當四十多歲的人。好恐怖!三C產品盡量少用。',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-11-10T06:18:52.830Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_24': {
      references: [
        {
          createdAt: '2019-11-13T16:17:15.137Z',
          appId: 'WEBSITE',
          type: 'URL',
          permalink: '',
          userId: 'CBUT_user_35',
        },
      ],
      userId: 'CBUT_user_35',
      tags: [],
      createdAt: '2019-11-13T16:17:15.137Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-11-13T16:18:32.732Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 2,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_19',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2019-11-13T16:18:32.732Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T18:28:42.079Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-05-22T18:28:42.079Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'WEBSITE',
      lastRequestedAt: '2019-11-13T16:17:15.156Z',
      text:
        'My experience with burns is this:\nOnce I was cooking some corn and stuck my fork...',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-11-13T16:17:15.137Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_12': {
      references: [
        {
          createdAt: '2018-03-30T12:13:57.322Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_80',
        },
      ],
      userId: 'CBUT_user_80',
      tags: [],
      createdAt: '2018-03-30T12:13:57.322Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2018-03-30T14:19:54.465Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_7',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2018-03-30T14:19:54.465Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T14:05:43.227Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_3',
          status: 'NORMAL',
          updatedAt: '2020-05-19T14:05:43.227Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-03-30T12:13:57.363Z',
      text: '民眾檢舉交通罰單籲7日不用繳費',
      normalArticleReplycount: 0,
      updatedAt: '2018-03-30T12:13:57.322Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_6': {
      references: [
        {
          createdAt: '2017-01-10T01:12:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-10T01:12:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-03-24T16:51:23.378Z',
          negativeFeedbackCount: 1,
          replyType: 'RUMOR',
          positiveFeedbackCount: 40,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_66',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-03-24T16:51:23.378Z',
        },
        {
          createdAt: '2017-10-03T03:37:10.744Z',
          negativeFeedbackCount: 3,
          replyType: 'RUMOR',
          positiveFeedbackCount: 63,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_1',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-10-03T03:37:10.744Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-04-08T13:45:36.418Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 4,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_67',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2020-04-08T13:45:36.418Z',
        },
      ],
      normalArticleReplyCount: 3,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T17:58:18.866Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_3',
          status: 'NORMAL',
          updatedAt: '2020-05-21T17:58:18.866Z',
        },
      ],
      replyRequestCount: 5,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-10T01:12:00.000Z',
      text:
        '*轉po*\n昨天年金改革委員會第14次會議，重點如以下幾點：\n1.各項年金請領年齡皆為65歲（未滿65歲無法請領），不分族群職業類別。\n勞保原為60歲即可領，未...',
      hyperlinks: [],
      updatedAt: '2017-01-10T01:12:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_27': {
      references: [
        {
          createdAt: '2017-06-24T02:25:38.653Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_81',
      tags: [],
      createdAt: '2017-06-24T02:25:38.653Z',
      articleReplies: [
        {
          createdAt: '2017-06-24T07:35:54.959Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_68',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2017-06-24T07:35:54.959Z',
        },
        {
          createdAt: '2017-06-27T15:25:17.276Z',
          negativeFeedbackCount: 1,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_22',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2017-06-27T15:25:17.276Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T03:38:53.843Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_7',
          status: 'NORMAL',
          updatedAt: '2020-05-20T03:38:53.843Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-06-24T02:25:38.668Z',
      text:
        '連署增設公幼\n\nhttp://join.gov.tw/idea/detail/6a024fc8-ceb3-45cc-a37a-59cbec2d6bd4\n各位夥伴...',
      hyperlinks: [
        {
          summary:
            '提議內容或建議事項\n\t\t\t\t台灣少子化已是全球第一名，目前總生育率約1.175%，日本為1.4%、韓國1.5%，港澳及新加坡為1.2%。目前全國公幼2465園，...',
          title:
            '公共化幼兒園應在4年內達到1000班之政策目標- 提點子-公共政策網路參與平臺',
          url:
            'http://join.gov.tw/idea/detail/6a024fc8-ceb3-45cc-a37a-59cbec2d6bd4',
        },
      ],
      updatedAt: '2017-06-24T02:25:38.653Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_14': {
      references: [
        {
          createdAt: '2017-07-10T11:37:00.087Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_82',
      tags: [],
      createdAt: '2017-07-10T11:37:00.087Z',
      articleReplies: [
        {
          createdAt: '2017-07-10T14:01:55.451Z',
          negativeFeedbackCount: 2,
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_69',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2017-07-10T14:01:55.451Z',
        },
        {
          negativeFeedbackCount: 1,
          createdAt: '2018-03-11T02:45:59.130Z',
          replyType: 'OPINIONATED',
          positiveFeedbackCount: 5,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_9',
          userId: 'CBUT_user_15',
          status: 'NORMAL',
          updatedAt: '2018-03-11T02:45:59.130Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T05:14:28.507Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-05-21T05:14:28.507Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-07-10T11:37:00.105Z',
      text:
        '轉載：\n以下是一位在臺灣出生在台灣成長的純正台湾人轉來的。\n\n什麼叫「兩蔣時代」？以下就是：\n\n1，是治安良好的時代。\n2，是沒有民粹的時代。\n3，是禮義廉恥、...',
      hyperlinks: [],
      updatedAt: '2017-07-10T11:37:00.087Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_9': {
      references: [
        {
          createdAt: '2017-05-14T13:07:55.057Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_83',
      tags: [],
      createdAt: '2017-05-14T13:07:55.057Z',
      articleReplies: [
        {
          createdAt: '2017-07-09T15:02:23.375Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 146,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_70',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-07-09T15:02:23.375Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-12-06T16:05:05.617Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 13,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_4',
          userId: 'CBUT_user_54',
          status: 'NORMAL',
          updatedAt: '2019-12-06T16:05:05.617Z',
        },
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-03-26T06:01:58.473Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 0,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_71',
          userId: 'CBUT_user_84',
          status: 'DELETED',
          updatedAt: '2020-03-26T06:05:11.732Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-20T06:31:05.776Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-20T06:31:05.776Z',
        },
      ],
      replyRequestCount: 2,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-05-21T04:51:51.057Z',
      text:
        '(牛奶加一物，三高、痛風、癌症都不見了！姐夫82歲了還活力無窮！轉發出去功德無量！)【快來看】│這篇文不錯分享給你看│ →    http://ezvivi.c...',
      hyperlinks: [
        {
          summary:
            '每天一杯牛奶加上它，痛風不見了！還遠離三高，對抗癌細胞！太神奇了！！\n\t\t\t\t\t\n\t\t\t\t\t\n\t\t\t\t\t\n\n\n\n\n\t\t\t\t\t\t\t\n\t\t\n\t\n\n\n\n\t\n\t\t\n\t\t...',
          title:
            '牛奶加一物，三高、痛風、癌症都不見了！姐夫82歲了還活力無窮！轉發出去功德無量！ | Love分享',
          url: 'http://ezvivi.com/article/257739.asp',
        },
        {
          summary:
            '☆COLOR99☆韓衣--持平價奢華路線˙給妳專屬寵愛˙教妳變化多端造型、讓大美人兒也可以穿出自信，讓您有最方便的購物選擇單次消費金額滿1元或累計消費金額滿2元...',
          title: 'Color99-Yahoo!奇摩超級商城',
          url: 'http://goo.gl/mKq5t0',
        },
      ],
      updatedAt: '2017-05-14T13:07:55.057Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_18': {
      references: [
        {
          createdAt: '2017-01-22T04:05:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-22T04:05:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-04-12T11:21:23.420Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 11,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_72',
          userId: 'CBUT_user_85',
          status: 'NORMAL',
          updatedAt: '2017-04-12T11:21:23.420Z',
        },
        {
          createdAt: '2017-04-12T11:25:01.494Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 48,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_13',
          userId: 'CBUT_user_85',
          status: 'NORMAL',
          updatedAt: '2017-04-12T11:25:01.494Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T11:37:33.109Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T11:37:33.109Z',
        },
      ],
      replyRequestCount: 12,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-04-08T08:27:32.840Z',
      text:
        '日本醫學博士說:\n\n防失智, 增強記憶力, 不用吃藥, 可以用食療, 他推薦要吃4種食物 :\n\n1, 番茄煮熟防失智。\n2, 蘋果增強記憶力。\n3, 黑巧克力活...',
      hyperlinks: [],
      updatedAt: '2017-01-22T04:05:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_11': {
      references: [
        {
          createdAt: '2017-12-30T01:01:44.438Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_86',
      tags: [],
      createdAt: '2017-12-30T01:01:44.438Z',
      articleReplies: [
        {
          createdAt: '2018-01-01T12:00:04.040Z',
          negativeFeedbackCount: 4,
          replyType: 'NOT_RUMOR',
          positiveFeedbackCount: 10,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_73',
          userId: 'CBUT_user_45',
          status: 'NORMAL',
          updatedAt: '2018-01-01T12:00:04.040Z',
        },
        {
          createdAt: '2018-01-23T15:42:02.682Z',
          negativeFeedbackCount: 0,
          replyType: 'RUMOR',
          positiveFeedbackCount: 35,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_6',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2018-01-23T15:42:02.682Z',
        },
      ],
      normalArticleReplyCount: 2,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T23:26:42.995Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-19T23:26:42.995Z',
        },
      ],
      replyRequestCount: 3,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2017-12-31T04:18:42.495Z',
      text:
        '舊金山市長李孟賢（Edwin M. Lee）已於12 日凌晨1時11分因心臟病發於舊金山總醫院過世，享年65歲。\n請花幾分鐘閱讀此文：\n\n1. 假設現在是晚上7...',
      hyperlinks: [],
      updatedAt: '2017-12-30T01:01:44.438Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_29': {
      references: [
        {
          createdAt: '2019-08-15T02:39:39.532Z',
          appId: 'RUMORS_LINE_BOT',
          type: 'LINE',
          userId: 'CBUT_user_87',
        },
      ],
      userId: 'CBUT_user_87',
      tags: [],
      createdAt: '2019-08-15T02:39:39.532Z',
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2019-08-21T02:10:43.994Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_24',
          userId: 'CBUT_user_88',
          status: 'NORMAL',
          updatedAt: '2019-08-21T02:10:43.994Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-22T05:13:17.490Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_0',
          status: 'NORMAL',
          updatedAt: '2020-05-22T05:13:17.490Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2019-08-15T02:39:39.597Z',
      text:
        '運動手錶跟手機互聯，當然手機內所有資料都會進入他們的伺服器（任何品牌都如此）唯獨中國只要發送「詆譭攻擊等圖文」就會鎖定情搜追蹤你。以保護自己及國安角度，供大家參考。',
      normalArticleReplycount: 0,
      hyperlinks: [],
      updatedAt: '2019-08-15T02:39:39.532Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_0': {
      text:
        '轉貼\n\n中華民族崛起的主要敵人 AngloSaxon 民族\n\n兩百年來，鴉片戰爭開始，中國最主要的敵人就是英國，AngloSaxon 民族，現在隨著中國的崛起​...',
      createdAt: '2020-09-08T15:30:44.659Z',
      updatedAt: '2020-09-08T15:30:44.659Z',
      userId: 'CBUT_user_87',
      appId: 'RUMORS_LINE_BOT',
      references: [
        {
          type: 'LINE',
          createdAt: '2020-09-08T15:30:44.659Z',
          userId: 'CBUT_user_87',
          appId: 'RUMORS_LINE_BOT',
        },
      ],
      articleReplies: [],
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-09-12T07:19:46.285Z',
          positiveFeedbackCount: 1,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bow',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_0',
          status: 'NORMAL',
          updatedAt: '2020-09-12T07:19:46.285Z',
        },
      ],
      normalArticleReplyCount: 0,
      normalArticleCategoryCount: 1,
      replyRequestCount: 2,
      tags: [],
      hyperlinks: [],
      lastRequestedAt: '2020-09-17T12:44:47.688Z',
    },
    '/articles/doc/CBUT_article_20': {
      references: [
        {
          createdAt: '2018-01-26T02:28:32.022Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_89',
      tags: [],
      createdAt: '2018-01-26T02:28:32.022Z',
      articleReplies: [
        {
          createdAt: '2018-01-26T03:24:02.724Z',
          negativeFeedbackCount: 0,
          replyType: 'NOT_ARTICLE',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_15',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2018-01-26T03:24:02.724Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-19T20:49:05.228Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_11',
          status: 'NORMAL',
          updatedAt: '2020-05-19T20:49:05.228Z',
        },
      ],
      replyRequestCount: 1,
      appId: 'RUMORS_LINE_BOT',
      lastRequestedAt: '2018-01-26T02:28:32.033Z',
      text:
        '不能只有我拿到！現在來家樂福滴血認親"完成會員認證"，免費拿娘家好禮，完成再抽會員點數一千萬點！\r\nhttps://goo.gl/sNbJrY',
      hyperlinks: [
        {
          summary: null,
          title: null,
          url: 'https://goo.gl/sNbJrY',
        },
      ],
      updatedAt: '2018-01-26T02:28:32.022Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_13': {
      references: [
        {
          createdAt: '2017-01-13T03:49:00.000Z',
          type: 'LINE',
        },
      ],
      userId: 'CBUT_user_37',
      tags: [],
      createdAt: '2017-01-13T03:49:00.000Z',
      articleReplies: [
        {
          createdAt: '2017-07-07T09:39:54.031Z',
          negativeFeedbackCount: 5,
          replyType: 'RUMOR',
          positiveFeedbackCount: 24,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_8',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2017-07-07T09:39:54.031Z',
        },
      ],
      normalArticleReplyCount: 1,
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-05-21T13:46:40.190Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bert',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_8',
          status: 'NORMAL',
          updatedAt: '2020-05-21T13:46:40.190Z',
        },
      ],
      replyRequestCount: 3,
      appId: 'BOT_LEGACY',
      lastRequestedAt: '2017-01-13T03:49:00.000Z',
      text:
        '太白粉擺上一百年也不長蟲，你相信嗎? \n\n肉羹麵、蚵仔麵線、酸辣湯…勾芡的東西容易引起痛風及糖尿病之慢性病 。 \n喜歡吃燴飯肉羹湯的人請注意！ \n\n烹飪老師是西...',
      hyperlinks: [],
      updatedAt: '2017-01-13T03:49:00.000Z',
      normalArticleCategoryCount: 1,
    },
    '/articles/doc/CBUT_article_43': {
      text:
        '百億離岸風電國產大餅訂單爆肥水外流\n蘋果日報\n離岸風電產業近年因政策利多及多家國際能源大廠來台投資，號稱將帶動八千億產值，其中鋼鐵股王世紀集團被視為民營鋼廠的領...',
      createdAt: '2020-09-26T07:29:51.676Z',
      updatedAt: '2020-09-26T07:29:51.676Z',
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      references: [
        {
          type: 'URL',
          permalink: '',
          createdAt: '2020-09-26T07:29:51.676Z',
          userId: 'CBUT_user_35',
          appId: 'WEBSITE',
        },
      ],
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-09-26T07:30:21.061Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_74',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2020-09-26T07:30:21.061Z',
        },
      ],
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-09-26T15:59:24.667Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bow',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-09-26T15:59:24.667Z',
        },
      ],
      normalArticleReplyCount: 1,
      normalArticleCategoryCount: 1,
      replyRequestCount: 1,
      tags: [],
      hyperlinks: [],
      lastRequestedAt: '2020-09-26T07:29:51.708Z',
    },
    '/articles/doc/CBUT_article_44': {
      text:
        '柯文哲行程曝光，也讓現場媒體相當質疑，特別是先前曝光的國務院、國防部皆被取消，由於當天柯的行程相當鬆散，加上柯訪美期間每天都安排早上受訪，唯獨華盛頓安排中午，也...',
      createdAt: '2020-09-26T09:39:48.802Z',
      updatedAt: '2020-09-26T09:39:48.802Z',
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      references: [
        {
          type: 'URL',
          permalink: '',
          createdAt: '2020-09-26T09:39:48.802Z',
          userId: 'CBUT_user_35',
          appId: 'WEBSITE',
        },
      ],
      articleReplies: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-09-26T09:40:02.621Z',
          replyType: 'RUMOR',
          positiveFeedbackCount: 1,
          appId: 'WEBSITE',
          replyId: 'CBUT_reply_75',
          userId: 'CBUT_user_35',
          status: 'NORMAL',
          updatedAt: '2020-09-26T09:40:02.621Z',
        },
      ],
      articleCategories: [
        {
          negativeFeedbackCount: 0,
          createdAt: '2020-09-26T16:07:22.734Z',
          positiveFeedbackCount: 0,
          appId: 'DEVELOPMENT_BACKEND',
          aiModel: 'bow',
          userId: 'CBUT_user_36',
          aiConfidence: 1,
          categoryId: 'CBUT_category_2',
          status: 'NORMAL',
          updatedAt: '2020-09-26T16:07:22.734Z',
        },
      ],
      normalArticleReplyCount: 1,
      normalArticleCategoryCount: 1,
      replyRequestCount: 1,
      tags: [],
      hyperlinks: [],
      lastRequestedAt: '2020-09-26T09:39:48.829Z',
    },
    '/replies/doc/CBUT_reply_76': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '陸委會指出，習近平近日再定義「九二共識」是「兩岸同屬一個中國、共同謀求國家統一」，更倡議探索「一國兩制」台灣方案，踐踏民主程序，透過花瓶式的「政治協商」來分化台...',
      reference:
        'https://www.cna.com.tw/news/aipl/201901090206.aspx\n\n\nhttps://www.rti.org.tw/news/view/id/2007762',
      createdAt: '2019-01-10T07:11:59.378Z',
      hyperlinks: [
        {
          summary:
            '中國國民黨9日在中央黨部舉行中常會，國民黨主席吳敦義（圖）主持會議。中央社記者裴禛攝  108年1月9日（中央社記者余祥台北9日電）國民黨主席吳敦義今天向總統蔡...',
          title: '吳敦義向總統喊話  重新正確認知九二共識 | 政治 | 中央社 CNA',
          url: 'https://www.cna.com.tw/news/aipl/201901090206.aspx',
        },
        {
          summary:
            '時間：2019-01-09 21:24\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t新聞引據：中央社\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t撰稿編輯：陳子華\n\t\t\t\t\t\t\t\t\n\n...',
          title: '國民黨續提九二共識 陸委會：無異附和中共',
          url: 'https://www.rti.org.tw/news/view/id/2007762',
        },
      ],
    },
    '/replies/doc/CBUT_reply_77': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1.新北市的「老人健保補助」，必須符合年紀和設籍滿一年的要件，社會局會主動寄專函通知符合資格的民眾，並非「帶身分證至區公所辦理」。\n\n2.65歲以上領3224元...',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3737',
      createdAt: '2020-04-28T05:56:52.104Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3737',
          summary:
            '「只要戶籍在新北市，年滿65歲可到區公所帶身份證存摺辦理健保補助金，三千二百二十四元，半年領一次...」，經查：\n\n一、新北市確實有「老人健保補助」，但必須是符...',
          title:
            '【錯誤】網傳「只要戶籍在新北市，年滿65歲可到區公所帶身份證存摺辦理健保補助金，三千二百二十四元，半年領一次...」？',
          url: 'https://tfc-taiwan.org.tw/articles/3737',
        },
      ],
    },
    '/replies/doc/CBUT_reply_78': {
      userId: 'CBUT_user_90',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text: '訊息與謠言查證無關。',
      reference: '',
      createdAt: '2020-07-31T08:25:23.003Z',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_79': {
      createdAt: '2017-12-04T15:18:29.041Z',
      appId: 'DEVELOPMENT_FRONTEND',
      text: '聊天',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_16',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_80': {
      userId: 'CBUT_user_45',
      appId: 'DEVELOPMENT_FRONTEND',
      type: 'RUMOR',
      text: '微信支付自2018年起即有收費。',
      reference: 'http://www.sohu.com/a/243987392_561670',
      createdAt: '2019-11-18T18:36:40.797Z',
      hyperlinks: [
        {
          normalizedUrl: 'http://www.sohu.com/a/243987392_561670',
          summary:
            '由于您未安装flash播放器视频无法播放，点击这里安装\n          \n          \n            \n            微信收费升...',
          title: '8月1日起微信又要多收一笔钱，上亿用户将受影响',
          url: 'http://www.sohu.com/a/243987392_561670',
        },
      ],
    },
    '/replies/doc/CBUT_reply_69': {
      reference: '',
      createdAt: '2017-07-10T14:01:55.446Z',
      appId: 'WEBSITE',
      text: '此應該是個人心得。',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_45',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_30': {
      reference: '',
      createdAt: '2017-12-27T14:55:54.646Z',
      appId: 'WEBSITE',
      text: '這是商業活動廣告 ',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_35',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_15': {
      reference: '',
      createdAt: '2018-01-26T03:24:02.717Z',
      appId: 'WEBSITE',
      text: '這是商業活動廣告 ',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_35',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_21': {
      reference: '',
      createdAt: '2017-11-19T10:08:18.033Z',
      appId: 'WEBSITE',
      text: '文字長度太短，疑似為使用者手動輸入之查詢語句，不像轉傳文章。',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_35',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_26': {
      reference: '',
      createdAt: '2017-09-13T13:44:48.946Z',
      appId: 'WEBSITE',
      text: '內容為個人遊記，不在查證範圍',
      type: 'NOT_ARTICLE',
      userId: 'CBUT_user_38',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_27': {
      userId: 'CBUT_user_40',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text: '訊息與謠言查證無關。',
      reference: '',
      createdAt: '2018-07-10T16:17:54.862Z',
    },
    '/replies/doc/CBUT_reply_20': {
      userId: 'CBUT_user_49',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '內政部警政署的輿情回應專區，有特別針對這一則消息製作澄清一覽表：網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表,如附件資料。',
      reference:
        'https://www.npa.gov.tw/NPAGip/wSite/public/Attachment/f1531122098570.pdf',
      createdAt: '2018-10-01T16:02:38.646Z',
      hyperlinks: [
        {
          summary: null,
          title: null,
          url:
            'https://www.npa.gov.tw/NPAGip/wSite/public/Attachment/f1531122098570.pdf',
        },
      ],
    },
    '/replies/doc/CBUT_reply_70': {
      reference:
        'http://www.mygopen.com/2017/06/blog-post_26.html\n神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！',
      createdAt: '2017-07-09T15:02:23.370Z',
      appId: 'WEBSITE',
      text:
        '基本上地瓜葉其實單吃就有效果，牛奶、豆漿可加可不加，民眾不用迷信。 三高或是糖尿病的患者還是要搭配醫生的治療，不能單靠吃地瓜葉就想將身體調養好，民眾千萬要注意。...',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！\n\n\n\n\n\n\n\n\n\n有些食物固然有效，但網路謠言常會過於誇大，像內容為「神...',
          title:
            '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！ ｜MyGoPen',
          url: 'http://www.mygopen.com/2017/06/blog-post_26.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_39': {
      reference:
        'https://www.nownews.com/news/20100519/636359\n藥水比藥錠吸收快？　專業藥師破除網路傳言',
      createdAt: '2017-12-14T04:09:00.110Z',
      appId: 'WEBSITE',
      text:
        '針對特殊族群，如兒童或老人，為了讓藥物更容易服用或是給藥劑量精準，可能會將藥物製成藥水。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '為了提供更好的閱讀內容，我們使用相關網站技術來改善\n使用者體驗，也尊重用戶的隱私權，特別提出說明。 OK',
          title: '找不到與「 News 20100519 636359」相關的文章或頁面',
          url: 'https://www.nownews.com/news/20100519/636359',
        },
      ],
    },
    '/replies/doc/CBUT_reply_0': {
      reference:
        'https://kknews.cc/zh-tw/fashion/lznly52.html\n太早用眼霜會長脂肪粒？別再傳這樣的謠言了',
      createdAt: '2017-11-15T05:23:58.752Z',
      appId: 'WEBSITE',
      text:
        '眼霜並不是導致脂肪粒產生的原因。脂肪粒大多數情況下指的是粟丘疹，它是一種形成於毛囊皮脂腺的表皮囊腫，內部聚集了角質化的殘留物，通常為白色/淡黃色凸起。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '很多妹子們覺得20幾歲還年輕，用不上眼霜。但皺紋的產生是逐步累積的過程，年輕的時候不明顯並不能代表你沒有這個問題！\n...\n脂肪粒的真相\n提到眼霜，就會出現脂肪...',
          title: '太早用眼霜會長脂肪粒？別再傳這樣的謠言了！',
          url: 'https://kknews.cc/zh-tw/fashion/lznly52.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_1': {
      reference:
        'http://www.mygopen.com/2017/10/14line.html\n【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！',
      createdAt: '2017-10-03T03:37:10.740Z',
      appId: 'WEBSITE',
      text:
        '純屬謠言一則，年金改革辦公室也做出澄清了，這完全不是事實，事實上年改會在14次針對年金領取資格會中沒有達成任何共識，勞保方面，根據勞保條例規定，勞保老年年金請領...',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！\n\n\n\n\n\n\n\n\n\n已經傳很久的 LINE 謠言，內容為「昨天年金改革委員會第14次會議，...',
          title:
            '【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！ ｜MyGoPen',
          url: 'http://www.mygopen.com/2017/10/14line.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_35': {
      reference:
        'http://www.edu.tw/News_Content.aspx?n=9E⋯&sms=169B8E91BB75571F&s=6DDDD36D6874DB8...',
      createdAt: '2017-07-06T06:44:19.192Z',
      appId: 'WEBSITE',
      text:
        '教育部依性別平等教育法等相關規定實施性別平等教育課程，教科書也都經過審定。若家長有具體內容，可提出檢舉，教育部會請教科書業者再做審視及調整。\n目前國小教科書的「...',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '抱歉！請您再重新檢視輸入的網址是否正確。您也可以回到上一頁或首頁。\n\t\t\t\t\n\t\t\t\t\n\t\t\t\t\tSorry. Please check one more ti...',
          title: '教育部全球資訊網',
          url:
            'http://www.edu.tw/News_Content.aspx?n=9E⋯&sms=169B8E91BB75571F&s=6DDDD36D6874DB88',
        },
        {
          summary:
            '隨著同志婚姻議題之各方表述及熱烈討論，「性/性別教育」也一併成為關注焦點。各式各樣關於「性/性別教育」的資訊在電視廣告、LINE、臉書等通訊軟體流傳，但大多數描...',
          title:
            '【圖輯】 性別教育不只是性解放 揭密性別教育的十大真相 - The News Lens 關鍵評論網',
          url: 'https://www.thenewslens.com/article/56138',
        },
        {
          summary:
            '同性戀，亦稱為同性向、同性愛，是以同性為對象建立起親密關係，或以此性傾向做為主要自我認同的行為或現象[1]。同性戀連同雙性戀和異性戀構成了性傾向連續光譜不同程度...',
          title: '同性戀 - 维基百科，自由的百科全书',
          url: 'https://zh.wikipedia.org/wiki/同性戀',
        },
      ],
    },
    '/replies/doc/CBUT_reply_8': {
      reference:
        'https://www.fda.gov.tw/tc/newsContent.aspx?cid=5049&id=22092\n會中毒，所以要少吃勾芡料理？',
      createdAt: '2017-07-07T09:39:54.019Z',
      appId: 'WEBSITE',
      text:
        '太白粉的樹薯塊根含有微量具有毒性之氰酸，食用後會引發噁心、嘔吐、下痢、昏眩，甚至會呼吸衰竭而死亡。\n樹薯粉是將樹薯經過浸水、脫汁、壓榨、加熱和乾燥等加工過程後抽...',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '聽說吃樹薯粉會中毒，所以要少吃勾芡料理，這是真的嗎？\n                【發布日期：2017-05-02】 \n                發布單位：企劃及科技管理組',
          title:
            '聽說吃樹薯粉會中毒，所以要少吃勾芡料理，這是真的嗎？-衛生福利部食品藥物管理署',
          url: 'https://www.fda.gov.tw/tc/newsContent.aspx?cid=5049&id=22092',
        },
      ],
    },
    '/replies/doc/CBUT_reply_32': {
      reference: 'http://news.ltn.com.tw/news/life/paper/1060978',
      createdAt: '2017-03-27T14:21:52.619Z',
      appId: 'WEBSITE',
      text:
        '教育部昨澄清，目前性平教材是早經公聽及專家修訂而無爭議，是提供教師增加知能，盼外界勿再以訛傳訛。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '2016-12-12〔記者吳柏軒、翁聿煌、沈佩瑤／台北報導〕婚姻平權議題沸騰，也牽動校園性平教育備受矚目。近期網路卻屢傳出影片《性教慾》，引用過期資料誣指官方要...',
          title: '性平教材遭抹黑 教部：勿以訛傳訛 - 生活 - 自由時報電子報',
          url: 'http://news.ltn.com.tw/news/life/paper/1060978',
        },
      ],
    },
    '/replies/doc/CBUT_reply_72': {
      reference:
        'https://www.everydayhealth.com.tw/article/857\nhttps://www.amazon.co.jp/%E8%AA%8D...',
      createdAt: '2017-04-12T11:21:23.414Z',
      appId: 'WEBSITE',
      text:
        '文中的日本醫學博士是安田和人，有針對失智症食療出書，內容確實包含文中四項食物。',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_85',
      hyperlinks: [
        {
          summary:
            '【早安健康／孫于琛】每個人都有可能得到失智症，除了多運動並多訓練腦力外，其實日常飲食對我們身體才有最大的影響。不是只有吃藥才能治好，有很多種食物其實對預防甚至改...',
          title: '早安健康｜防失智不用吃藥！日本醫學博士推薦4食材',
          url: 'https://www.everydayhealth.com.tw/article/857',
        },
        {
          summary:
            'Prime Video                                                                     ...',
          title:
            '認知症 治った! 助かった! この方法 | 安田 和人 |本 | 通販 | Amazon',
          url:
            'https://www.amazon.co.jp/%E8%AA%8D%E7%9F%A5%E7%97%87-%E6%B2%BB%E3%81%A3%E3%81%9F...',
        },
      ],
    },
    '/replies/doc/CBUT_reply_13': {
      reference: 'http://www.tada2002.org.tw/tada_know_02.html',
      createdAt: '2017-04-12T11:25:01.484Z',
      appId: 'WEBSITE',
      text:
        '失智症的成因多元，如有症狀須儘早就醫對症下藥，食療是好的輔助方式，但文中提到「不用吃藥」有誇大之嫌。',
      type: 'RUMOR',
      userId: 'CBUT_user_85',
      hyperlinks: [
        {
          summary:
            '失智人口知多少\n                    台灣失智症人口推估全球失智症人口快速增加依據2017年國際失智症協會（ADI）資料，推估2017年將新增...',
          title: '關於失智症-認識失智症',
          url: 'http://www.tada2002.org.tw/tada_know_02.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_66': {
      reference:
        'http://pension.president.gov.tw/cp.aspx?n=03E38CC0FF75CFBE&s=FB4868255F8CFFAD',
      createdAt: '2017-03-24T16:51:23.373Z',
      appId: 'WEBSITE',
      text: '年改會第14次會議重點引述非事實，請參閱資料來源。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary: '',
          title:
            '總統府國家年金改革委員會-【澄清稿】年改會第14次會議重點引述非事實',
          url:
            'http://pension.president.gov.tw/cp.aspx?n=03E38CC0FF75CFBE&s=FB4868255F8CFFAD',
        },
      ],
    },
    '/replies/doc/CBUT_reply_6': {
      reference:
        'http://news.ltn.com.tw/news/life/paper/111309\n心肌梗塞突襲 咳嗽不能自救 快就醫才對',
      createdAt: '2018-01-23T15:42:02.674Z',
      appId: 'WEBSITE',
      text:
        '假的。\n這項處置主要是用於當醫護人員觀測到患者心跳出現異常，為了延長其意識清醒時間，由專業人員指導患者咳嗽；而非一般心臟病患者自救。\n台北榮總急診部主治醫師高偉...',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '2007-01-13記者洪素卿／台北報導「現在是下午5點50分，忙忙碌碌的你上了一整天的班，正在開車回家的路上。突然！你感到胸口有一股劇痛，並且開始蔓延到手臂和...',
          title: '心肌梗塞突襲 咳嗽不能自救 快就醫才對 - 生活 - 自由時報電子報',
          url: 'http://news.ltn.com.tw/news/life/paper/111309',
        },
      ],
    },
    '/replies/doc/CBUT_reply_23': {
      reference:
        '\nhttp://www.appledaily.com.tw/realtimenews/article/new/20170614/1139982/\n海獸胃腺蟲」鑽胃壁　專家：過去國內僅１例',
      createdAt: '2017-08-29T04:03:40.178Z',
      appId: 'WEBSITE',
      text:
        '過去台灣只有一個海獸胃腺蟲鑽入胃壁的病例，而且僅有一隻蟲。而經查詢，台北馬偕醫院也沒有名為林清風的醫師。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '最近有則LINE訊息在網路上廣傳，指好多人因吃生魚片，「胃壁附著「海獸胃腺蟲」，有的病人甚至胃壁上滿滿都是，無法夾出，驅蟲藥也很難治，魚片中的生鮭魚含蟲量是最可...',
          title: '「海獸胃腺蟲」鑽胃壁　專家：過去國內僅１例 | 蘋果日報',
          url:
            'http://www.appledaily.com.tw/realtimenews/article/new/20170614/1139982/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_73': {
      reference: 'https://zh.wikipedia.org/wiki/李孟贤',
      createdAt: '2018-01-01T12:00:04.036Z',
      appId: 'WEBSITE',
      text:
        '舊金山前市長確實因為心臟病去世，但其他部分應該只是想要提醒大家注意心臟疾病。',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_45',
      hyperlinks: [
        {
          summary:
            '李孟贤Edwin Mah Lee\n\n第43任舊金山市長\n任期2011年1月11日－2017年12月12日前任\n加文·纽森继任\n伦敦·布里德（代理）个人资料出生\n...',
          title: '李孟贤 - 维基百科，自由的百科全书',
          url: 'https://zh.wikipedia.org/wiki/李孟贤',
        },
      ],
    },
    '/replies/doc/CBUT_reply_37': {
      reference:
        'http://www.appledaily.com.tw/realtimenews/article/new/20150819/672912/',
      createdAt: '2017-01-10T02:42:00.000Z',
      appId: null,
      text:
        '公路總局說，這些內容都是錯的，麥擱傳啦！\n公路總局監理組組長林福山表示，這則訊息每隔一段時間就會現身在網路\n\n根據道路交通管理處罰條例第60條規定，汽車駕駛人如...',
      type: 'RUMOR',
      hyperlinks: [
        {
          summary:
            '最近不少Line友又開始瘋傳一則已有10年以上年資的網路謠言，內容稱「本月20日起，全國將實施紅燈越線處罰條例，凡是在等待紅燈時，超出白色禁止越線者馬上處以90...',
          title: '本月20日實施紅燈越線處罰條例！假的啦 | 蘋果日報',
          url:
            'http://www.appledaily.com.tw/realtimenews/article/new/20150819/672912/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_16': {
      reference: 'http://www.ettoday.net/news/20130826/261531.htm',
      createdAt: '2017-03-21T16:25:34.315Z',
      appId: 'WEBSITE',
      text:
        '愛滋病毒離開活體後，1個小時以內就無法存活。即使愛滋病毒血液被滴入罐頭，接觸後感染機率非常低。\n另外，就算吃進仍存活的愛滋病毒，如果口腔沒有傷口，也不會因此感染愛滋病。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '▲國際愛滋病研討會發表資料照片。（圖／達志影像／美聯社）\n東森新聞記者陳稚暉、謝宗樺／台中報導\n又是一起網路謠言！已經有人在臉書（Facebook）上看到消息誤...',
          title: '謠言勿再傳！「泰國罐頭遭滴愛滋血」太扯 | ETtoday新聞雲',
          url: 'http://www.ettoday.net/news/20130826/261531.htm',
        },
      ],
    },
    '/replies/doc/CBUT_reply_38': {
      reference:
        'http://www.traffic.police.ntpc.gov.tw/cp-1084-32629-27.html\n網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表',
      createdAt: '2017-10-15T16:05:28.182Z',
      appId: 'WEBSITE',
      text:
        '假的。近來網路、Line瘋狂流傳，自20日起交通違規將大幅調漲，由於調幅頗大，網友互傳時罵聲不斷，均為錯誤訊息，網路謠言希望能止於智者，不要再以訛傳訛混淆視聽。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary: '',
          title:
            '網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表-新北市政府警察局交通警察大隊',
          url: 'http://www.traffic.police.ntpc.gov.tw/cp-1084-32629-27.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_7': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '假的。其實是「違規行為終了日」到「檢舉日期」超過七天，才不予舉發，跟開單日期一點關係都沒有。\n\n根據道路交通管理處罰條例第 7-1 條的解釋：對於違反本條例之行...',
      reference:
        'https://www.mygopen.com/2018/03/line_27.html\n【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言',
      createdAt: '2018-03-30T14:19:54.455Z',
      hyperlinks: [
        {
          summary:
            '【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言\n\n\n\n\n\n\n\n\n\n最近 LINE 收到「如果遇到檢舉罰單請注意檢舉日期跟開單日期，很多朋友都不知道...',
          title:
            '【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言 ｜MyGoPen',
          url: 'https://www.mygopen.com/2018/03/line_27.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_3': {
      reference: 'https://www.youtube.com/watch?v=RbrCFWkwhPQ',
      createdAt: '2017-08-01T13:53:32.743Z',
      appId: 'WEBSITE',
      text: '此為不實謠言。',
      type: 'RUMOR',
      userId: 'CBUT_user_45',
      hyperlinks: [
        {
          summary:
            '香蕉擺久了會變黑，網路有一段影片，宣稱利用白米、夾鏈袋和吹風機，就可以讓香蕉回春，由黑變黃。記者實際操作，香蕉不僅沒復活，還被理化老師打臉。',
          title: '【中視新聞】米+吹風機 香蕉回春黑轉黃? 實測! 20150618',
          url: 'https://www.youtube.com/watch?v=RbrCFWkwhPQ',
        },
      ],
    },
    '/replies/doc/CBUT_reply_71': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '基本上地瓜葉其實單吃就有效果，牛奶、豆漿可加可不加，民眾不用迷信。 三高或是糖尿病的患者還是要搭配醫生的治療，不能單靠吃地瓜葉就想將身體調養好，民眾千萬要注意。...',
      reference:
        'http://www.mygopen.com/2017/06/blog-post_26.html\n神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！',
      createdAt: '2018-06-10T13:21:16.828Z',
      hyperlinks: [
        {
          summary:
            '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！\n\n\n\n\n\n\n\n\n\n有些食物固然有效，但網路謠言常會過於誇大，像內容為「神...',
          title:
            '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！ ｜MyGoPen',
          url: 'http://www.mygopen.com/2017/06/blog-post_26.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_10': {
      reference:
        'http://sp1.hso.mohw.gov.tw/doctor/Often_question/type_detail.php?PageNo=2&q_type...',
      createdAt: '2018-02-07T17:33:53.022Z',
      appId: 'WEBSITE',
      text:
        '在夜間睡眠時，由於活動量降低，會使得體溫下降，倘保暖不足時，就會出現手腳冰冷的現象，甚至連身體都涼；同時吸入冷空氣刺激支氣管黏膜，而引發咳嗽。\n ',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary: '提醒您，網路醫療諮詢服務並不能代替就醫。',
          title: '',
          url:
            'http://sp1.hso.mohw.gov.tw/doctor/Often_question/type_detail.php?PageNo=2&q_type...',
        },
        {
          summary:
            '▲幼兒沒病卻「咳不停」？半夜踢被＋空氣太冷是主因。（圖／示意圖／達志影像）\n記者嚴云岑／台北報導\n3歲男童，最近幾個月，半夜經常咳不停，雖然睡眠並未因此中斷，就...',
          title:
            '幼兒沒病卻「咳不停」？　半夜踢被＋空氣太冷是主因 | ETtoday健康雲',
          url: 'https://health.ettoday.net/news/614785',
        },
      ],
    },
    '/replies/doc/CBUT_reply_40': {
      userId: 'CBUT_user_45',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text: '公投結果並不會影響到大法官的釋憲結果，照常會生效。',
      reference:
        'https://www.storm.mg/article/376727\nhttps://www.chinatimes.com/realtimenews/20180419002190-260407',
      createdAt: '2018-10-23T11:51:05.408Z',
      hyperlinks: [
        {
          summary:
            '立法院甫完成《公投法》的修法，公投門檻大幅降低，「直接民權」顛覆「代議制」的大戰一觸即發。前司法院副院長蘇永欽接受《風傳媒》專訪指出，公投若決定的是法律事項，無...',
          title:
            '反同婚公投過關可推翻釋憲案？蘇永欽：大法官說了算，但恐違反民主理論-風傳媒',
          url: 'https://www.storm.mg/article/376727',
        },
        {
          summary: null,
          title: null,
          url: 'https://www.chinatimes.com/realtimenews/20180419002190-260407',
        },
      ],
    },
    '/replies/doc/CBUT_reply_41': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '假的。\n1.衛福部回應，外籍人士感染愛滋人數並無增加。婚姻的成立，需要多重步驟，且除了我國，國外許多先進國家，也提供愛滋治療，疫情也已獲控制。因此，入籍台灣享受...',
      reference:
        'http://www.cdc.gov.tw/info.aspx?treeid=4⋯id=1bd193ed6dabaee6&tid=0579D4F36FF0AF9...',
      createdAt: '2018-10-23T17:31:53.634Z',
      hyperlinks: [
        {
          summary: '',
          title: '衛生福利部疾病管制署',
          url:
            'http://www.cdc.gov.tw/info.aspx?treeid=4⋯id=1bd193ed6dabaee6&tid=0579D4F36FF0AF94',
        },
        {
          summary:
            '最近有太多關於愛滋的謠言，讓我們來告訴你真相究竟是什麼\n製作：台灣同志諮詢熱線協會、愛滋權促會\n設計：開合跳工作室\n　\n\t以下這些流言/謠言，你是不是也聽過呢？...',
          title: '【破除謠言】你所不知道的愛滋真相網頁版(另附錄政府闢謠圖文)',
          url: 'https://hotline.org.tw/blog/1111',
        },
        {
          summary:
            'udn.com 提供即時新聞以及豐富的政治、社會、地方、兩岸、國際、財經、數位、運動、NBA、娛樂、生活、健康、旅遊新聞，以最即時、多元的內容，滿足行動世代的需求。',
          title: '',
          url: 'https://udn.com/news/story/6656/2484493',
        },
        {
          summary:
            '12\n05月\n\t\t\t\t\t\t\t\t\t\n\n\t\t\t\t\t\t\t\t\t\n\n\t\t\t\t\t\t\t\t\t\t落實校園性別平等教育，保護孩子遠離性侵害（新聞稿）\n\n\t\t\t\t\t\t\t\t\t\t 201...',
          title: '教育部性別平等全球資訊網',
          url: 'https://www.gender.edu.tw/web/index.php/m1/m1_03_01?sid=408',
        },
        {
          summary: null,
          title: null,
          url: 'https://www.tgeea.org.tw/issue/3396/',
        },
        {
          summary:
            '婚姻平權議題持續延燒，反同團體在同婚修法之外，另外再開性別平等教育戰場，以家長之名在全國縣市議會串連議員發動提案、開座談會，近日甚至將矛頭對準校園，動員外校家長...',
          title: '性別光譜不是你想的性別混淆　高市印宣傳摺頁破除性平謠言--上報',
          url: 'https://www.upmedia.mg/news_info.php?SerialNo=18179',
        },
        {
          summary: null,
          title: null,
          url: 'https://www.awakening.org.tw/topic/2460',
        },
        {
          summary:
            '下一代幸福聯盟（以下簡稱下福盟）發起「反同志教育公投案」，內容是：「你是否同意在國民教育階段內（國中及國小），教育部及各級學校不應對學生實施性別平等教育法施行細...',
          title:
            '反同志教育公投一旦通過，同志的無奈人生只會無止盡地延續下去 - The News Lens 關鍵評論網',
          url: 'https://www.thenewslens.com/article/103774',
        },
        {
          summary:
            'udn.com 提供即時新聞以及豐富的政治、社會、地方、兩岸、國際、財經、數位、運動、NBA、娛樂、生活、健康、旅遊新聞，以最即時、多元的內容，滿足行動世代的需求。',
          title: '',
          url: 'https://udn.com/news/story/10966/2483385',
        },
        {
          summary:
            '喧騰多時的同性婚姻爭議終於有解，大法官今釋憲宣告《民法》禁止同性結婚違憲。而根據法務部統計資料顯示，目前世界共有23國承認同性婚姻並以法律保障，其中南非、英國另...',
          title: '【同婚釋憲】除了台灣　全球23國承認同婚 | 蘋果日報',
          url:
            'http://www.appledaily.com.tw/realtimenews/article/new/20170524/1124709/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_42': {
      userId: 'CBUT_user_53',
      appId: 'WEBSITE',
      type: 'NOT_RUMOR',
      text:
        '1. 大法官748釋憲確實為「亞洲第1同婚國家」打開了「潘朵拉」盒子！為何呢？因為探究婚姻的「真義」( https://bit.ly/2O2sRAH )，顯然與...',
      reference:
        'https://tw.news.yahoo.com/%E5%90%8C%E6%80%A7%E5%A9%9A%E5%A7%BB%E9%87%8B%E6%86%B2...',
      createdAt: '2018-10-24T13:19:03.710Z',
    },
    '/replies/doc/CBUT_reply_46': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 馬偕醫院沒有「林清風」醫師。\n2. 這謠言連寄生蟲名字都寫錯，正確名稱是「海獸胃線蟲」\n3. 鮭魚被歸類為淡水魚，不容易遭海獸胃腺蟲寄生，反倒白帶魚、青魚...',
      reference: '《泛科學》有文章闢謠：\nhttps://pansci.asia/archives/122243',
      createdAt: '2018-12-12T04:33:27.009Z',
      hyperlinks: [
        {
          summary:
            '作者 / 林慶順教授    編輯 / 吳培安、雷雅淇\n\n《緊急通知》\n台北馬偕醫院傳來訊息：\n資深醫生（林清風）傳來：「請大家以後千萬不要再吃生魚片了！」\n因為...',
          title:
            '吃生魚片容易感染「海獸胃線蟲」，尤其鮭魚的含蟲量最高？ - PanSci 泛科學',
          url: 'https://pansci.asia/archives/122243',
        },
      ],
    },
    '/replies/doc/CBUT_reply_18': {
      userId: 'CBUT_user_73',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '覆蓋麵粉不但無法冷卻皮膚，還會阻止皮膚散熱，甚至使傷口感染。 燙傷應立刻以流動清水沖洗冷卻，醬油、牙膏、麵粉都不是在無菌的環境下生產的，可能引起傷口感染，而且增...',
      reference:
        '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤http://news.ltn.com.tw/news/life/paper/1101504\n燙傷撒麵粉能...',
      createdAt: '2018-12-28T13:27:11.319Z',
      hyperlinks: [
        {
          summary:
            '2017-05-11記者陳炳宏／台北報導一名4歲男童嬉戲時，不慎絆倒電熱水壺，造成大面積燙傷，阿公和阿嬤在慌亂中，急忙脫掉他的衣服，並撒麵粉，企圖「降溫」，沒想...',
          title:
            '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤 - 生活 - 自由時報電子報',
          url: 'http://news.ltn.com.tw/news/life/paper/1101504',
        },
        {
          summary:
            '一名4歲男童因被熱水燙傷，爺爺奶奶一時心急，搶在第一時間先脫下男童衣服，還撒上麵粉，以為能藉此降溫，結果導致傷口更難散熱。治療燒燙傷的偏方千奇百怪，除了小護士、...',
          title: '燙傷撒麵粉能降溫？醫生：敷藥前得先刮層皮 - 康健雜誌',
          url:
            'http://www.commonhealth.com.tw/article/article.action?nid=74762',
        },
      ],
    },
    '/replies/doc/CBUT_reply_25': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text: '送出文章的人在嘗試與機器人聊天。',
      reference: '',
      createdAt: '2019-03-19T14:57:01.101Z',
    },
    '/replies/doc/CBUT_reply_64': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '假的。華府智庫戰略暨國際研究中心（CSIS）亞洲資深顧問葛來儀（Bonnie Glaser）說，有些人在捏造事件，假新聞非常普遍。',
      reference:
        'https://news.ltn.com.tw/news/politics/breakingnews/2754266\n\nhttps://www.setn.com...',
      createdAt: '2019-04-15T16:40:37.582Z',
      hyperlinks: [
        {
          summary:
            '高雄市長韓國瑜9日啟程訪美。（資料照）\n\n\n2019-04-10 13:15〔即時新聞／綜合報導〕高雄市長韓國瑜昨（9日）啟程訪美，臉書社團「韓國瑜加油團」日前...',
          title:
            '川普求見韓國瑜？ 葛來儀一句話狠打臉.... - 政治 - 自由時報電子報',
          url: 'https://news.ltn.com.tw/news/politics/breakingnews/2754266',
        },
        {
          summary:
            '政治中心／台北報導高雄市長韓國瑜即將出發訪美，如今卻有韓粉在臉書社團爆料，說美國總統川普透過秘密管道傳達訊息給高雄市政府，希望韓國瑜能充當美中貿易戰之間的和事佬...',
          title:
            '說故事大師！韓粉爆川普求見韓國瑜　網酸：不是說好取代習 | 政治 | 三立新聞網  SETN.COM',
          url: 'https://www.setn.com/News.aspx?NewsID=524453',
        },
        {
          summary: null,
          title: null,
          url:
            'https://www.storm.mg/article/1159024?srcid=gAAAAABctLPDyIyvBIeudem6SegNC_3_JhLhr...',
        },
      ],
    },
    '/replies/doc/CBUT_reply_63': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '日本人砍伐台灣森林總面積：1萬8432公頃 \n國民政府與民間砍伐總面積：34萬4千多公頃 ',
      reference: 'https://www.douban.com/group/topic/60942708/',
      createdAt: '2019-04-18T11:30:15.497Z',
      hyperlinks: [
        {
          summary:
            '来自: 凱恩司(叫我凱拔!!)\n                2014-08-26 10:32:17\n            \n\n            \n ...',
          title: '台灣歷史課本不提的森林史真相　(轉　作者：黃鈺翔)',
          url: 'https://www.douban.com/group/topic/60942708/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_28': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '高中課本裡面只有同志諮詢熱線的電話。影片中手機的網址，是家長自己上網找的同志諮詢熱線官網，教育部審定的課本裡面並沒有網址。\n\n同志諮詢熱線是同志朋友的生命線，2...',
      reference:
        '尤美女委員臉書貼文：台灣同志諮詢熱線是同志朋友的生命線\nhttps://www.facebook.com/yumeinu/posts/1553089634706...',
      createdAt: '2019-05-13T08:50:36.806Z',
      hyperlinks: [
        {
          summary:
            'To personalise content, tailor and measure ads and provide a safer experience, w...',
          title: 'Security Check',
          url: 'https://www.facebook.com/yumeinu/posts/1553089634706916',
        },
        {
          summary:
            '台灣同志諮詢熱線協會對於網路謠傳本會「鼓勵高中生吸毒」影片之回應\n\n近日社群媒體上流傳本會「鼓勵高中生吸毒」的影片，內容為有心人士惡意地針對本會經營的爽歪歪網站...',
          title:
            '20190513台灣同志諮詢熱線協會對於網路謠傳本會「鼓勵高中生吸毒」影片之回應',
          url: 'https://hotline.org.tw/news/1594',
        },
        {
          summary:
            'Skip to content\n\t\n\t\n\t\n\t\t\n\t\t\n\t\t\t\n\n\t\t\t\n\t\t\t\n\t\t\t\t\n\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\tThe Works at Toron...',
          title: 'Harm Reduction Supplies & Locations',
          url: 'https://reurl.cc/WWVMe',
        },
        {
          summary:
            '派對藥物的使用在台灣是既存的事實，而政府單位對於派對藥物的因應方式，長期以來僅僅著重在「恐嚇藥物很可怕」、 「被抓到你就完了」的階段，卻忽略了現實環境中，許多人...',
          title: '關於用藥—重要提醒 | 熱線男同志性愉悅性教育網站-爽歪歪 SongYY',
          url: 'https://www.songyy.org.tw/sex/sex-6/sex6-2',
        },
      ],
    },
    '/replies/doc/CBUT_reply_29': {
      reference:
        'http://news.ltn.com.tw/news/life/breakingnews/1950973\n泰豐大火空氣落塵',
      createdAt: '2017-06-12T11:50:12.535Z',
      appId: 'WEBSITE',
      text:
        '衛生局會根據環保局監測結果隨時提醒民眾注意防範事項，呼籲大家不要過度恐慌。',
      type: 'RUMOR',
      userId: 'CBUT_user_35',
      hyperlinks: [
        {
          summary:
            '2017-01-18 11:27〔記者陳昀／桃園報導〕桃園市中壢區昨天發生輪胎廠大火，空氣含有落塵，桃園市政府衛生局提醒民眾，這幾天要儘量避免外出，衛生局也準備...',
          title: '泰豐大火空氣落塵嚴重 3萬口罩免費索取 - 生活 - 自由時報電子報',
          url: 'http://news.ltn.com.tw/news/life/breakingnews/1950973',
        },
      ],
    },
    '/replies/doc/CBUT_reply_68': {
      reference:
        'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
      createdAt: '2017-06-24T07:35:54.955Z',
      appId: 'WEBSITE',
      text:
        '目前平台之連署議案僅針對公共化幼稚園，要求政府在特定年度未完成之目標數量，因累加至下一年，確保最終仍為1000間。\n並如原訊息中沒有要求沒開成的非營利幼兒園一定...',
      type: 'RUMOR',
      userId: 'CBUT_user_45',
      hyperlinks: [
        {
          summary:
            '教育部長潘文忠今(20)日赴行政院報告「擴大幼兒教保公共化」相關規劃，為符膺總統托育政策，提供價格合理、品質有保障的教保服務，減輕家長的經濟負擔，教育部以穩健提...',
          title: '擴大公共化教保服務，教育部規劃增設公共化幼兒園1,000班',
          url:
            'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
        },
      ],
    },
    '/replies/doc/CBUT_reply_58': {
      reference:
        'http://www.appledaily.com.tw/appledaily/article/headline/20151103/36878044/',
      createdAt: '2017-06-20T17:29:47.805Z',
      appId: 'WEBSITE',
      text:
        '江守山醫師畢業自陽明大學醫學系，曾任台北榮總腎臟科總醫師、內科主治醫師；新光醫院腎臟科主治醫師、腎臟科主任，專長為腎病、腎衰竭治療。對醫療保健應該有一定專業。但...',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_45',
      hyperlinks: [
        {
          summary:
            '【綜合報導】腎臟科名醫江守山日前撰文，影射他任職的台北市新光醫院營運出狀況、醫療設備長期不更換、用藥較差等，恐吹熄燈號。昨院方反擊，指兩年前就查獲江涉Ａ健保，讓...',
          title: '名醫江守山A健保 醫院開除 | 蘋果日報',
          url:
            'http://www.appledaily.com.tw/appledaily/article/headline/20151103/36878044/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_31': {
      reference: 'https://my83.com.tw/blogs?p=1338',
      createdAt: '2017-06-19T14:54:52.139Z',
      appId: 'WEBSITE',
      text:
        '根據保險資訊網站My83表示，只要非犯罪或過失的汽機車事故，事故的受害者不論有無過失，能迅速獲得理賠，不需在急需就醫時還花時間等待理賠金，所以政府強制汽車、機車及機器腳踏車主投保。',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_45',
      hyperlinks: [
        {
          summary:
            '汽車族、機車族及電動腳踏車族們，對於每年按時繳交強制險的規範及保障都有瞭解了嗎？\n\nMY83希望藉由強制險10問答，讓大家清楚強制投保的強制險，到底提供什麼保障...',
          title:
            '強制投保的強制險，有什麼保障？10 問答告訴你！ - 《MY83 保險專欄》',
          url: 'https://my83.com.tw/blogs?p=1338',
        },
      ],
    },
    '/replies/doc/CBUT_reply_2': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '「外國同性戀藉結婚來台治療愛滋病，都是健保支付，全民買單」這句話其實並不算錯，但實務上並不會有「拖垮健保」的狀況，也跟同性婚姻關係不大，所以不用太擔心唷。\n\n身...',
      reference:
        '衛福部 - 有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明 \nhttps://www.cdc.gov.tw/in...',
      createdAt: '2018-10-25T16:17:06.241Z',
      hyperlinks: [
        {
          summary:
            '有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明： 外籍人士因工作或其他因素申請在台居留，並合法居留六個月以上，才...',
          title:
            '有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明',
          url:
            'https://www.cdc.gov.tw/info.aspx?treeid=45da8e73a81d495d&nowtreeid=1bd193ed6daba...',
        },
        {
          summary:
            '圖／沃草烙哲學提供最近大家應該多少都有看到一些網路資訊，指出同性婚姻合法化會吸引外籍愛滋患者1來台就醫，然後拖垮台灣健保。藉由這些資訊，有些反同人士進一步建立這...',
          title:
            '蔣維倫／愛滋補助是用來拖垮健保的嗎？從效益主義觀看 | 沃草烙哲學 | 鳴人堂',
          url: 'https://opinion.udn.com/opinion/story/6685/2362679',
        },
        {
          summary:
            '最近有太多關於愛滋的謠言，讓我們來告訴你真相究竟是什麼\n製作：台灣同志諮詢熱線協會、愛滋權促會\n設計：開合跳工作室\n　\n\t以下這些流言/謠言，你是不是也聽過呢？...',
          title: '【破除謠言】你所不知道的愛滋真相網頁版(另附錄政府闢謠圖文)',
          url: 'https://hotline.org.tw/blog/1111',
        },
        {
          summary:
            '▲最具影響力的新聞，都在NOWnews今日新聞。（圖／翻攝自網路）\n\n網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔說法，甚至嘲諷「...',
          title:
            '沒這麼好康！外國人治愛滋　疾管署：須先自掏40萬 | 生活 | NOWnews 今日新聞',
          url: 'https://www.nownews.com/news/20170206/2395108/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_33': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '「小六大兒子被強迫跟另外一個女生練習自慰」是錯的。\n\n影片裡的媽媽在 1:30 的時候說的是：「哥哥在六年級，拿回學習單，哥哥回來問，為什麼學這個東西，要把我和...',
      reference:
        '學習單影本與教材截圖\nhttp://max-everyday.com/2017/01/masturbation/\n\n2017/1/4 中央社 - 議員爆小學課程...',
      createdAt: '2018-11-03T16:14:49.163Z',
      hyperlinks: [
        {
          summary:
            '要不要教小學六年級的同學這個題目，目前爭議很大，有很多家長贊成，很多家長反對。\n北市議員痛批國小課程鼓吹自慰？ 教育局回應：正確的觀念、態度才是最重要的，手冊是...',
          title: '國小六年級學習「什麼叫自慰」 – Max的每一天',
          url: 'http://max-everyday.com/2017/01/masturbation/',
        },
        {
          summary:
            '（中央社記者游凱翔台北4日電）台北市議員汪志冰與多名家長今天抨擊有小學在課堂教導「什麼是自慰」，教育局事後強調，這本教材來自學童常提的420個兩性問題，而自慰只...',
          title:
            '議員爆小學課程教導自慰 真相是這樣 | 地方 | 重點新聞 | 中央社 CNA',
          url: 'https://www.cna.com.tw/news/firstnews/201701040187.aspx',
        },
      ],
    },
    '/replies/doc/CBUT_reply_34': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '訊息與影片標題的「小六大兒子被強迫跟另外一個女生練習自慰」是錯的。\n\n影片裡的媽媽在 1:30 的時候說的是：「哥哥在六年級，拿回學習單，哥哥回來問，為什麼學這...',
      reference:
        '學習單影本與教材截圖\nhttp://max-everyday.com/2017/01/masturbation/\n\n2017/1/4 中央社 - 議員爆小學課程...',
      createdAt: '2018-11-03T16:17:08.155Z',
      hyperlinks: [
        {
          summary:
            '要不要教小學六年級的同學這個題目，目前爭議很大，有很多家長贊成，很多家長反對。\n北市議員痛批國小課程鼓吹自慰？ 教育局回應：正確的觀念、態度才是最重要的，手冊是...',
          title: '國小六年級學習「什麼叫自慰」 – Max的每一天',
          url: 'http://max-everyday.com/2017/01/masturbation/',
        },
        {
          summary:
            '（中央社記者游凱翔台北4日電）台北市議員汪志冰與多名家長今天抨擊有小學在課堂教導「什麼是自慰」，教育局事後強調，這本教材來自學童常提的420個兩性問題，而自慰只...',
          title:
            '議員爆小學課程教導自慰 真相是這樣 | 地方 | 重點新聞 | 中央社 CNA',
          url: 'https://www.cna.com.tw/news/firstnews/201701040187.aspx',
        },
      ],
    },
    '/replies/doc/CBUT_reply_54': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text: '訊息與謠言查證無關。',
      reference: '',
      createdAt: '2019-06-02T01:40:59.872Z',
    },
    '/replies/doc/CBUT_reply_60': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 燙傷的正確處理方式應是「沖、脫、泡、蓋、送」\n2. 奇美醫院急診部副主任許建清則表示，醬油、牙膏、麵粉都不是在無菌的環境下生產的，可能引起傷口感染，而且增...',
      reference:
        'https://www.commonhealth.com.tw/article/article.action?nid=74762',
      createdAt: '2019-07-02T04:09:52.081Z',
      hyperlinks: [
        {
          summary: null,
          title: null,
          url:
            'https://www.commonhealth.com.tw/article/article.action?nid=74762',
        },
      ],
    },
    '/replies/doc/CBUT_reply_9': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text: '每個人對於兩蔣時代的生命經歷不同，因此有各自不同的觀點。',
      reference:
        '網友逐條回應\nhttps://m.facebook.com/morningdusts/posts/645289662344314\n\nhttps://m.face...',
      createdAt: '2018-03-11T02:45:59.101Z',
      hyperlinks: [
        {
          summary:
            '1.因為白色恐怖加特務，你會被說讀禁書就死了 2.因為白色恐怖加特務，你會被說你民粹就死了 3.因為這些事情以外的部分，都不敢上報入書或寫成歌，不然報紙就被禁了...',
          title: '晨塵',
          url: 'https://m.facebook.com/morningdusts/posts/645289662344314',
        },
        {
          summary:
            '以下˙是我的臉友 Pasoa Yang 對26條 “兩蔣時代是布拉布拉的美好時代” 的回應 :   "我很不幸, 能夠親身感受那個悲惨時代。 這是「兩蔣時代」。...',
          title: '段震宇',
          url:
            'https://m.facebook.com/story.php?story_fbid=1728483420523821&id=100000865406669',
        },
      ],
    },
    '/replies/doc/CBUT_reply_22': {
      reference:
        '比對 join 平台提案內容以及教育部新聞稿 https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB\n',
      createdAt: '2017-06-27T15:25:17.272Z',
      appId: 'WEBSITE',
      text:
        '於「擴大幼兒教保公共化」的政府公開說明中，確實沒有指出如果沒有達成預計數字的話會有什麼作為。\n全教總於提案之中，確實提議將少開的班級加入下一年度班級數，而且在提...',
      type: 'NOT_RUMOR',
      userId: 'CBUT_user_15',
      hyperlinks: [
        {
          summary:
            '教育部長潘文忠今(20)日赴行政院報告「擴大幼兒教保公共化」相關規劃，為符膺總統托育政策，提供價格合理、品質有保障的教保服務，減輕家長的經濟負擔，教育部以穩健提...',
          title: '擴大公共化教保服務，教育部規劃增設公共化幼兒園1,000班',
          url:
            'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
        },
      ],
    },
    '/replies/doc/CBUT_reply_43': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 2017年度使用台灣健保之外籍愛滋病患，僅有 25 人，花費僅佔台灣健保資源之百萬分之五點二。對台灣健保之影響微乎其微。\n2. 我國不禁止外籍愛滋病患入境...',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/260',
      createdAt: '2018-11-22T15:55:57.477Z',
      hyperlinks: [
        {
          summary:
            '通訊軟體流傳多則「外國愛滋病患濫用台灣健保」之謠言，聲稱「全球愛滋病患感謝台灣健保、愛滋病治療將會弄垮臺灣的健保」。\n經查：\n1. 2017年度使用台灣健保之外...',
          title:
            '【錯誤】網傳「全球愛滋病患感謝台灣健保、外國愛滋病患者濫用台灣健保…愛滋病治療將會弄垮臺灣的健保」？',
          url: 'https://tfc-taiwan.org.tw/articles/260',
        },
      ],
    },
    '/replies/doc/CBUT_reply_17': {
      userId: 'CBUT_user_47',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '有關外界網路平台及line群組流傳出現：「現在的國小三年級教保險套、國小四年級教自慰、國小五年級教如何許多人一起性行為…」等類似言論，經教育部及國教院查證國小教...',
      reference:
        '教育部澄清網路群組流傳教科書不實性別平等教育內容之回應\nhttps://www.edu.tw/News_Content.aspx?n=FD56C961F1677...',
      createdAt: '2018-11-07T06:17:00.435Z',
      hyperlinks: [
        {
          summary: '',
          title: '教育部澄清網路群組流傳教科書不實性別平等教育內容之回應',
          url:
            'https://www.edu.tw/News_Content.aspx?n=FD56C961F1677400&sms=E6059C30DDBD5135&s=FC73C38D12A5F849',
        },
      ],
    },
    '/replies/doc/CBUT_reply_36': {
      userId: 'CBUT_user_48',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '有關外界網路平台及line群組流傳出現：「現在的國小三年級教保險套、國小四年級教自慰、國小五年級教如何許多人一起性行為…」等類似言論，經教育部及國教院查證國小教...',
      reference:
        '教育部性別平等教育 資訊網\nhttps://www.gender.edu.tw/web/index.php/home/index',
      createdAt: '2018-11-15T13:54:13.229Z',
      hyperlinks: [
        {
          summary:
            '教育部期望透過性別平等教育相關的課程與教學機會，教導學生瞭解並尊重不同的性別特質、性別認同與性傾向，清楚地讓學生認知其個體差異並不會影響個人能力及與他人之互動關...',
          title: '教育部告訴您【性別平等教育教什麼】',
          url: 'https://youtu.be/fHSCYj96Fwc',
        },
        {
          summary:
            '國家教育研究院依據課程綱要審定教科書，學校使用期間各界對教科書提出意見或建議，國家教育研究院都會依循相關機制妥善處理，並回應外界意見，期待性別平等教育落實在學生學習。',
          title: '教育部告訴您【性別平等教育教科書疑義處理說明篇】',
          url: 'https://youtu.be/K0I7lfliwOc',
        },
        {
          summary: '',
          title:
            '【教育部告訴您性別平等教育學校教科書選用及課程如何把關說明篇】',
          url: 'https://youtu.be/gy6d_xkgge8',
        },
        {
          summary:
            ':::\n\n\n\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t網站地圖\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t  \n\t\t\t\t\t\t\t\t線上調查專區\n\t\t\t\t\t\t  \n   ...',
          title: '教育部性別平等全球資訊網',
          url: 'https://www.gender.edu.tw/web/index.php/home/index',
        },
      ],
    },
    '/replies/doc/CBUT_reply_61': {
      userId: 'CBUT_user_74',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '至於坊間流傳的撒麵粉、鹽巴、太白粉、沙拉油、豬油等偏方，不但效果差，而且到醫院醫師還要先花時間清潔才能評估傷口，反而延遲救治時間。\n陽光基金會倡導組專案經理莊麗...',
      reference: 'https://news.ltn.com.tw/news/life/paper/1101504',
      createdAt: '2019-07-23T08:06:47.678Z',
      hyperlinks: [
        {
          summary:
            '2017-05-11記者陳炳宏／台北報導一名4歲男童嬉戲時，不慎絆倒電熱水壺，造成大面積燙傷，阿公和阿嬤在慌亂中，急忙脫掉他的衣服，並撒麵粉，企圖「降溫」，沒想...',
          title:
            '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤 - 生活 - 自由時報電子報',
          url: 'https://news.ltn.com.tw/news/life/paper/1101504',
        },
      ],
    },
    '/replies/doc/CBUT_reply_57': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '台灣事實查核中心已經證實為【錯誤】訊息囉！\n一、食藥署、疾管署，還有泰國的食藥署早已證實此訊息為網路謠言誤。\n二、專家指出，愛滋病毒離開人體後，短時間內便會失去...',
      reference: 'https://tfc-taiwan.org.tw/articles/740',
      createdAt: '2019-08-14T05:27:33.030Z',
      hyperlinks: [
        {
          summary:
            '【錯誤】「泰國水果罐頭被愛滋病患者血液汙染」謠言再起 \n      \n         \n\n網路、社群媒體近期流傳「泰國水果罐頭遭愛滋病毒汙染」訊息，經查：\n\n...',
          title: '【錯誤】「泰國水果罐頭被愛滋病患者血液汙染」謠言再起',
          url: 'https://tfc-taiwan.org.tw/articles/740',
        },
      ],
    },
    '/replies/doc/CBUT_reply_24': {
      userId: 'CBUT_user_88',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '無論是電子產品或是網站平台，均在註冊會員或啟用裝置時，會有一份資料蒐集同意書，其中會說明資料蒐集範圍，授權公司，去識別化方式等\n因此「手機內所有資料都會進入他們...',
      reference: ' ',
      createdAt: '2019-08-21T02:10:43.987Z',
    },
    '/replies/doc/CBUT_reply_47': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '台灣事實查核中心已經證實為【錯誤】訊息！\n一、馬偕醫院2017年已於官網澄清，本訊息為網路謠言。\n二、專家指出，海獸胃線蟲寄生在海洋生物體內，人類並非宿主，目前...',
      reference: 'https://tfc-taiwan.org.tw/articles/772',
      createdAt: '2019-08-28T02:48:00.667Z',
      hyperlinks: [
        {
          summary:
            '近來網傳「台北馬偕醫院傳來訊息，指出好多病人因吃生魚片，胃壁附著『海獸胃腺蟲』，甚至胃壁上滿滿都是無法夾出來，驅蟲藥也很難根治」，並且搭配病患眼中夾出寄生蟲的影...',
          title:
            '【錯誤】網傳「台北馬偕醫院傳來訊息，好多人因為吃了生魚片，胃壁附著『海獸胃腺蟲』」？',
          url: 'https://tfc-taiwan.org.tw/articles/772',
        },
      ],
    },
    '/replies/doc/CBUT_reply_19': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        'FAKE.\nThe advice it provides runs contrary to all accepted knowledge about how t...',
      reference: 'https://www.snopes.com/fact-check/flour-power/',
      createdAt: '2019-11-13T16:18:32.723Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://www.snopes.com/fact-check/flour-power/',
          summary:
            'Claim:   The application of flour is an effective and recommended treatment for ...',
          title: 'Should You Treat Burns with Flour?',
          url: 'https://www.snopes.com/fact-check/flour-power/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_65': {
      userId: 'CBUT_user_79',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text: '文字長度太短，疑似為使用者手動輸入之查詢語句，不像轉傳文章。',
      reference: '',
      createdAt: '2019-11-18T07:31:32.985Z',
    },
    '/replies/doc/CBUT_reply_14': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '假的。手機導致的疾病包含器官老化為無稽之談，目前手機釋放的射頻輻射（ RF ）被國際癌症研究機構（ International Agency for Resea...',
      reference:
        'https://www.thenewslens.com/article/91387\nhttps://www.medpartner.club/electromag...',
      createdAt: '2019-11-18T13:31:29.656Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://www.thenewslens.com/article/91387',
          summary:
            '人活著就是要吃東西！對於繁忙的現代人而言，無論是從便利商店購買香噴噴的微波食品，或者是加熱昨晚沒吃完的剩菜剩飯，微波爐似乎是生活當中必不可少的家電之一。    ...',
          title:
            '【圖輯】會致癌？有輻射？各種微波爐謠言一次破解 - The News Lens 關鍵評論網',
          url: 'https://www.thenewslens.com/article/91387',
        },
        {
          normalizedUrl:
            'https://www.medpartner.club/electromagnetic-wave-cancer-myth/',
          summary:
            '手機電磁波真的會致癌嗎？相關的網路傳言一大堆，到底應該相信哪一個？手機真的對我們身體有這麼大的危害嗎？我平常工作就是需要常常使用手機與客戶聯繫，要怎麼辦？其實這...',
          title:
            '手機電磁波致癌！？專家教你如何降低3C危害保護家人健康！ | 美的好朋友',
          url: 'https://www.medpartner.club/electromagnetic-wave-cancer-myth/',
        },
        {
          normalizedUrl:
            'https://cn.nytimes.com/health/20180205/cell-phones-cancer/zh-hant/',
          summary:
            '手機致癌嗎？儘管已研究多年，這個問題依然沒有明確的答案。但是衛生官員稱，週五公布的兩項分別在大鼠和小鼠身上進行的相關政府研究表明，即便有風險，也是比較小的。多年...',
          title: '手機輻射致癌嗎？科學家研究十年後有了新發現',
          url:
            'https://cn.nytimes.com/health/20180205/cell-phones-cancer/zh-hant/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_4': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '地瓜葉和牛奶的組合含有豐富的營養成分，可以幫助預防骨質疏鬆、延緩血糖上升，但對已罹患的疾病沒有治療效果。傳言所稱「可以治療三高、痛風和癌症」，並不正確。',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/1481',
      createdAt: '2019-12-06T16:05:05.572Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/1481',
          summary:
            '網傳「只要牛奶加上它（地瓜葉），三高、痛風、癌症都不見了」，經查：\n\n一、地瓜葉和牛奶的組合含有豐富的營養成分，可以幫助預防骨質疏鬆、延緩血糖上升，但對已罹患的...',
          title:
            '【錯誤】網傳「只要牛奶加上它（地瓜葉），三高、痛風、癌症都不見了」？',
          url: 'https://tfc-taiwan.org.tw/articles/1481',
        },
      ],
    },
    '/replies/doc/CBUT_reply_5': {
      userId: 'CBUT_user_68',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '1. 本文出自商周雜誌〈冰封中國關鍵報告〉。\n2. 中國有「世界最大勞動力人口國」的優勢，爆發大型流行病情，固然會影響各產業及國際貿易，若疫情發生在任何一個大型...',
      reference:
        '【商周】冰封中國關鍵報告\nhttps://www.businessweekly.com.tw/magazine/article_page.aspx?id=600...',
      createdAt: '2020-02-27T09:11:33.760Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.businessweekly.com.tw/magazine/article_page.aspx?id=6001194',
          summary:
            '一個疑似從武漢海鮮市場開始的病毒，現在，造成中國經濟停擺，全世界都地震！當貨出不去，也進不來，中國的冰封效應，正如何在台灣快速擴散？商周兵分5路，從一個乳液的瓶...',
          title: '冰封中國關鍵報告  - 封面摘要 - 商業周刊',
          url:
            'https://www.businessweekly.com.tw/magazine/article_page.aspx?id=6001194',
        },
        {
          normalizedUrl:
            'https://www.theverge.com/2020/2/12/21127754/mwc-2020-canceled-coronavirus-trade-...',
          summary:
            'The world’s biggest phone show, Mobile World Congress, is no longer taking place...',
          title:
            'The world’s biggest phone show has been canceled due to coronavirus concerns',
          url:
            'https://www.theverge.com/2020/2/12/21127754/mwc-2020-canceled-coronavirus-trade-...',
        },
        {
          normalizedUrl:
            'https://www.reuters.com/article/us-china-health-fda/fda-identified-20-drugs-with...',
          summary:
            'The ultrastructural morphology exhibited by the 2019 Novel Coronavirus (2019-nCo...',
          title:
            'FDA identified 20 drugs with shortage risks due to coronavirus outbreak',
          url:
            'https://www.reuters.com/article/us-china-health-fda/fda-identified-20-drugs-with...',
        },
        {
          normalizedUrl:
            'https://www.spglobal.com/platts/en/market-insights/latest-news/oil/022620-oil-fu...',
          summary:
            'HighlightsCrude lowest since January 2019\n  More countries report COVID-19 cases...',
          title:
            'Oil extends losses as coronavirus outbreak accelerates outside China',
          url:
            'https://www.spglobal.com/platts/en/market-insights/latest-news/oil/022620-oil-fu...',
        },
      ],
    },
    '/replies/doc/CBUT_reply_11': {
      userId: 'CBUT_user_15',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '美國闢謠網站 Snopes 向 James Robb 博士本人查證，博士表示這是他寫的信，但那是給家人與親朋好友的信，並沒有預期到會被公開。\n\n關於其中「鋅錠劑...',
      reference:
        '2020/3/2 Did a Noted Pathologist Write This Viral Coronavirus Advice Letter?\nhtt...',
      createdAt: '2020-03-03T05:10:50.254Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.snopes.com/fact-check/zinc-lozenges-coronavirus/',
          summary:
            'In late February 2020, as a new coronavirus spread worldwide, a letter providing...',
          title:
            'Did a Noted Pathologist Write This Viral Coronavirus Advice Letter?',
          url: 'https://www.snopes.com/fact-check/zinc-lozenges-coronavirus/',
        },
      ],
    },
    '/replies/doc/CBUT_reply_53': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 12月17日立法院三讀通過《毒品危害防制條例》部分條文修正草案，是把處罰標準加重，而非放寬。\n2. 根據毒品危害防制條例規定，有「販賣」毒品意圖就處以刑罰...',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/1670',
      createdAt: '2019-12-25T16:46:47.879Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/1670',
          summary: '',
          title:
            '【錯誤】網傳「民進黨已經通過了毒品條例了，校園內可以擁有5克的毒品，才被視同販毒。台灣除了是(同性戀島)(愛滋病島)現在又增加了一個(毒品島)了」？ | 台灣事實查核中心',
          url: 'https://tfc-taiwan.org.tw/articles/1670',
        },
      ],
    },
    '/replies/doc/CBUT_reply_44': {
      userId: 'CBUT_user_57',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '下附影片為羅馬尼亞於2019/5/28拍攝的，可在Youtube上找到，並非義大利\n再根據另一個網頁查證，當日為羅馬尼亞的冰雹',
      reference:
        '羅馬尼亞冰雹查證資料出處：https://www.mygopen.com/2019/07/Romania-hail.html\n羅馬尼亞冰雹影片 https://...',
      createdAt: '2020-02-21T02:49:47.068Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://www.mygopen.com/2019/07/Romania-hail.html',
          summary: '',
          title: '',
          url: 'https://www.mygopen.com/2019/07/Romania-hail.html',
        },
        {
          normalizedUrl: 'https://www.youtube.com/watch?v=IDogwFIaE58',
          summary: 'Stone rain\nSubscribe,Like and share.\nFor more videos',
          title: 'Stone rain in Romania',
          url: 'https://www.youtube.com/watch?v=IDogwFIaE58',
        },
      ],
    },
    '/replies/doc/CBUT_reply_51': {
      userId: 'CBUT_user_65',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '是修法更嚴執行：法務部在毒品危害防制條例三讀修正通過的當天也發布新聞稿，說明這次的修法重點，包括重懲毒販，提高製造、販賣、運輸毒品的刑度及罰金；將持有第3級、第...',
      reference: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx\n',
      createdAt: '2019-12-24T02:41:34.864Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
          summary:
            '立法院會17日三讀修正通過毒品危害防制條例部分條文，持有第三級、第四級毒品純質淨重5公克以上者可處有期徒刑。圖為第四級毒品5-MeO-DIPT，俗名火狐狸。（示...',
          title:
            '網傳校園攜毒免罰 綠委：修法更嚴勿混淆視聽 | 社會 | 重點新聞 | 中央社 CNA',
          url: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
        },
      ],
    },
    '/replies/doc/CBUT_reply_52': {
      userId: 'CBUT_user_65',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '而有臉書粉絲專頁po文指出，國民黨團當時提出修正動議，持有第3級、第4級毒品的純質淨重，1公克以上就要課予刑責，但被民進黨表決封殺。此外，專頁上還附上一張圖卡，...',
      reference: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
      createdAt: '2019-12-24T02:55:42.325Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
          summary:
            '立法院會17日三讀修正通過毒品危害防制條例部分條文，持有第三級、第四級毒品純質淨重5公克以上者可處有期徒刑。圖為第四級毒品5-MeO-DIPT，俗名火狐狸。（示...',
          title:
            '網傳校園攜毒免罰 綠委：修法更嚴勿混淆視聽 | 社會 | 重點新聞 | 中央社 CNA',
          url: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
        },
      ],
    },
    '/replies/doc/CBUT_reply_12': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '影片中男主角實為古巴裔美國演員威廉·列維（William Levy）。其中一幕來自墨西哥電視劇（Triunfo del amor ）。\n',
      reference:
        'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml\n義大利女友感染新冠肺炎，男友脫掉防護服見最後一面後雙雙殉情？謠言！\n',
      createdAt: '2020-04-03T04:24:14.807Z',
      hyperlinks: [
        {
          normalizedUrl: 'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml',
          summary:
            '北京市科协主办\n\t\t\t\t\t\t\n\t\t\t\n\t\t\t\n\n\t\t\n\n\n\n\n\n\t\n\n\t\t\n\n\t\t\t\n\n\t\t\t\n\n\t\t\t\n\t\t\t\t\n\t\t\t\t\t首页HOME\n\t\t\t\t\t科技热闻N...',
          title:
            '意大利女友感染新冠肺炎，男友脱掉防护服见最后一面后双双殉情？谣言！',
          url: 'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml',
        },
      ],
    },
    '/replies/doc/CBUT_reply_50': {
      userId: 'CBUT_user_63',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '一個場合當中誰做了什麼、意味什麼，這都是沒有直接證據的推論。此訊息當中可以留意的是“無症狀卻可能有傳染力的隱性感染者”造成的疫情傳染風險。由於本次新型冠狀肺炎疫...',
      reference:
        '口罩用途可以參考文章\nhttp://www-o.ntust.edu.tw/h1n1/page16.htm',
      createdAt: '2020-04-06T07:42:40.309Z',
      hyperlinks: [
        {
          normalizedUrl: 'http://www-o.ntust.edu.tw/h1n1/page16.htm',
          summary:
            '一、為什麼戴口罩可以預防傳染病? \n\t\t\n\t\t          \n\t\t1.避免把病毒或細菌傳染給別人。\n\t\t\n\t\t            \n\t\t因：咳嗽及打噴...',
          title: '口罩知多少',
          url: 'http://www-o.ntust.edu.tw/h1n1/page16.htm',
        },
      ],
    },
    '/replies/doc/CBUT_reply_67': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 年金改革委員會第14次會議早在2016年9月29日就已舉行，會議為蒐集各方意見，並未做成任何決議。\n2. 軍公教人員退休年齡規定，依年資各有不同，並非一律...',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3573',
      createdAt: '2020-04-08T13:45:36.390Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3573',
          summary:
            '網傳「昨天年金改革委員會第14次會議...各項年金請領年齡皆為65歲...軍人終身俸不再發放...80歲以後到死1塊錢不給...」，經查：\n\n一、年金改革委員會...',
          title:
            '【錯誤】網傳「昨天年金改革委員會第14次會議...各項年金請領年齡皆為65歲...軍人終身俸不再發放...80歲以後到死1塊錢不給...」？',
          url: 'https://tfc-taiwan.org.tw/articles/3573',
        },
      ],
    },
    '/replies/doc/CBUT_reply_62': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text: '網傳影片為2010年墨西哥電視劇《愛的勝利》片段。',
      reference: 'https://tfc-taiwan.org.tw/articles/3542',
      createdAt: '2020-04-08T14:05:42.091Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3542',
          summary:
            '【錯誤】網傳影片「一對意大利情侣，本来約定今年完婚，女友感染新冠肺炎在搶救，男友見最后一面，果断脱掉防護服並親吻女友」？\n      \n         \n\n網...',
          title:
            '【錯誤】網傳影片「一對意大利情侣，本来約定今年完婚，女友感染新冠肺炎在搶救，男友見最后一面，果断脱掉防護服並親吻女友」？',
          url: 'https://tfc-taiwan.org.tw/articles/3542',
        },
      ],
    },
    '/replies/doc/CBUT_reply_45': {
      userId: 'CBUT_user_54',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '1. 網傳影片出處是2019年5月28日羅馬尼亞北部札勒烏（Zalau）市下冰雹的景象，不是義大利羅馬下石頭雨。\n2. 台灣事實查核中心檢索義大利媒體及國際通訊社，並無義大利羅馬下石頭雨的事件。',
      reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3477',
      createdAt: '2020-04-08T14:19:31.619Z',
      hyperlinks: [
        {
          normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3477',
          summary:
            '網傳照片文字宣稱「意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨...」，經查：\n\n一、網傳影片出處是2019年5月28日羅馬尼亞北部札勒烏（Zalau）...',
          title:
            '【錯誤】網傳影片搭配文字宣稱「意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨......」？',
          url: 'https://tfc-taiwan.org.tw/articles/3477',
        },
      ],
    },
    '/replies/doc/CBUT_reply_56': {
      userId: 'CBUT_user_69',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '此為改編自金正日2011年的訃聞，只有名字和時間有改動，其餘皆相同。\n"朝鲜劳动党中央委员会和朝鲜劳动党中央军事委员会、朝鲜民主主义人民共和国国防委员会和最高人...',
      reference: 'http://world.people.com.cn/GB/16651924.html',
      createdAt: '2020-04-27T09:34:49.512Z',
      hyperlinks: [
        {
          normalizedUrl: 'http://world.people.com.cn/GB/16651924.html',
          summary:
            '12月19日，北京，朝鲜大使馆降半旗悼念金正日。 　　据中央电视台报道，朝鲜中央电视台19日播出该国最高领导人金正日逝世讣告。　　讣告如下：　　朝鲜全体党员、人...',
          title: '朝鲜官方媒体播出金正日讣告--国际--人民网',
          url: 'http://world.people.com.cn/GB/16651924.html',
        },
      ],
    },
    '/replies/doc/CBUT_reply_55': {
      userId: 'CBUT_user_68',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '此為假冒北韓官方的訊息。北韓官媒《朝鮮中央通訊社》4月23日仍有報導金正恩回電感謝敘利亞阿薩德的新聞，南韓青瓦台4月23日也表示「沒有具體證據顯示金正恩的健康狀...',
      reference:
        '命危消息傳出後 北韓官媒曝金正恩最新動態\nhttps://news.ltn.com.tw/news/world/breakingnews/3142735\n\nTh...',
      createdAt: '2020-04-26T15:36:41.541Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://news.ltn.com.tw/news/world/breakingnews/3142735',
          summary:
            '北韓領導人金正恩身體狀況引發外界關注。（美聯社）\n    \n\n\n    2020-04-23 07:36:23〔即時新聞／綜合報導〕北韓領導人金正恩缺席祖父金日...',
          title:
            '命危消息傳出後 北韓官媒曝金正恩最新動態 - 國際 - 自由時報電子報',
          url: 'https://news.ltn.com.tw/news/world/breakingnews/3142735',
        },
        {
          normalizedUrl:
            'https://www.foxnews.com/world/kim-jong-un-mystery-north-korea-dictator-health',
          summary:
            'What’s the story with Kim Jong Un?The North Korean leader missed a high-profile ...',
          title:
            "The mystery surrounding Kim Jong Un: What's going on with the North Korean dictator's health?",
          url:
            'https://www.foxnews.com/world/kim-jong-un-mystery-north-korea-dictator-health',
        },
        {
          normalizedUrl:
            'http://world.kbs.co.kr/service/news_view.htm?lang=k&Seq_Code=353396',
          summary:
            'Photo : YONHAP News\n                \n\n                \n                         ...',
          title: '靑 NSC',
          url:
            'http://world.kbs.co.kr/service/news_view.htm?lang=k&Seq_Code=353396',
        },
        {
          normalizedUrl:
            'http://www.uriminzokkiri.com/index.php?ptype=cgisas&mtype=view&no=1190981',
          summary: '',
          title: '',
          url:
            'http://www.uriminzokkiri.com/index.php?ptype=cgisas&mtype=view&no=1190981',
        },
      ],
    },
    '/replies/doc/CBUT_reply_48': {
      userId: 'CBUT_user_60',
      appId: 'WEBSITE',
      type: 'OPINIONATED',
      text:
        '■對不同城市及其官員對疫情處置的評價，屬個人評論；亦有評論從貧困人口、企業靈活度等不同角度分析舊金山與紐約的疫情處置差異。\n\n■文中提及人物資訊、事件時序大抵正...',
      reference:
        '【疫情處置評論】\n\n布魯金斯學會-Coronavirus lessons from New York and San Francisco\nhttps://www...',
      createdAt: '2020-04-28T13:51:19.413Z',
    },
    '/replies/doc/CBUT_reply_49': {
      userId: 'CBUT_user_61',
      appId: 'WEBSITE',
      type: 'NOT_ARTICLE',
      text:
        '此訊息需要更多數據，不過難以統計，其他關於政策的訂定時間與描述很清楚。',
      reference: '',
      createdAt: '2020-05-05T09:41:19.615Z',
      hyperlinks: [],
    },
    '/replies/doc/CBUT_reply_59': {
      userId: 'CBUT_user_71',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '影片含有許多不實訊息，例如 2:09 聲稱蜂蜜是二級燙傷首選藥物，有嚴重誤導人之嫌，已有其他急診醫師說醫療級才可，正確做法還是立刻沖水降溫，嚴重時趕快送醫。',
      reference: 'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
      createdAt: '2020-07-22T03:51:29.624Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
          summary:
            '【本報台北訊】居家治療燒燙傷偏方不少，牙膏、蛋清等五花八門，多數遭專家否定，不止沒療效還恐增加感染風險，實證研究顯示，「蜂蜜」具消炎殺菌功效，有助照護燒燙傷口。...',
          title: '燙傷可抹蜂蜜？醫師：醫療級才可 | 醫藥 | 人間福報',
          url: 'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
        },
      ],
    },
    '/replies/doc/CBUT_reply_74': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '有關報載世紀鋼廠房建置時程落後之情形，世紀鋼公開表示，內容未經查核，顯與事實有違，該公司為國內率先響應離岸風電國產化的本土公司，除了觀音廠廠房擴建產能，投入生產...',
      reference:
        'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
      createdAt: '2020-09-26T07:30:21.054Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
          summary:
            '有關媒體報載「百億離岸風電國產大餅訂單爆肥水外流」一文，指總統蔡英文去(108)年在台北港水下基礎基樁對接儀式中，為世紀集團對水下基礎製造技術、廠房等投資背書，...',
          title: '逐步推動離岸風電產業政策 開發商與在地產業攜手共創雙贏',
          url:
            'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
        },
      ],
    },
    '/replies/doc/CBUT_reply_75': {
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      type: 'RUMOR',
      text:
        '據外交部瞭解，拜會美國行政部門的相關行程，均由臺北市政府自行洽請美國在台協會（AIT)協助安排，並非由外交部或駐處洽排。相關安排情形外交部均不便評論，宜由臺北市政府自行説明。',
      reference:
        'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
      createdAt: '2020-09-26T09:40:02.610Z',
      hyperlinks: [
        {
          normalizedUrl:
            'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
          summary: '',
          title: '',
          url:
            'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
        },
      ],
    },
    '/replyrequests/doc/CBUT_article_40__CBUT_user_17__DEVELOPMENT_BACKEND': {
      createdAt: '2017-05-30T15:36:48.738Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_17',
      updatedAt: '2017-05-30T15:36:48.738Z',
      articleId: 'CBUT_article_40',
    },
    '/replyrequests/doc/CBUT_article_36__CBUT_user_17__DEVELOPMENT_BACKEND': {
      createdAt: '2017-05-10T13:36:53.795Z',
      appId: 'DEVELOPMENT_BACKEND',
      userId: 'CBUT_user_17',
      updatedAt: '2017-05-10T13:36:53.795Z',
      articleId: 'CBUT_article_36',
    },
    '/replyrequests/doc/CBUT_article_44__CBUT_user_35__WEBSITE': {
      articleId: 'CBUT_article_44',
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      reason: '',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2020-09-26T09:39:48.829Z',
      updatedAt: '2020-09-26T09:39:48.829Z',
    },
    '/replyrequests/doc/CBUT_article_38__CBUT_user_56__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_38',
      userId: 'CBUT_user_56',
      appId: 'RUMORS_LINE_BOT',
      reason: '找不到相關訊息  繁簡夾雜',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2020-02-21T02:02:55.271Z',
      updatedAt: '2020-02-21T02:02:55.271Z',
    },
    '/replyrequests/doc/CBUT_article_42__CBUT_user_35__WEBSITE': {
      articleId: 'CBUT_article_42',
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      reason: '',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2019-04-15T16:38:11.473Z',
      updatedAt: '2019-04-15T16:38:11.473Z',
    },
    '/replyrequests/doc/CBUT_article_43__CBUT_user_35__WEBSITE': {
      articleId: 'CBUT_article_43',
      userId: 'CBUT_user_35',
      appId: 'WEBSITE',
      reason: '',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2020-09-26T07:29:51.708Z',
      updatedAt: '2020-09-26T07:29:51.708Z',
    },
    '/replyrequests/doc/CBUT_article_39__CBUT_user_69__WEBSITE': {
      articleId: 'CBUT_article_39',
      userId: 'CBUT_user_69',
      appId: 'WEBSITE',
      reason:
        '此為改編自金正日2011年的訃聞，只有名字和時間有改動，其餘皆相同。\n\n"　　朝鲜劳动党中央委员会和朝鲜劳动党中央军事委员会、朝鲜民主主义人民共和国国防委员会和...',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2020-04-27T09:34:01.196Z',
      updatedAt: '2020-04-27T09:34:01.196Z',
    },
    '/replyrequests/doc/CBUT_article_35__CBUT_user_42__RUMORS_LINE_BOT': {
      createdAt: '2017-12-24T23:48:52.805Z',
      appId: 'RUMORS_LINE_BOT',
      userId: 'CBUT_user_42',
      updatedAt: '2017-12-24T23:48:52.805Z',
      articleId: 'CBUT_article_35',
    },
    '/replyrequests/doc/CBUT_article_37__CBUT_user_37__BOT_LEGACY': {
      createdAt: '2017-01-11T08:52:00.000Z',
      appId: 'BOT_LEGACY',
      userId: 'CBUT_user_37',
      articleId: 'CBUT_article_37',
    },
    '/replyrequests/doc/CBUT_article_41__CBUT_user_77__RUMORS_LINE_BOT': {
      articleId: 'CBUT_article_41',
      userId: 'CBUT_user_77',
      appId: 'RUMORS_LINE_BOT',
      reason:
        '我用關鍵字找，發現有不一樣的說法 https://m.douban.com/group/topic/60942708/',
      feedbacks: [],
      positiveFeedbackCount: 0,
      negativeFeedbackCount: 0,
      createdAt: '2019-04-17T06:56:36.153Z',
      updatedAt: '2019-04-17T06:56:36.153Z',
    },
    '/analytics/doc/article_CBUT_article_30_2019-03-06': {
      date: '2019-03-06T00:00:00.000+08:00',
      docId: 'CBUT_article_30',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 2,
        lineUser: 2,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_34'
    },
    '/analytics/doc/article_CBUT_article_30_2019-03-12': {
      date: '2019-03-12T00:00:00.000+08:00',
      docId: 'CBUT_article_30',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_34'
    },
    '/analytics/doc/article_CBUT_article_30_2019-04-21': {
      date: '2019-04-21T00:00:00.000+08:00',
      docId: 'CBUT_article_30',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 9,
        lineUser: 5,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_34'
    },
    '/analytics/doc/article_CBUT_article_30_2019-08-05': {
      date: '2019-08-05T00:00:00.000+08:00',
      docId: 'CBUT_article_30',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 1,
        lineUser: 5,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_34'
    },
    '/analytics/doc/article_CBUT_article_31_2019-05-12': {
      date: '2019-05-12T00:00:00.000+08:00',
      docId: 'CBUT_article_31',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 8,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_31_2019-07-13': {
      date: '2019-07-13T00:00:00.000+08:00',
      docId: 'CBUT_article_31',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 8,
        lineUser: 5,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_31_2019-08-19': {
      date: '2019-08-19T00:00:00.000+08:00',
      docId: 'CBUT_article_31',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 3,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_31_2019-08-28': {
      date: '2019-08-28T00:00:00.000+08:00',
      docId: 'CBUT_article_31',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 5,
        lineUser: 6,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_31_2019-12-24': {
      date: '2019-12-24T00:00:00.000+08:00',
      docId: 'CBUT_article_31',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 4
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_32_2019-04-05': {
      date: '2019-04-05T00:00:00.000+08:00',
      docId: 'CBUT_article_32',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_39'
    },
    '/analytics/doc/article_CBUT_article_32_2019-04-29': {
      date: '2019-04-29T00:00:00.000+08:00',
      docId: 'CBUT_article_32',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 6,
        lineUser: 7,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_39'
    },
    '/analytics/doc/article_CBUT_article_32_2019-11-30': {
      date: '2019-11-30T00:00:00.000+08:00',
      docId: 'CBUT_article_32',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 3
      },
      docUserId: 'CBUT_user_39'
    },
    '/analytics/doc/article_CBUT_article_33_2019-03-21': {
      date: '2019-03-21T00:00:00.000+08:00',
      docId: 'CBUT_article_33',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 5,
        lineUser: 9,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/article_CBUT_article_33_2019-08-28': {
      date: '2019-08-28T00:00:00.000+08:00',
      docId: 'CBUT_article_33',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 8,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/article_CBUT_article_33_2019-09-23': {
      date: '2019-09-23T00:00:00.000+08:00',
      docId: 'CBUT_article_33',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/article_CBUT_article_33_2019-12-28': {
      date: '2019-12-28T00:00:00.000+08:00',
      docId: 'CBUT_article_33',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 2,
        lineUser: 1,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/article_CBUT_article_34_2019-01-25': {
      date: '2019-01-25T00:00:00.000+08:00',
      docId: 'CBUT_article_34',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 7,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_34_2019-03-07': {
      date: '2019-03-07T00:00:00.000+08:00',
      docId: 'CBUT_article_34',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 10
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_15_2019-05-07': {
      date: '2019-05-07T00:00:00.000+08:00',
      docId: 'CBUT_article_15',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 4,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_41'
    },
    '/analytics/doc/article_CBUT_article_15_2019-07-17': {
      date: '2019-07-17T00:00:00.000+08:00',
      docId: 'CBUT_article_15',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_41'
    },
    '/analytics/doc/article_CBUT_article_15_2019-08-04': {
      date: '2019-08-04T00:00:00.000+08:00',
      docId: 'CBUT_article_15',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 8
      },
      docUserId: 'CBUT_user_41'
    },
    '/analytics/doc/article_CBUT_article_15_2019-08-10': {
      date: '2019-08-10T00:00:00.000+08:00',
      docId: 'CBUT_article_15',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 10,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_41'
    },
    '/analytics/doc/article_CBUT_article_15_2019-09-09': {
      date: '2019-09-09T00:00:00.000+08:00',
      docId: 'CBUT_article_15',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 6
      },
      docUserId: 'CBUT_user_41'
    },
    '/analytics/doc/article_CBUT_article_35_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_article_35',
      type: 'article',
      stats: {
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_42'
    },
    '/analytics/doc/article_CBUT_article_35_2019-06-09': {
      date: '2019-06-09T00:00:00.000+08:00',
      docId: 'CBUT_article_35',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 1,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_42'
    },
    '/analytics/doc/article_CBUT_article_35_2019-06-19': {
      date: '2019-06-19T00:00:00.000+08:00',
      docId: 'CBUT_article_35',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 5,
        lineUser: 6,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_42'
    },
    '/analytics/doc/article_CBUT_article_35_2019-07-14': {
      date: '2019-07-14T00:00:00.000+08:00',
      docId: 'CBUT_article_35',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 3,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_42'
    },
    '/analytics/doc/article_CBUT_article_26_2019-01-08': {
      date: '2019-01-08T00:00:00.000+08:00',
      docId: 'CBUT_article_26',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 5,
        lineUser: 1,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_43'
    },
    '/analytics/doc/article_CBUT_article_26_2019-05-09': {
      date: '2019-05-09T00:00:00.000+08:00',
      docId: 'CBUT_article_26',
      type: 'article',
      stats: {
        lineUser: 1,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_43'
    },
    '/analytics/doc/article_CBUT_article_26_2019-06-05': {
      date: '2019-06-05T00:00:00.000+08:00',
      docId: 'CBUT_article_26',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 8,
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_43'
    },
    '/analytics/doc/article_CBUT_article_26_2019-10-19': {
      date: '2019-10-19T00:00:00.000+08:00',
      docId: 'CBUT_article_26',
      type: 'article',
      stats: {
        lineUser: 4,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_43'
    },
    '/analytics/doc/article_CBUT_article_26_2019-11-02': {
      date: '2019-11-02T00:00:00.000+08:00',
      docId: 'CBUT_article_26',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 4,
        lineUser: 9,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_43'
    },
    '/analytics/doc/article_CBUT_article_36_2019-01-20': {
      date: '2019-01-20T00:00:00.000+08:00',
      docId: 'CBUT_article_36',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 10,
        lineUser: 6,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_44'
    },
    '/analytics/doc/article_CBUT_article_36_2019-02-02': {
      date: '2019-02-02T00:00:00.000+08:00',
      docId: 'CBUT_article_36',
      type: 'article',
      stats: {
        lineUser: 3,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_44'
    },
    '/analytics/doc/article_CBUT_article_36_2019-02-28': {
      date: '2019-02-28T00:00:00.000+08:00',
      docId: 'CBUT_article_36',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 5,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_44'
    },
    '/analytics/doc/article_CBUT_article_36_2019-06-20': {
      date: '2019-06-20T00:00:00.000+08:00',
      docId: 'CBUT_article_36',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 7,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_44'
    },
    '/analytics/doc/article_CBUT_article_8_2019-04-16': {
      date: '2019-04-16T00:00:00.000+08:00',
      docId: 'CBUT_article_8',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 5,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_46'
    },
    '/analytics/doc/article_CBUT_article_8_2019-04-27': {
      date: '2019-04-27T00:00:00.000+08:00',
      docId: 'CBUT_article_8',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 3,
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_46'
    },
    '/analytics/doc/article_CBUT_article_8_2019-05-16': {
      date: '2019-05-16T00:00:00.000+08:00',
      docId: 'CBUT_article_8',
      type: 'article',
      stats: {
        lineUser: 2,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_46'
    },
    '/analytics/doc/article_CBUT_article_8_2019-06-12': {
      date: '2019-06-12T00:00:00.000+08:00',
      docId: 'CBUT_article_8',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 9,
        lineUser: 10,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_46'
    },
    '/analytics/doc/article_CBUT_article_8_2019-11-14': {
      date: '2019-11-14T00:00:00.000+08:00',
      docId: 'CBUT_article_8',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 1,
        lineUser: 9,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_46'
    },
    '/analytics/doc/article_CBUT_article_22_2019-02-09': {
      date: '2019-02-09T00:00:00.000+08:00',
      docId: 'CBUT_article_22',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 10,
        lineUser: 6,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_22_2019-04-21': {
      date: '2019-04-21T00:00:00.000+08:00',
      docId: 'CBUT_article_22',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 7,
        lineUser: 9,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_25_2019-03-24': {
      date: '2019-03-24T00:00:00.000+08:00',
      docId: 'CBUT_article_25',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 10,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_25_2019-04-01': {
      date: '2019-04-01T00:00:00.000+08:00',
      docId: 'CBUT_article_25',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_25_2019-04-14': {
      date: '2019-04-14T00:00:00.000+08:00',
      docId: 'CBUT_article_25',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_25_2019-06-14': {
      date: '2019-06-14T00:00:00.000+08:00',
      docId: 'CBUT_article_25',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_25_2019-07-09': {
      date: '2019-07-09T00:00:00.000+08:00',
      docId: 'CBUT_article_25',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_5_2019-05-25': {
      date: '2019-05-25T00:00:00.000+08:00',
      docId: 'CBUT_article_5',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 3,
        lineUser: 10,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_50'
    },
    '/analytics/doc/article_CBUT_article_5_2019-08-15': {
      date: '2019-08-15T00:00:00.000+08:00',
      docId: 'CBUT_article_5',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_50'
    },
    '/analytics/doc/article_CBUT_article_37_2019-01-10': {
      date: '2019-01-10T00:00:00.000+08:00',
      docId: 'CBUT_article_37',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_37_2019-03-31': {
      date: '2019-03-31T00:00:00.000+08:00',
      docId: 'CBUT_article_37',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 10,
        lineUser: 6,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_37_2019-06-18': {
      date: '2019-06-18T00:00:00.000+08:00',
      docId: 'CBUT_article_37',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 4,
        lineUser: 5,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_37_2019-10-22': {
      date: '2019-10-22T00:00:00.000+08:00',
      docId: 'CBUT_article_37',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 2,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_37_2019-12-02': {
      date: '2019-12-02T00:00:00.000+08:00',
      docId: 'CBUT_article_37',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 10,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_7_2019-01-13': {
      date: '2019-01-13T00:00:00.000+08:00',
      docId: 'CBUT_article_7',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 2,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_51'
    },
    '/analytics/doc/article_CBUT_article_7_2019-03-23': {
      date: '2019-03-23T00:00:00.000+08:00',
      docId: 'CBUT_article_7',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 4,
        lineUser: 2,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_51'
    },
    '/analytics/doc/article_CBUT_article_7_2019-09-28': {
      date: '2019-09-28T00:00:00.000+08:00',
      docId: 'CBUT_article_7',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_51'
    },
    '/analytics/doc/article_CBUT_article_7_2019-10-17': {
      date: '2019-10-17T00:00:00.000+08:00',
      docId: 'CBUT_article_7',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 2,
        lineUser: 10,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_51'
    },
    '/analytics/doc/article_CBUT_article_7_2019-11-14': {
      date: '2019-11-14T00:00:00.000+08:00',
      docId: 'CBUT_article_7',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 2,
        lineUser: 2,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_51'
    },
    '/analytics/doc/article_CBUT_article_16_2019-07-22': {
      date: '2019-07-22T00:00:00.000+08:00',
      docId: 'CBUT_article_16',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 9,
        lineUser: 3,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_55'
    },
    '/analytics/doc/article_CBUT_article_16_2019-08-24': {
      date: '2019-08-24T00:00:00.000+08:00',
      docId: 'CBUT_article_16',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 4,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_55'
    },
    '/analytics/doc/article_CBUT_article_16_2019-09-08': {
      date: '2019-09-08T00:00:00.000+08:00',
      docId: 'CBUT_article_16',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 9
      },
      docUserId: 'CBUT_user_55'
    },
    '/analytics/doc/article_CBUT_article_38_2019-03-31': {
      date: '2019-03-31T00:00:00.000+08:00',
      docId: 'CBUT_article_38',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 10,
        lineUser: 10,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_56'
    },
    '/analytics/doc/article_CBUT_article_28_2019-03-08': {
      date: '2019-03-08T00:00:00.000+08:00',
      docId: 'CBUT_article_28',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 10,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_58'
    },
    '/analytics/doc/article_CBUT_article_2_2019-05-04': {
      date: '2019-05-04T00:00:00.000+08:00',
      docId: 'CBUT_article_2',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 8,
        lineUser: 9,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_59'
    },
    '/analytics/doc/article_CBUT_article_1_2019-02-08': {
      date: '2019-02-08T00:00:00.000+08:00',
      docId: 'CBUT_article_1',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_62'
    },
    '/analytics/doc/article_CBUT_article_1_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_article_1',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 8,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_62'
    },
    '/analytics/doc/article_CBUT_article_1_2019-04-09': {
      date: '2019-04-09T00:00:00.000+08:00',
      docId: 'CBUT_article_1',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 7
      },
      docUserId: 'CBUT_user_62'
    },
    '/analytics/doc/article_CBUT_article_3_2019-08-22': {
      date: '2019-08-22T00:00:00.000+08:00',
      docId: 'CBUT_article_3',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_64'
    },
    '/analytics/doc/article_CBUT_article_4_2019-05-16': {
      date: '2019-05-16T00:00:00.000+08:00',
      docId: 'CBUT_article_4',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 7,
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_66'
    },
    '/analytics/doc/article_CBUT_article_4_2019-06-24': {
      date: '2019-06-24T00:00:00.000+08:00',
      docId: 'CBUT_article_4',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_66'
    },
    '/analytics/doc/article_CBUT_article_4_2019-12-01': {
      date: '2019-12-01T00:00:00.000+08:00',
      docId: 'CBUT_article_4',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_66'
    },
    '/analytics/doc/article_CBUT_article_4_2019-12-14': {
      date: '2019-12-14T00:00:00.000+08:00',
      docId: 'CBUT_article_4',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 9,
        lineUser: 2,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_66'
    },
    '/analytics/doc/article_CBUT_article_39_2019-05-22': {
      date: '2019-05-22T00:00:00.000+08:00',
      docId: 'CBUT_article_39',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 10,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_67'
    },
    '/analytics/doc/article_CBUT_article_39_2019-07-18': {
      date: '2019-07-18T00:00:00.000+08:00',
      docId: 'CBUT_article_39',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 4,
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_67'
    },
    '/analytics/doc/article_CBUT_article_39_2019-09-15': {
      date: '2019-09-15T00:00:00.000+08:00',
      docId: 'CBUT_article_39',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 3,
        lineUser: 10,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_67'
    },
    '/analytics/doc/article_CBUT_article_21_2019-01-06': {
      date: '2019-01-06T00:00:00.000+08:00',
      docId: 'CBUT_article_21',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 1
      },
      docUserId: 'CBUT_user_70'
    },
    '/analytics/doc/article_CBUT_article_21_2019-01-17': {
      date: '2019-01-17T00:00:00.000+08:00',
      docId: 'CBUT_article_21',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 10,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_70'
    },
    '/analytics/doc/article_CBUT_article_21_2019-02-05': {
      date: '2019-02-05T00:00:00.000+08:00',
      docId: 'CBUT_article_21',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 6,
        lineUser: 8,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_70'
    },
    '/analytics/doc/article_CBUT_article_40_2019-04-09': {
      date: '2019-04-09T00:00:00.000+08:00',
      docId: 'CBUT_article_40',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 9,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_17'
    },
    '/analytics/doc/article_CBUT_article_23_2019-01-06': {
      date: '2019-01-06T00:00:00.000+08:00',
      docId: 'CBUT_article_23',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 7,
        lineUser: 3,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_72'
    },
    '/analytics/doc/article_CBUT_article_23_2019-02-26': {
      date: '2019-02-26T00:00:00.000+08:00',
      docId: 'CBUT_article_23',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 9,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_72'
    },
    '/analytics/doc/article_CBUT_article_23_2019-12-02': {
      date: '2019-12-02T00:00:00.000+08:00',
      docId: 'CBUT_article_23',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 4,
        lineUser: 2,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_72'
    },
    '/analytics/doc/article_CBUT_article_23_2019-12-08': {
      date: '2019-12-08T00:00:00.000+08:00',
      docId: 'CBUT_article_23',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_72'
    },
    '/analytics/doc/article_CBUT_article_17_2019-05-01': {
      date: '2019-05-01T00:00:00.000+08:00',
      docId: 'CBUT_article_17',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 4,
        lineUser: 9,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_75'
    },
    '/analytics/doc/article_CBUT_article_10_2019-10-04': {
      date: '2019-10-04T00:00:00.000+08:00',
      docId: 'CBUT_article_10',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 7,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_76'
    },
    '/analytics/doc/article_CBUT_article_10_2019-10-15': {
      date: '2019-10-15T00:00:00.000+08:00',
      docId: 'CBUT_article_10',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 6,
        lineUser: 7,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_76'
    },
    '/analytics/doc/article_CBUT_article_41_2019-04-11': {
      date: '2019-04-11T00:00:00.000+08:00',
      docId: 'CBUT_article_41',
      type: 'article',
      stats: {
        lineUser: 10,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_77'
    },
    '/analytics/doc/article_CBUT_article_41_2019-04-15': {
      date: '2019-04-15T00:00:00.000+08:00',
      docId: 'CBUT_article_41',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_77'
    },
    '/analytics/doc/article_CBUT_article_41_2019-08-18': {
      date: '2019-08-18T00:00:00.000+08:00',
      docId: 'CBUT_article_41',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_77'
    },
    '/analytics/doc/article_CBUT_article_41_2019-10-04': {
      date: '2019-10-04T00:00:00.000+08:00',
      docId: 'CBUT_article_41',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 9,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_77'
    },
    '/analytics/doc/article_CBUT_article_41_2019-12-01': {
      date: '2019-12-01T00:00:00.000+08:00',
      docId: 'CBUT_article_41',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 8,
        lineUser: 7,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_77'
    },
    '/analytics/doc/article_CBUT_article_42_2019-01-16': {
      date: '2019-01-16T00:00:00.000+08:00',
      docId: 'CBUT_article_42',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 4,
        lineUser: 9,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_42_2019-02-12': {
      date: '2019-02-12T00:00:00.000+08:00',
      docId: 'CBUT_article_42',
      type: 'article',
      stats: {
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_42_2019-10-19': {
      date: '2019-10-19T00:00:00.000+08:00',
      docId: 'CBUT_article_42',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 4,
        lineUser: 4,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_42_2019-11-30': {
      date: '2019-11-30T00:00:00.000+08:00',
      docId: 'CBUT_article_42',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 1,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_19_2019-02-26': {
      date: '2019-02-26T00:00:00.000+08:00',
      docId: 'CBUT_article_19',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 10,
        lineUser: 10,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_78'
    },
    '/analytics/doc/article_CBUT_article_19_2019-09-12': {
      date: '2019-09-12T00:00:00.000+08:00',
      docId: 'CBUT_article_19',
      type: 'article',
      stats: {
        lineUser: 1,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_78'
    },
    '/analytics/doc/article_CBUT_article_24_2019-01-03': {
      date: '2019-01-03T00:00:00.000+08:00',
      docId: 'CBUT_article_24',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 5,
        lineUser: 1,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_24_2019-03-18': {
      date: '2019-03-18T00:00:00.000+08:00',
      docId: 'CBUT_article_24',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_24_2019-06-06': {
      date: '2019-06-06T00:00:00.000+08:00',
      docId: 'CBUT_article_24',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 7,
        lineUser: 7,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_24_2019-06-26': {
      date: '2019-06-26T00:00:00.000+08:00',
      docId: 'CBUT_article_24',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 7,
        lineUser: 2,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_12_2019-01-21': {
      date: '2019-01-21T00:00:00.000+08:00',
      docId: 'CBUT_article_12',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_80'
    },
    '/analytics/doc/article_CBUT_article_12_2019-06-03': {
      date: '2019-06-03T00:00:00.000+08:00',
      docId: 'CBUT_article_12',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_80'
    },
    '/analytics/doc/article_CBUT_article_12_2019-11-02': {
      date: '2019-11-02T00:00:00.000+08:00',
      docId: 'CBUT_article_12',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 7,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_80'
    },
    '/analytics/doc/article_CBUT_article_6_2019-08-27': {
      date: '2019-08-27T00:00:00.000+08:00',
      docId: 'CBUT_article_6',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 2,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_27_2019-01-08': {
      date: '2019-01-08T00:00:00.000+08:00',
      docId: 'CBUT_article_27',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 6,
        lineUser: 10,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_81'
    },
    '/analytics/doc/article_CBUT_article_27_2019-08-08': {
      date: '2019-08-08T00:00:00.000+08:00',
      docId: 'CBUT_article_27',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 8,
        lineUser: 7,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_81'
    },
    '/analytics/doc/article_CBUT_article_27_2019-09-16': {
      date: '2019-09-16T00:00:00.000+08:00',
      docId: 'CBUT_article_27',
      type: 'article',
      stats: {
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_81'
    },
    '/analytics/doc/article_CBUT_article_27_2019-10-21': {
      date: '2019-10-21T00:00:00.000+08:00',
      docId: 'CBUT_article_27',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 2,
        lineUser: 7,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_81'
    },
    '/analytics/doc/article_CBUT_article_14_2019-01-30': {
      date: '2019-01-30T00:00:00.000+08:00',
      docId: 'CBUT_article_14',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 4,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_82'
    },
    '/analytics/doc/article_CBUT_article_14_2019-05-08': {
      date: '2019-05-08T00:00:00.000+08:00',
      docId: 'CBUT_article_14',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 2,
        lineUser: 8,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_82'
    },
    '/analytics/doc/article_CBUT_article_14_2019-08-23': {
      date: '2019-08-23T00:00:00.000+08:00',
      docId: 'CBUT_article_14',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 5,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_82'
    },
    '/analytics/doc/article_CBUT_article_14_2019-10-25': {
      date: '2019-10-25T00:00:00.000+08:00',
      docId: 'CBUT_article_14',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 4,
        lineUser: 9,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_82'
    },
    '/analytics/doc/article_CBUT_article_9_2019-01-23': {
      date: '2019-01-23T00:00:00.000+08:00',
      docId: 'CBUT_article_9',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 2,
        lineUser: 8,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_83'
    },
    '/analytics/doc/article_CBUT_article_9_2019-07-29': {
      date: '2019-07-29T00:00:00.000+08:00',
      docId: 'CBUT_article_9',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 1,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_83'
    },
    '/analytics/doc/article_CBUT_article_18_2019-02-16': {
      date: '2019-02-16T00:00:00.000+08:00',
      docId: 'CBUT_article_18',
      type: 'article',
      stats: {
        webUser: 9,
        webVisit: 4,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_18_2019-08-05': {
      date: '2019-08-05T00:00:00.000+08:00',
      docId: 'CBUT_article_18',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 8,
        lineUser: 3,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_18_2019-10-15': {
      date: '2019-10-15T00:00:00.000+08:00',
      docId: 'CBUT_article_18',
      type: 'article',
      stats: {
        lineUser: 2,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_11_2019-05-22': {
      date: '2019-05-22T00:00:00.000+08:00',
      docId: 'CBUT_article_11',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 9
      },
      docUserId: 'CBUT_user_86'
    },
    '/analytics/doc/article_CBUT_article_11_2019-07-10': {
      date: '2019-07-10T00:00:00.000+08:00',
      docId: 'CBUT_article_11',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 8,
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_86'
    },
    '/analytics/doc/article_CBUT_article_11_2019-09-08': {
      date: '2019-09-08T00:00:00.000+08:00',
      docId: 'CBUT_article_11',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 8,
        lineUser: 2,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_86'
    },
    '/analytics/doc/article_CBUT_article_29_2019-01-13': {
      date: '2019-01-13T00:00:00.000+08:00',
      docId: 'CBUT_article_29',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 5,
        lineUser: 9,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_29_2019-02-03': {
      date: '2019-02-03T00:00:00.000+08:00',
      docId: 'CBUT_article_29',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 10,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_29_2019-03-08': {
      date: '2019-03-08T00:00:00.000+08:00',
      docId: 'CBUT_article_29',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 4,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_29_2019-11-16': {
      date: '2019-11-16T00:00:00.000+08:00',
      docId: 'CBUT_article_29',
      type: 'article',
      stats: {
        lineUser: 5,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_29_2019-12-13': {
      date: '2019-12-13T00:00:00.000+08:00',
      docId: 'CBUT_article_29',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 9,
        lineUser: 5,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_0_2019-01-06': {
      date: '2019-01-06T00:00:00.000+08:00',
      docId: 'CBUT_article_0',
      type: 'article',
      stats: {
        lineUser: 4,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_0_2019-04-15': {
      date: '2019-04-15T00:00:00.000+08:00',
      docId: 'CBUT_article_0',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 9
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_0_2019-05-19': {
      date: '2019-05-19T00:00:00.000+08:00',
      docId: 'CBUT_article_0',
      type: 'article',
      stats: {
        webUser: 5,
        webVisit: 9
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_0_2019-08-22': {
      date: '2019-08-22T00:00:00.000+08:00',
      docId: 'CBUT_article_0',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_0_2019-12-26': {
      date: '2019-12-26T00:00:00.000+08:00',
      docId: 'CBUT_article_0',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 9,
        lineUser: 7,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_87'
    },
    '/analytics/doc/article_CBUT_article_20_2019-04-26': {
      date: '2019-04-26T00:00:00.000+08:00',
      docId: 'CBUT_article_20',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_89'
    },
    '/analytics/doc/article_CBUT_article_20_2019-07-31': {
      date: '2019-07-31T00:00:00.000+08:00',
      docId: 'CBUT_article_20',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 5,
        lineUser: 10,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_89'
    },
    '/analytics/doc/article_CBUT_article_20_2019-08-13': {
      date: '2019-08-13T00:00:00.000+08:00',
      docId: 'CBUT_article_20',
      type: 'article',
      stats: {
        webUser: 4,
        webVisit: 3,
        lineUser: 2,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_89'
    },
    '/analytics/doc/article_CBUT_article_20_2019-08-19': {
      date: '2019-08-19T00:00:00.000+08:00',
      docId: 'CBUT_article_20',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 7,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_89'
    },
    '/analytics/doc/article_CBUT_article_20_2019-09-24': {
      date: '2019-09-24T00:00:00.000+08:00',
      docId: 'CBUT_article_20',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_89'
    },
    '/analytics/doc/article_CBUT_article_13_2019-01-22': {
      date: '2019-01-22T00:00:00.000+08:00',
      docId: 'CBUT_article_13',
      type: 'article',
      stats: {
        webUser: 10,
        webVisit: 8,
        lineUser: 1,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_13_2019-02-19': {
      date: '2019-02-19T00:00:00.000+08:00',
      docId: 'CBUT_article_13',
      type: 'article',
      stats: {
        webUser: 2,
        webVisit: 4,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_13_2019-06-30': {
      date: '2019-06-30T00:00:00.000+08:00',
      docId: 'CBUT_article_13',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 7,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_13_2019-12-09': {
      date: '2019-12-09T00:00:00.000+08:00',
      docId: 'CBUT_article_13',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_37'
    },
    '/analytics/doc/article_CBUT_article_43_2019-03-21': {
      date: '2019-03-21T00:00:00.000+08:00',
      docId: 'CBUT_article_43',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_43_2019-04-28': {
      date: '2019-04-28T00:00:00.000+08:00',
      docId: 'CBUT_article_43',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 4,
        lineUser: 2,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_43_2019-06-20': {
      date: '2019-06-20T00:00:00.000+08:00',
      docId: 'CBUT_article_43',
      type: 'article',
      stats: {
        webUser: 7,
        webVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_43_2019-08-31': {
      date: '2019-08-31T00:00:00.000+08:00',
      docId: 'CBUT_article_43',
      type: 'article',
      stats: {
        webUser: 3,
        webVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_44_2019-03-26': {
      date: '2019-03-26T00:00:00.000+08:00',
      docId: 'CBUT_article_44',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 3,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_44_2019-04-18': {
      date: '2019-04-18T00:00:00.000+08:00',
      docId: 'CBUT_article_44',
      type: 'article',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 3,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_44_2019-05-06': {
      date: '2019-05-06T00:00:00.000+08:00',
      docId: 'CBUT_article_44',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 1,
        lineUser: 9,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_44_2019-10-22': {
      date: '2019-10-22T00:00:00.000+08:00',
      docId: 'CBUT_article_44',
      type: 'article',
      stats: {
        webUser: 1,
        webVisit: 6,
        lineUser: 7,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/article_CBUT_article_44_2019-11-27': {
      date: '2019-11-27T00:00:00.000+08:00',
      docId: 'CBUT_article_44',
      type: 'article',
      stats: {
        webUser: 6,
        webVisit: 4,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_76_2019-09-03': {
      date: '2019-09-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_76',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 6,
        lineUser: 3,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_77_2019-04-27': {
      date: '2019-04-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_77',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 8,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_78_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_78',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 5,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_90'
    },
    '/analytics/doc/reply_CBUT_reply_78_2019-05-03': {
      date: '2019-05-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_78',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 6
      },
      docUserId: 'CBUT_user_90'
    },
    '/analytics/doc/reply_CBUT_reply_78_2019-06-19': {
      date: '2019-06-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_78',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 7,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_90'
    },
    '/analytics/doc/reply_CBUT_reply_79_2019-05-09': {
      date: '2019-05-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_79',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 2
      },
      docUserId: 'CBUT_user_16'
    },
    '/analytics/doc/reply_CBUT_reply_79_2019-05-16': {
      date: '2019-05-16T00:00:00.000+08:00',
      docId: 'CBUT_reply_79',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_16'
    },
    '/analytics/doc/reply_CBUT_reply_79_2019-10-27': {
      date: '2019-10-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_79',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 9,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_16'
    },
    '/analytics/doc/reply_CBUT_reply_79_2019-12-23': {
      date: '2019-12-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_79',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 1,
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_16'
    },
    '/analytics/doc/reply_CBUT_reply_80_2019-04-26': {
      date: '2019-04-26T00:00:00.000+08:00',
      docId: 'CBUT_reply_80',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 2,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_80_2019-05-03': {
      date: '2019-05-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_80',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 1,
        lineUser: 2,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_80_2019-07-26': {
      date: '2019-07-26T00:00:00.000+08:00',
      docId: 'CBUT_reply_80',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4,
        lineUser: 7,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_80_2019-09-03': {
      date: '2019-09-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_80',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 3,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_69_2019-01-22': {
      date: '2019-01-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_69',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 4,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_69_2019-03-19': {
      date: '2019-03-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_69',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 5,
        lineUser: 2,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_69_2019-10-16': {
      date: '2019-10-16T00:00:00.000+08:00',
      docId: 'CBUT_reply_69',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 6,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_69_2019-12-23': {
      date: '2019-12-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_69',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 9,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_30_2019-02-25': {
      date: '2019-02-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_30',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 6,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_15_2019-09-07': {
      date: '2019-09-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_15',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 3,
        lineUser: 10,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_15_2019-12-08': {
      date: '2019-12-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_15',
      type: 'reply',
      stats: {
        lineUser: 2,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_21_2019-09-21': {
      date: '2019-09-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_21',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 2,
        lineUser: 5,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_21_2019-12-09': {
      date: '2019-12-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_21',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 1,
        lineUser: 2,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_26_2019-03-30': {
      date: '2019-03-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_26',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 4,
        lineUser: 10,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_38'
    },
    '/analytics/doc/reply_CBUT_reply_26_2019-05-07': {
      date: '2019-05-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_26',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 3
      },
      docUserId: 'CBUT_user_38'
    },
    '/analytics/doc/reply_CBUT_reply_26_2019-06-07': {
      date: '2019-06-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_26',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 9
      },
      docUserId: 'CBUT_user_38'
    },
    '/analytics/doc/reply_CBUT_reply_26_2019-08-26': {
      date: '2019-08-26T00:00:00.000+08:00',
      docId: 'CBUT_reply_26',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 3
      },
      docUserId: 'CBUT_user_38'
    },
    '/analytics/doc/reply_CBUT_reply_27_2019-06-17': {
      date: '2019-06-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_27',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 1,
        lineUser: 2,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_40'
    },
    '/analytics/doc/reply_CBUT_reply_20_2019-04-09': {
      date: '2019-04-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_20',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 3,
        lineUser: 3,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_49'
    },
    '/analytics/doc/reply_CBUT_reply_20_2019-05-01': {
      date: '2019-05-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_20',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 1,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_49'
    },
    '/analytics/doc/reply_CBUT_reply_20_2019-12-25': {
      date: '2019-12-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_20',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 9,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_49'
    },
    '/analytics/doc/reply_CBUT_reply_70_2019-01-03': {
      date: '2019-01-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_70',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 4,
        lineUser: 4,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_70_2019-05-22': {
      date: '2019-05-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_70',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_70_2019-08-13': {
      date: '2019-08-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_70',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_70_2019-10-13': {
      date: '2019-10-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_70',
      type: 'reply',
      stats: {
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_70_2019-12-07': {
      date: '2019-12-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_70',
      type: 'reply',
      stats: {
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_39_2019-04-04': {
      date: '2019-04-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_39',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 9,
        lineUser: 10,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_39_2019-05-21': {
      date: '2019-05-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_39',
      type: 'reply',
      stats: {
        lineUser: 1,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_0_2019-07-05': {
      date: '2019-07-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_0',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_1_2019-01-18': {
      date: '2019-01-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_1',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 6,
        lineUser: 9,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_1_2019-03-17': {
      date: '2019-03-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_1',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_1_2019-05-29': {
      date: '2019-05-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_1',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 8,
        lineUser: 4,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_1_2019-07-03': {
      date: '2019-07-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_1',
      type: 'reply',
      stats: {
        lineUser: 7,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_35_2019-01-27': {
      date: '2019-01-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_35',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 6,
        lineUser: 1,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_35_2019-06-01': {
      date: '2019-06-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_35',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_35_2019-08-01': {
      date: '2019-08-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_35',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 8,
        lineUser: 9,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_35_2019-10-02': {
      date: '2019-10-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_35',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 9,
        lineUser: 2,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_8_2019-01-15': {
      date: '2019-01-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_8',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 6,
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_8_2019-07-12': {
      date: '2019-07-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_8',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_8_2019-08-22': {
      date: '2019-08-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_8',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 1,
        lineUser: 6,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_8_2019-09-24': {
      date: '2019-09-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_8',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_8_2019-10-23': {
      date: '2019-10-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_8',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_32_2019-02-08': {
      date: '2019-02-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_32',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_32_2019-12-27': {
      date: '2019-12-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_32',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 4,
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_72_2019-02-27': {
      date: '2019-02-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_72',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 5,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_72_2019-10-30': {
      date: '2019-10-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_72',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 10,
        lineUser: 8,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_72_2019-12-21': {
      date: '2019-12-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_72',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 2,
        lineUser: 4,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_13_2019-01-05': {
      date: '2019-01-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_13',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 2,
        lineUser: 3,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_13_2019-02-17': {
      date: '2019-02-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_13',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 7,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_13_2019-04-14': {
      date: '2019-04-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_13',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_13_2019-04-19': {
      date: '2019-04-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_13',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_13_2019-11-23': {
      date: '2019-11-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_13',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_85'
    },
    '/analytics/doc/reply_CBUT_reply_66_2019-04-06': {
      date: '2019-04-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_66',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 3,
        lineUser: 1,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_66_2019-05-07': {
      date: '2019-05-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_66',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_6_2019-02-05': {
      date: '2019-02-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_6',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 1,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_6_2019-04-18': {
      date: '2019-04-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_6',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_6_2019-07-10': {
      date: '2019-07-10T00:00:00.000+08:00',
      docId: 'CBUT_reply_6',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 8,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_6_2019-08-12': {
      date: '2019-08-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_6',
      type: 'reply',
      stats: {
        lineUser: 3,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_6_2019-12-12': {
      date: '2019-12-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_6',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_23_2019-01-17': {
      date: '2019-01-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_23',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 6,
        lineUser: 8,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_23_2019-07-11': {
      date: '2019-07-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_23',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 8,
        lineUser: 8,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_23_2019-07-30': {
      date: '2019-07-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_23',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 5,
        lineUser: 6,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_23_2019-08-29': {
      date: '2019-08-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_23',
      type: 'reply',
      stats: {
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_23_2019-12-07': {
      date: '2019-12-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_23',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 3,
        lineUser: 1,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_73_2019-03-10': {
      date: '2019-03-10T00:00:00.000+08:00',
      docId: 'CBUT_reply_73',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 1,
        lineUser: 2,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_73_2019-05-07': {
      date: '2019-05-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_73',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 7,
        lineUser: 4,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_73_2019-08-20': {
      date: '2019-08-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_73',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_37_2019-02-18': {
      date: '2019-02-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_37',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 2,
        lineUser: 4,
        lineVisit: 1
      }
    },
    '/analytics/doc/reply_CBUT_reply_37_2019-07-14': {
      date: '2019-07-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_37',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 4
      }
    },
    '/analytics/doc/reply_CBUT_reply_37_2019-11-04': {
      date: '2019-11-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_37',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 2
      }
    },
    '/analytics/doc/reply_CBUT_reply_16_2019-03-02': {
      date: '2019-03-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_16',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 10,
        lineUser: 9,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_16_2019-04-20': {
      date: '2019-04-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_16',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 7,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_16_2019-05-17': {
      date: '2019-05-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_16',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 2,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_16_2019-08-25': {
      date: '2019-08-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_16',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 3,
        lineUser: 3,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_16_2019-09-23': {
      date: '2019-09-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_16',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 6,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_38_2019-08-08': {
      date: '2019-08-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_38',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_38_2019-09-11': {
      date: '2019-09-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_38',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 10,
        lineUser: 8,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_38_2019-12-30': {
      date: '2019-12-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_38',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 8,
        lineUser: 7,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_7_2019-03-23': {
      date: '2019-03-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_7',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 9,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_7_2019-06-06': {
      date: '2019-06-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_7',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 8,
        lineUser: 3,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_7_2019-07-10': {
      date: '2019-07-10T00:00:00.000+08:00',
      docId: 'CBUT_reply_7',
      type: 'reply',
      stats: {
        lineUser: 8,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_7_2019-08-27': {
      date: '2019-08-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_7',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 9,
        lineUser: 9,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_7_2019-12-19': {
      date: '2019-12-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_7',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 3,
        lineUser: 8,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_3_2019-03-06': {
      date: '2019-03-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_3',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 4
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_3_2019-04-09': {
      date: '2019-04-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_3',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4,
        lineUser: 3,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_3_2019-11-02': {
      date: '2019-11-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_3',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 9,
        lineUser: 10,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_71_2019-05-18': {
      date: '2019-05-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_71',
      type: 'reply',
      stats: {
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_71_2019-07-11': {
      date: '2019-07-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_71',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 2,
        lineUser: 3,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_71_2019-08-26': {
      date: '2019-08-26T00:00:00.000+08:00',
      docId: 'CBUT_reply_71',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 2,
        lineUser: 7,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_71_2019-12-25': {
      date: '2019-12-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_71',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 4,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_10_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_10',
      type: 'reply',
      stats: {
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_10_2019-05-02': {
      date: '2019-05-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_10',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 9,
        lineUser: 1,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_10_2019-08-05': {
      date: '2019-08-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_10',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_10_2019-10-01': {
      date: '2019-10-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_10',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_10_2019-12-25': {
      date: '2019-12-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_10',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 9,
        lineUser: 7,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_40_2019-01-04': {
      date: '2019-01-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_40',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 7,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_40_2019-03-09': {
      date: '2019-03-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_40',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 2,
        lineUser: 1,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_40_2019-04-29': {
      date: '2019-04-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_40',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 3,
        lineUser: 8,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_40_2019-09-24': {
      date: '2019-09-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_40',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 1,
        lineUser: 3,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_40_2019-10-25': {
      date: '2019-10-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_40',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 9,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_41_2019-11-22': {
      date: '2019-11-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_41',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_42_2019-03-23': {
      date: '2019-03-23T00:00:00.000+08:00',
      docId: 'CBUT_reply_42',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 10,
        lineUser: 4,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_53'
    },
    '/analytics/doc/reply_CBUT_reply_42_2019-07-17': {
      date: '2019-07-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_42',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_53'
    },
    '/analytics/doc/reply_CBUT_reply_42_2019-08-03': {
      date: '2019-08-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_42',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 5,
        lineUser: 7,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_53'
    },
    '/analytics/doc/reply_CBUT_reply_42_2019-09-15': {
      date: '2019-09-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_42',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 10,
        lineUser: 9,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_53'
    },
    '/analytics/doc/reply_CBUT_reply_42_2019-10-01': {
      date: '2019-10-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_42',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 7,
        lineUser: 1,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_53'
    },
    '/analytics/doc/reply_CBUT_reply_46_2019-03-14': {
      date: '2019-03-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_46',
      type: 'reply',
      stats: {
        lineUser: 7,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_46_2019-11-14': {
      date: '2019-11-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_46',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_18_2019-05-20': {
      date: '2019-05-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_18',
      type: 'reply',
      stats: {
        lineUser: 6,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_73'
    },
    '/analytics/doc/reply_CBUT_reply_18_2019-08-11': {
      date: '2019-08-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_18',
      type: 'reply',
      stats: {
        lineUser: 6,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_73'
    },
    '/analytics/doc/reply_CBUT_reply_25_2019-04-09': {
      date: '2019-04-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_25',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 10,
        lineUser: 2,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_25_2019-06-22': {
      date: '2019-06-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_25',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 9,
        lineUser: 1,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_25_2019-08-30': {
      date: '2019-08-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_25',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 4,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_64_2019-01-19': {
      date: '2019-01-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_64',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_64_2019-03-13': {
      date: '2019-03-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_64',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_64_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_64',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_63_2019-04-18': {
      date: '2019-04-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_63',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 1,
        lineUser: 8,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_63_2019-07-05': {
      date: '2019-07-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_63',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 5,
        lineUser: 10,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_28_2019-08-30': {
      date: '2019-08-30T00:00:00.000+08:00',
      docId: 'CBUT_reply_28',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 10,
        lineUser: 1,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_29_2019-02-27': {
      date: '2019-02-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_29',
      type: 'reply',
      stats: {
        lineUser: 2,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_29_2019-10-13': {
      date: '2019-10-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_29',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 9,
        lineUser: 8,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_68_2019-03-24': {
      date: '2019-03-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_68',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 1,
        lineUser: 8,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_68_2019-07-08': {
      date: '2019-07-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_68',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 9,
        lineUser: 8,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_68_2019-10-11': {
      date: '2019-10-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_68',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 2,
        lineUser: 3,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_68_2019-10-16': {
      date: '2019-10-16T00:00:00.000+08:00',
      docId: 'CBUT_reply_68',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 5,
        lineUser: 5,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_58_2019-01-13': {
      date: '2019-01-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_58',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_58_2019-04-11': {
      date: '2019-04-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_58',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 9,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_58_2019-11-07': {
      date: '2019-11-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_58',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 10,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_58_2019-12-12': {
      date: '2019-12-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_58',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4,
        lineUser: 9,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_31_2019-10-02': {
      date: '2019-10-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_31',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_45'
    },
    '/analytics/doc/reply_CBUT_reply_2_2019-04-11': {
      date: '2019-04-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_2',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 5
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_2_2019-06-01': {
      date: '2019-06-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_2',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_2_2019-07-04': {
      date: '2019-07-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_2',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 1
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_2_2019-10-20': {
      date: '2019-10-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_2',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4,
        lineUser: 2,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_2_2019-11-18': {
      date: '2019-11-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_2',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6,
        lineUser: 9,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_33_2019-03-13': {
      date: '2019-03-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_33',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_33_2019-03-29': {
      date: '2019-03-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_33',
      type: 'reply',
      stats: {
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_33_2019-09-13': {
      date: '2019-09-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_33',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_33_2019-10-09': {
      date: '2019-10-09T00:00:00.000+08:00',
      docId: 'CBUT_reply_33',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 8,
        lineUser: 3,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_34_2019-01-12': {
      date: '2019-01-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_34',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_34_2019-06-22': {
      date: '2019-06-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_34',
      type: 'reply',
      stats: {
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_34_2019-11-29': {
      date: '2019-11-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_34',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 10,
        lineUser: 3,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_34_2019-12-15': {
      date: '2019-12-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_34',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 3,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_54_2019-01-29': {
      date: '2019-01-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_54',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 8,
        lineUser: 3,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_60_2019-03-19': {
      date: '2019-03-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_60',
      type: 'reply',
      stats: {
        lineUser: 3,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_60_2019-03-21': {
      date: '2019-03-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_60',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 8,
        lineUser: 1,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_60_2019-04-15': {
      date: '2019-04-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_60',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 10,
        lineUser: 5,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_60_2019-05-04': {
      date: '2019-05-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_60',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 2,
        lineUser: 10,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_9_2019-06-02': {
      date: '2019-06-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_9',
      type: 'reply',
      stats: {
        lineUser: 9,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_22_2019-01-26': {
      date: '2019-01-26T00:00:00.000+08:00',
      docId: 'CBUT_reply_22',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 1,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_22_2019-05-19': {
      date: '2019-05-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_22',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 5,
        lineUser: 4,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_43_2019-02-14': {
      date: '2019-02-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_43',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 7,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_43_2019-02-22': {
      date: '2019-02-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_43',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 8
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_43_2019-08-24': {
      date: '2019-08-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_43',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 9,
        lineUser: 7,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_43_2019-10-31': {
      date: '2019-10-31T00:00:00.000+08:00',
      docId: 'CBUT_reply_43',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_17_2019-09-19': {
      date: '2019-09-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_17',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 3,
        lineUser: 7,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_47'
    },
    '/analytics/doc/reply_CBUT_reply_36_2019-09-14': {
      date: '2019-09-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_36',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 8,
        lineUser: 10,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_48'
    },
    '/analytics/doc/reply_CBUT_reply_36_2019-12-14': {
      date: '2019-12-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_36',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6,
        lineUser: 2,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_48'
    },
    '/analytics/doc/reply_CBUT_reply_61_2019-01-12': {
      date: '2019-01-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_61',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_74'
    },
    '/analytics/doc/reply_CBUT_reply_61_2019-01-28': {
      date: '2019-01-28T00:00:00.000+08:00',
      docId: 'CBUT_reply_61',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 4,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_74'
    },
    '/analytics/doc/reply_CBUT_reply_61_2019-03-19': {
      date: '2019-03-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_61',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 3
      },
      docUserId: 'CBUT_user_74'
    },
    '/analytics/doc/reply_CBUT_reply_61_2019-05-12': {
      date: '2019-05-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_61',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 4,
        lineUser: 3,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_74'
    },
    '/analytics/doc/reply_CBUT_reply_57_2019-03-29': {
      date: '2019-03-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_57',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 8,
        lineUser: 4,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_57_2019-04-29': {
      date: '2019-04-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_57',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 10,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_57_2019-05-06': {
      date: '2019-05-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_57',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 3,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_57_2019-11-02': {
      date: '2019-11-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_57',
      type: 'reply',
      stats: {
        lineUser: 4,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_57_2019-12-25': {
      date: '2019-12-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_57',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 8,
        lineUser: 2,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_24_2019-01-28': {
      date: '2019-01-28T00:00:00.000+08:00',
      docId: 'CBUT_reply_24',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_88'
    },
    '/analytics/doc/reply_CBUT_reply_24_2019-10-16': {
      date: '2019-10-16T00:00:00.000+08:00',
      docId: 'CBUT_reply_24',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 7,
        lineUser: 7,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_88'
    },
    '/analytics/doc/reply_CBUT_reply_47_2019-05-07': {
      date: '2019-05-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_47',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_47_2019-05-08': {
      date: '2019-05-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_47',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 9,
        lineUser: 1,
        lineVisit: 10
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_19_2019-01-11': {
      date: '2019-01-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_19',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 3,
        lineUser: 2,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_19_2019-09-24': {
      date: '2019-09-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_19',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_65_2019-02-14': {
      date: '2019-02-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_65',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 1,
        lineUser: 5,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_79'
    },
    '/analytics/doc/reply_CBUT_reply_65_2019-06-03': {
      date: '2019-06-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_65',
      type: 'reply',
      stats: {
        webUser: 10,
        webVisit: 9,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_79'
    },
    '/analytics/doc/reply_CBUT_reply_65_2019-09-10': {
      date: '2019-09-10T00:00:00.000+08:00',
      docId: 'CBUT_reply_65',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 9,
        lineUser: 5,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_79'
    },
    '/analytics/doc/reply_CBUT_reply_65_2020-01-01': {
      date: '2020-01-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_65',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 7,
        lineUser: 8,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_79'
    },
    '/analytics/doc/reply_CBUT_reply_14_2019-02-06': {
      date: '2019-02-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_14',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_14_2019-02-18': {
      date: '2019-02-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_14',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_14_2019-03-13': {
      date: '2019-03-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_14',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 5,
        lineUser: 3,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_14_2019-04-08': {
      date: '2019-04-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_14',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 9,
        lineUser: 4,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_4_2019-08-02': {
      date: '2019-08-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_4',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_5_2019-03-16': {
      date: '2019-03-16T00:00:00.000+08:00',
      docId: 'CBUT_reply_5',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 9,
        lineUser: 5,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_68'
    },
    '/analytics/doc/reply_CBUT_reply_5_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_5',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 1,
        lineUser: 4,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_68'
    },
    '/analytics/doc/reply_CBUT_reply_5_2019-09-21': {
      date: '2019-09-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_5',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 5,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_68'
    },
    '/analytics/doc/reply_CBUT_reply_5_2019-11-13': {
      date: '2019-11-13T00:00:00.000+08:00',
      docId: 'CBUT_reply_5',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 10,
        lineUser: 9,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_68'
    },
    '/analytics/doc/reply_CBUT_reply_11_2019-12-22': {
      date: '2019-12-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_11',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 8,
        lineUser: 8,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_15'
    },
    '/analytics/doc/reply_CBUT_reply_53_2019-07-06': {
      date: '2019-07-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_53',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 3,
        lineUser: 2,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_53_2019-08-02': {
      date: '2019-08-02T00:00:00.000+08:00',
      docId: 'CBUT_reply_53',
      type: 'reply',
      stats: {
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_53_2019-10-20': {
      date: '2019-10-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_53',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 5,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_44_2019-01-15': {
      date: '2019-01-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_44',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 3,
        lineUser: 2,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_57'
    },
    '/analytics/doc/reply_CBUT_reply_44_2019-04-27': {
      date: '2019-04-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_44',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 8,
        lineUser: 10,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_57'
    },
    '/analytics/doc/reply_CBUT_reply_44_2019-05-29': {
      date: '2019-05-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_44',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 1,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_57'
    },
    '/analytics/doc/reply_CBUT_reply_44_2019-10-28': {
      date: '2019-10-28T00:00:00.000+08:00',
      docId: 'CBUT_reply_44',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 6,
        lineUser: 3,
        lineVisit: 5
      },
      docUserId: 'CBUT_user_57'
    },
    '/analytics/doc/reply_CBUT_reply_51_2019-03-22': {
      date: '2019-03-22T00:00:00.000+08:00',
      docId: 'CBUT_reply_51',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 7,
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_65'
    },
    '/analytics/doc/reply_CBUT_reply_51_2019-12-17': {
      date: '2019-12-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_51',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 8,
        lineUser: 5,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_65'
    },
    '/analytics/doc/reply_CBUT_reply_52_2019-03-08': {
      date: '2019-03-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_52',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 1
      },
      docUserId: 'CBUT_user_65'
    },
    '/analytics/doc/reply_CBUT_reply_52_2019-10-31': {
      date: '2019-10-31T00:00:00.000+08:00',
      docId: 'CBUT_reply_52',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 7,
        lineUser: 5,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_65'
    },
    '/analytics/doc/reply_CBUT_reply_12_2019-10-12': {
      date: '2019-10-12T00:00:00.000+08:00',
      docId: 'CBUT_reply_12',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 8,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_50_2019-02-05': {
      date: '2019-02-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_50',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 4,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_63'
    },
    '/analytics/doc/reply_CBUT_reply_50_2019-04-14': {
      date: '2019-04-14T00:00:00.000+08:00',
      docId: 'CBUT_reply_50',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 7,
        lineUser: 6,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_63'
    },
    '/analytics/doc/reply_CBUT_reply_50_2019-07-01': {
      date: '2019-07-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_50',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 2,
        lineUser: 4,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_63'
    },
    '/analytics/doc/reply_CBUT_reply_50_2019-12-29': {
      date: '2019-12-29T00:00:00.000+08:00',
      docId: 'CBUT_reply_50',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 9
      },
      docUserId: 'CBUT_user_63'
    },
    '/analytics/doc/reply_CBUT_reply_67_2019-02-06': {
      date: '2019-02-06T00:00:00.000+08:00',
      docId: 'CBUT_reply_67',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 7,
        lineUser: 1,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_62_2019-03-17': {
      date: '2019-03-17T00:00:00.000+08:00',
      docId: 'CBUT_reply_62',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 3,
        lineUser: 8,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_62_2019-09-28': {
      date: '2019-09-28T00:00:00.000+08:00',
      docId: 'CBUT_reply_62',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 6,
        lineUser: 5,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_45_2019-03-01': {
      date: '2019-03-01T00:00:00.000+08:00',
      docId: 'CBUT_reply_45',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 7,
        lineUser: 4,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_45_2019-07-05': {
      date: '2019-07-05T00:00:00.000+08:00',
      docId: 'CBUT_reply_45',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 2,
        lineUser: 1,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_45_2019-09-10': {
      date: '2019-09-10T00:00:00.000+08:00',
      docId: 'CBUT_reply_45',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 7,
        lineUser: 2,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_54'
    },
    '/analytics/doc/reply_CBUT_reply_56_2019-01-07': {
      date: '2019-01-07T00:00:00.000+08:00',
      docId: 'CBUT_reply_56',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 4,
        lineUser: 1,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_69'
    },
    '/analytics/doc/reply_CBUT_reply_56_2019-04-27': {
      date: '2019-04-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_56',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 2,
        lineUser: 8,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_69'
    },
    '/analytics/doc/reply_CBUT_reply_56_2019-05-19': {
      date: '2019-05-19T00:00:00.000+08:00',
      docId: 'CBUT_reply_56',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 6,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_69'
    },
    '/analytics/doc/reply_CBUT_reply_56_2019-10-18': {
      date: '2019-10-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_56',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 2,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_69'
    },
    '/analytics/doc/reply_CBUT_reply_55_2019-02-15': {
      date: '2019-02-15T00:00:00.000+08:00',
      docId: 'CBUT_reply_55',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 5,
        lineUser: 7,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_68'
    },
    '/analytics/doc/reply_CBUT_reply_48_2019-09-11': {
      date: '2019-09-11T00:00:00.000+08:00',
      docId: 'CBUT_reply_48',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 9,
        lineUser: 9,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_60'
    },
    '/analytics/doc/reply_CBUT_reply_48_2019-09-18': {
      date: '2019-09-18T00:00:00.000+08:00',
      docId: 'CBUT_reply_48',
      type: 'reply',
      stats: {
        webUser: 7,
        webVisit: 1,
        lineUser: 9,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_60'
    },
    '/analytics/doc/reply_CBUT_reply_48_2019-09-21': {
      date: '2019-09-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_48',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 1,
        lineUser: 6,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_60'
    },
    '/analytics/doc/reply_CBUT_reply_48_2019-11-27': {
      date: '2019-11-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_48',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 8,
        lineUser: 10,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_60'
    },
    '/analytics/doc/reply_CBUT_reply_48_2019-12-24': {
      date: '2019-12-24T00:00:00.000+08:00',
      docId: 'CBUT_reply_48',
      type: 'reply',
      stats: {
        lineUser: 8,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_60'
    },
    '/analytics/doc/reply_CBUT_reply_49_2019-05-21': {
      date: '2019-05-21T00:00:00.000+08:00',
      docId: 'CBUT_reply_49',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 5,
        lineUser: 6,
        lineVisit: 6
      },
      docUserId: 'CBUT_user_61'
    },
    '/analytics/doc/reply_CBUT_reply_49_2019-11-20': {
      date: '2019-11-20T00:00:00.000+08:00',
      docId: 'CBUT_reply_49',
      type: 'reply',
      stats: {
        webUser: 4,
        webVisit: 6,
        lineUser: 1,
        lineVisit: 7
      },
      docUserId: 'CBUT_user_61'
    },
    '/analytics/doc/reply_CBUT_reply_59_2019-01-31': {
      date: '2019-01-31T00:00:00.000+08:00',
      docId: 'CBUT_reply_59',
      type: 'reply',
      stats: {
        webUser: 5,
        webVisit: 7,
        lineUser: 8,
        lineVisit: 2
      },
      docUserId: 'CBUT_user_71'
    },
    '/analytics/doc/reply_CBUT_reply_59_2019-04-27': {
      date: '2019-04-27T00:00:00.000+08:00',
      docId: 'CBUT_reply_59',
      type: 'reply',
      stats: {
        webUser: 9,
        webVisit: 10,
        lineUser: 6,
        lineVisit: 4
      },
      docUserId: 'CBUT_user_71'
    },
    '/analytics/doc/reply_CBUT_reply_59_2019-06-08': {
      date: '2019-06-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_59',
      type: 'reply',
      stats: {
        webUser: 6,
        webVisit: 6,
        lineUser: 7,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_71'
    },
    '/analytics/doc/reply_CBUT_reply_59_2019-08-28': {
      date: '2019-08-28T00:00:00.000+08:00',
      docId: 'CBUT_reply_59',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 1,
        lineUser: 1,
        lineVisit: 8
      },
      docUserId: 'CBUT_user_71'
    },
    '/analytics/doc/reply_CBUT_reply_59_2019-10-25': {
      date: '2019-10-25T00:00:00.000+08:00',
      docId: 'CBUT_reply_59',
      type: 'reply',
      stats: {
        webUser: 8,
        webVisit: 2,
        lineUser: 1,
        lineVisit: 1
      },
      docUserId: 'CBUT_user_71'
    },
    '/analytics/doc/reply_CBUT_reply_74_2019-07-03': {
      date: '2019-07-03T00:00:00.000+08:00',
      docId: 'CBUT_reply_74',
      type: 'reply',
      stats: {
        webUser: 2,
        webVisit: 7
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_74_2019-11-04': {
      date: '2019-11-04T00:00:00.000+08:00',
      docId: 'CBUT_reply_74',
      type: 'reply',
      stats: {
        webUser: 1,
        webVisit: 6,
        lineUser: 6,
        lineVisit: 9
      },
      docUserId: 'CBUT_user_35'
    },
    '/analytics/doc/reply_CBUT_reply_75_2019-02-08': {
      date: '2019-02-08T00:00:00.000+08:00',
      docId: 'CBUT_reply_75',
      type: 'reply',
      stats: {
        webUser: 3,
        webVisit: 4,
        lineUser: 6,
        lineVisit: 3
      },
      docUserId: 'CBUT_user_35'
    }
  },
  expectedResults: {
    users: {
      '/users/doc/CBUT_user_0': {
        createdAt: '2019-02-18T06:28:55.720Z',
        updatedAt: '2019-02-18T06:28:55.720Z',
        email: 'cnMsf0z@example.com',
        name: 'uH2VDAgiwf UC6V4',
        facebookId: 'CBUT_facebook_0',
      },
      '/users/doc/CBUT_user_1': {
        createdAt: '2019-10-15T05:11:15.972Z',
        updatedAt: '2019-10-15T05:11:15.972Z',
        email: '8xeGKHZOda@example.com',
        name: 'QcGDf68rq D3ZWh',
        facebookId: 'CBUT_facebook_1',
      },
      '/users/doc/CBUT_user_10': {
        createdAt: '2020-03-12T14:45:03.825Z',
        updatedAt: '2020-03-12T14:45:03.825Z',
        email: 'LOv6ae2@example.com',
        name: '5aDOfh nMM705Ml',
        facebookId: 'CBUT_facebook_10',
      },
      '/users/doc/CBUT_user_11': {
        createdAt: '2020-02-07T15:27:05.303Z',
        updatedAt: '2020-02-07T15:27:05.303Z',
        email: 'ZRc1WF1ih@example.com',
        name: 'KDnWGKCis HUQADXS5dP',
        facebookId: 'CBUT_facebook_11',
      },
      '/users/doc/CBUT_user_12': {
        createdAt: '2019-05-14T02:20:27.275Z',
        updatedAt: '2019-05-14T02:20:27.275Z',
        name: 'BPZWZW BufLuDil',
        facebookId: 'CBUT_facebook_12',
      },
      '/users/doc/CBUT_user_13': {
        createdAt: '2019-06-18T15:20:54.339Z',
        updatedAt: '2019-06-18T15:20:54.339Z',
        email: 'DmjXIw@example.com',
        name: 'EXJbf brCVMftg',
        facebookId: 'CBUT_facebook_13',
      },
      '/users/doc/CBUT_user_14': {
        createdAt: '2020-09-26T21:38:58.624Z',
        updatedAt: '2020-09-26T21:38:58.624Z',
        email: 'Ci_HpyP6m@example.com',
        name: '8peFNlZdWT aSAqUlJ',
        facebookId: 'CBUT_facebook_14',
      },
      '/users/doc/CBUT_user_15': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_16': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_2': {
        createdAt: '2020-07-06T05:46:06.931Z',
        updatedAt: '2020-07-06T05:46:06.931Z',
        email: 'GFsBpOzw1t@example.com',
        name: 'fzAlW7 lTLr8U',
        facebookId: 'CBUT_facebook_2',
      },
      '/users/doc/CBUT_user_20': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_23': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_3': {
        createdAt: '2017-08-07T04:03:29.995Z',
        updatedAt: '2017-08-07T04:03:29.995Z',
        email: 'WNEhSxYx@example.com',
        name: 'nUWqA 9zu04Q',
        facebookId: 'CBUT_facebook_3',
      },
      '/users/doc/CBUT_user_35': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_38': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_4': {
        createdAt: '2019-03-30T03:46:39.014Z',
        updatedAt: '2019-03-30T03:46:39.014Z',
        email: '-49hG@example.com',
        name: '0W8kdJ Vg9XsuVhy',
        facebookId: 'CBUT_facebook_4',
      },
      '/users/doc/CBUT_user_40': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_45': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_47': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_48': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_49': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_5': {
        createdAt: '2018-12-25T10:06:57.131Z',
        updatedAt: '2018-12-25T10:06:57.131Z',
        email: 'hsnKHanBb5@example.com',
        name: 'dTPZcxn4W aWHchdvji',
        facebookId: 'CBUT_facebook_5',
      },
      '/users/doc/CBUT_user_53': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_54': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_57': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_6': {
        createdAt: '2019-02-12T03:40:04.690Z',
        updatedAt: '2019-02-12T03:40:04.690Z',
        email: 'X6M6CqP6-@example.com',
        name: '2PK0m KbAK-wu',
        facebookId: 'CBUT_facebook_6',
      },
      '/users/doc/CBUT_user_60': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_61': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_63': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_65': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_68': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_69': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_7': {
        createdAt: '2019-06-17T03:14:26.547Z',
        updatedAt: '2019-06-17T05:39:45.935Z',
        email: 'mtSHa9pQR@example.com',
        name: '1waeHG H5nlk5w',
        facebookId: 'CBUT_facebook_7',
      },
      '/users/doc/CBUT_user_71': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_73': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_74': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_79': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_8': {
        createdAt: '2018-12-02T05:48:31.102Z',
        updatedAt: '2018-12-02T05:48:31.102Z',
        email: '9bScmbJ@example.com',
        name: 'B4ywca jlF0w',
        facebookId: 'CBUT_facebook_8',
      },
      '/users/doc/CBUT_user_84': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_85': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_88': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/CBUT_user_9': {
        createdAt: '2020-05-05T09:14:15.364Z',
        updatedAt: '2020-05-05T09:14:15.364Z',
        email: 'k-MJ48Wozf@example.com',
        name: 'AvqX7 SXguFvvotD',
        facebookId: 'CBUT_facebook_9',
      },
      '/users/doc/CBUT_user_90': {
        createdAt: '2010-01-01T00:00:00Z',
      },
      '/users/doc/Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU': {
        appUserId: 'CBUT_user_37',
        appId: 'BOT_LEGACY',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/Z0k1e_9jAgBIhrjzs66_4pQrS0Gq4m79N5KCuHY9q8U1xUjYw': {
        appUserId: 'CBUT_user_35',
        appId: 'BOT_LEGACY',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_-jyEGNy2w04UVgIb9-imCXweVYP3sniZ-q4Y8JNXL9k': {
        appUserId: 'CBUT_user_26',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_4PNVISSWISKdcDP111lIO_cSmpR1jMjSyzDwLoYDvH4': {
        appUserId: 'CBUT_user_80',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_4WVN4mcFb9V8KKu6LT8xXvRud2LWcSa-UjL_qvy0ksw': {
        appUserId: 'CBUT_user_76',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_4iGkmTeduiI5SrYlHehwmaS3SX4Nszoyb1oyUWvLMEo': {
        appUserId: 'CBUT_user_82',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_9V5d6E7GKiGNn1Yv_ifdq_WCxK4GET6BedItZHNN3W0': {
        appUserId: 'CBUT_user_72',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_AunHFdGqCNC2QRUrQ9PSqkcE49Z4JTrkG9mYcgeogm8': {
        appUserId: 'CBUT_user_67',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_FQn8c8K4i67IISzvXVoOPipzRASqSgeBxPLNjReHVyY': {
        appUserId: 'CBUT_user_42',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_Imj3cjSxE4ieAND7jCqxBcT1kYMCozfzPqI2CM2B4d4': {
        appUserId: 'CBUT_user_18',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_J-qFf2nzWHvRnx3qA2hEhRhp4ASBnYRkq3IA2i7ngg8': {
        appUserId: 'CBUT_user_30',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_LD7dd370hd6B1O-rTkpX9I_n1AhIlTPD_QmDsGA6WoQ': {
        appUserId: 'CBUT_user_29',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_LY7EIPDC671J6K2uZ9EznQAEDhznt8gUPsK0cwSDbFQ': {
        appUserId: 'CBUT_user_24',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_MKBfQLupvhw4_Pf0ua4s1ag2aNnC7lr88z8Hyx7C31g': {
        appUserId: 'CBUT_user_89',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_O3LIyatOUXZjhZqeqQ5whc8glQZy6b-53QrsrO_SFbs': {
        appUserId: 'CBUT_user_28',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_RW_vDd8lKq4WdfWYxmVh6sJImYM96kix8X4A6FhRJHI': {
        appUserId: 'CBUT_user_55',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_TthEcGGgLb2we2tKAG98NuJq1KJE6Bd2WbQPK7kqG2M': {
        appUserId: 'CBUT_user_56',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_VSYVIlqF4l_puOdN_Pzu64YnjMc-nM5iu0OWcWTt4xk': {
        appUserId: 'CBUT_user_78',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_Y1PaNolSaA4JExH8wY1ODwOCa4lNbbsPOtxJts305XU': {
        appUserId: 'CBUT_user_87',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_YWVSKKajD6USiTghIEQzrWT_YKdnVVqVI96-cyS6zWA': {
        appUserId: 'CBUT_user_81',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_Yk4N-rJ2LtcKZc0Wa2smuJQNB5I7mmo-d2j1k-nyfSo': {
        appUserId: 'CBUT_user_83',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_Z8nYqKv190EeosqG5O1H8aeTg64m6xLkRyZOTxSYQ7A': {
        appUserId: 'CBUT_user_46',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_ZvTJZB57GHkjobdIGZXOtibZ9PYEmQu8pnG-rYXS85A': {
        appUserId: 'CBUT_user_32',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_f0pGq97e0_aH_2-8x0BLaj36xzuu2M1W54ZVcAZ4fM0': {
        appUserId: 'CBUT_user_39',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_fqo4vs7i2gmJi3mc1zlp_YgMi2_xyGiHfOR5_eLeYkE': {
        appUserId: 'CBUT_user_66',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_igegSRiBLdZh9argcdWuKn3cZzYGaSCes5WLJr-9l2s': {
        appUserId: 'CBUT_user_58',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_lQfJ8fARjHNzkXa4s0ZENAkcWbKn_UOARG7VY11af8E': {
        appUserId: 'CBUT_user_50',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_n66M1oaUXCwx_g7G3O5VFjHWL4dbZqq9aiEewD4s8-g': {
        appUserId: 'CBUT_user_62',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_nEkrBWbE4FfkBTIV9QMn1exDEGReMdemzfznDgx6nA8': {
        appUserId: 'CBUT_user_70',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_nRWu8ne4K6M1m6ibF8HbOq07hUxK3pvrOD4g7cn-okY': {
        appUserId: 'CBUT_user_64',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_pGTVQioRDsflknEcedoVAbKom5K10ipYSV4rWSq2G_4': {
        appUserId: 'CBUT_user_41',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_pYR1NvuKvLQfho6HCkJWbMqLYFnDzoATlM4SyqsxdEU': {
        appUserId: 'CBUT_user_44',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_ppZEoc7CPAXLOlzNOj3zwN_re6FKnwtjIiMmzS0_TFk': {
        appUserId: 'CBUT_user_77',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_pwhC-K1IePUO5xsLr_3CDzSaH1yk7_OfUJoJeG3lVi4': {
        appUserId: 'CBUT_user_19',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo': {
        appUserId: 'CBUT_user_17',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_rjLUCvhHqmYeT37nsWvYMXt7o0r3Mbniqdr7qU5LXgo': {
        appUserId: 'CBUT_user_51',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_sJtyYDWJm0yBONBKJix3-poQzSELnS-Xyr7WPZdwvvA': {
        appUserId: 'CBUT_user_75',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_u7STzSnt-bwqVRc9tCmf47W2LE79MRzNrA-_oxzLIFc': {
        appUserId: 'CBUT_user_27',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_uQip0_ezI3-UENg4tvIubbsdJDOHYWUs8sbOeCIaKN8': {
        appUserId: 'CBUT_user_52',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_uURhRzjSSV6s0wz7Patvy8Fb2hBM46AgZeTSyZ0OUfk': {
        appUserId: 'CBUT_user_43',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_v8peSS0932LsPFMsF61kNZqh7-ZkQ_2QHV22_C3PXf4': {
        appUserId: 'CBUT_user_86',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/j4S8C_xhN2O5Ouv6Y6AbEoDULy9Crc1TRdEaQ03ANLtAEJWnY': {
        appUserId: 'CBUT_user_59',
        appId: 'RUMORS_LINE_BOT',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_86RLd-liEHXfx4M_8H9J7Y0Wb13RIieRp5-2pFxb0Jo': {
        appUserId: 'CBUT_user_31',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_gGWtOcj9ARqZRT4msCGaXS-Iof7eL3XxmT3AOTTGCVU': {
        appUserId: 'CBUT_user_25',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_lAgNK9-guL13qEJSGROTsi_UZbF5N5WoQ17vnsOrl6s': {
        appUserId: 'CBUT_user_34',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_lHfjAYn66nWNP7XCDrdY1gaZmJbQ1wYAgzOTT8xsTXc': {
        appUserId: 'CBUT_user_21',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU': {
        appUserId: 'CBUT_user_36',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo': {
        appUserId: 'CBUT_user_17',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_sYnv5g2wKCqsHcFZh-LocxMbYLn-fPAet_WiV0JRIDE': {
        appUserId: 'CBUT_user_22',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
      '/users/doc/qO1C3_wR_HXQWSJ2jY6OA3JRGZrIgVXadixeKTQsX1qxuHsKo': {
        appUserId: 'CBUT_user_33',
        appId: 'DEVELOPMENT_BACKEND',
        avatarType: 'OpenPeeps',
      },
    },
    articlecategoryfeedbacks: {
      '/articlecategoryfeedbacks/doc/CBUT_article_0__CBUT_category_0__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_0',
        categoryId: 'CBUT_category_0',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-09-19T07:07:05.785Z',
        updatedAt: '2020-09-19T07:07:05.785Z',
      },
      '/articlecategoryfeedbacks/doc/CBUT_article_1__CBUT_category_1__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_1',
        categoryId: 'CBUT_category_1',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-07-25T03:32:50.190Z',
        updatedAt: '2020-07-25T03:32:50.190Z',
        comment: 'irrelevant',
      },
      '/articlecategoryfeedbacks/doc/CBUT_article_2__CBUT_category_1__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_2',
        categoryId: 'CBUT_category_1',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-07-25T03:31:47.629Z',
        updatedAt: '2020-07-25T03:31:47.629Z',
        comment: 'irrelevant',
      },
      '/articlecategoryfeedbacks/doc/CBUT_article_3__CBUT_category_1__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_3',
        categoryId: 'CBUT_category_1',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-07-25T03:35:36.459Z',
        updatedAt: '2020-07-25T03:35:36.459Z',
        comment: 'irrelevant',
      },
      '/articlecategoryfeedbacks/doc/CBUT_article_4__CBUT_category_1__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_4',
        categoryId: 'CBUT_category_1',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-07-25T04:14:22.278Z',
        updatedAt: '2020-07-25T04:14:22.278Z',
        comment: 'irrelevant',
      },
    },
    articlereplyfeedbacks: {
      '/articlereplyfeedbacks/doc/CBUT_article_10__CBUT_reply_5__qO1C3_lHfjAYn66nWNP7XCDrdY1gaZmJbQ1wYAgzOTT8xsTXc__DEVELOPMENT_BACKEND': {
        articleId: 'CBUT_article_10',
        replyId: 'CBUT_reply_5',
        userId: 'qO1C3_lHfjAYn66nWNP7XCDrdY1gaZmJbQ1wYAgzOTT8xsTXc',
        appId: 'DEVELOPMENT_BACKEND',
        createdAt: '2020-02-27T09:18:07.301Z',
        updatedAt: '2020-02-27T09:18:07.301Z',
        appUserId: 'CBUT_user_21',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_11__CBUT_reply_6__qO1C3_sYnv5g2wKCqsHcFZh-LocxMbYLn-fPAet_WiV0JRIDE__DEVELOPMENT_BACKEND': {
        createdAt: '2018-01-24T05:28:35.650Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_sYnv5g2wKCqsHcFZh-LocxMbYLn-fPAet_WiV0JRIDE',
        updatedAt: '2018-01-24T05:28:35.650Z',
        articleId: 'CBUT_article_11',
        replyId: 'CBUT_reply_6',
        appUserId: 'CBUT_user_22',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_12__CBUT_reply_7__CBUT_user_23__WEBSITE': {
        articleId: 'CBUT_article_12',
        replyId: 'CBUT_reply_7',
        userId: 'CBUT_user_23',
        appId: 'WEBSITE',
        createdAt: '2020-06-28T08:33:54.411Z',
        updatedAt: '2020-06-28T08:33:54.411Z',
        comment: '',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_13__CBUT_reply_8__j4S8C_LY7EIPDC671J6K2uZ9EznQAEDhznt8gUPsK0cwSDbFQ__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_13',
        replyId: 'CBUT_reply_8',
        userId: 'j4S8C_LY7EIPDC671J6K2uZ9EznQAEDhznt8gUPsK0cwSDbFQ',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2020-09-26T13:53:15.665Z',
        updatedAt: '2020-09-26T13:53:24.365Z',
        comment: '',
        appUserId: 'CBUT_user_24',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_14__CBUT_reply_9__qO1C3_gGWtOcj9ARqZRT4msCGaXS-Iof7eL3XxmT3AOTTGCVU__DEVELOPMENT_BACKEND': {
        articleId: 'CBUT_article_14',
        replyId: 'CBUT_reply_9',
        userId: 'qO1C3_gGWtOcj9ARqZRT4msCGaXS-Iof7eL3XxmT3AOTTGCVU',
        appId: 'DEVELOPMENT_BACKEND',
        createdAt: '2018-03-29T06:42:21.545Z',
        updatedAt: '2018-03-29T06:42:21.545Z',
        appUserId: 'CBUT_user_25',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_15__CBUT_reply_10__CBUT_user_15__DEVELOPMENT_FRONTEND': {
        createdAt: '2018-02-07T18:09:49.594Z',
        appId: 'DEVELOPMENT_FRONTEND',
        userId: 'CBUT_user_15',
        updatedAt: '2018-02-07T18:09:49.594Z',
        articleId: 'CBUT_article_15',
        replyId: 'CBUT_reply_10',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_16__CBUT_reply_11__j4S8C_-jyEGNy2w04UVgIb9-imCXweVYP3sniZ-q4Y8JNXL9k__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_16',
        replyId: 'CBUT_reply_11',
        userId: 'j4S8C_-jyEGNy2w04UVgIb9-imCXweVYP3sniZ-q4Y8JNXL9k',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2020-03-10T12:02:07.419Z',
        updatedAt: '2020-03-10T12:02:08.394Z',
        appUserId: 'CBUT_user_26',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_17__CBUT_reply_12__CBUT_user_15__WEBSITE': {
        articleId: 'CBUT_article_17',
        replyId: 'CBUT_reply_12',
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        createdAt: '2020-04-05T09:33:19.299Z',
        updatedAt: '2020-04-05T09:33:19.299Z',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_18__CBUT_reply_13__j4S8C_u7STzSnt-bwqVRc9tCmf47W2LE79MRzNrA-_oxzLIFc__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_18',
        replyId: 'CBUT_reply_13',
        userId: 'j4S8C_u7STzSnt-bwqVRc9tCmf47W2LE79MRzNrA-_oxzLIFc',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2019-02-20T05:11:06.611Z',
        updatedAt: '2019-02-20T05:11:06.611Z',
        appUserId: 'CBUT_user_27',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_19__CBUT_reply_14__CBUT_user_20__WEBSITE': {
        articleId: 'CBUT_article_19',
        replyId: 'CBUT_reply_14',
        userId: 'CBUT_user_20',
        appId: 'WEBSITE',
        createdAt: '2020-06-27T23:11:24.480Z',
        updatedAt: '2020-06-27T23:11:24.480Z',
        comment: '',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_20__CBUT_reply_15__CBUT_user_20__WEBSITE': {
        articleId: 'CBUT_article_20',
        replyId: 'CBUT_reply_15',
        userId: 'CBUT_user_20',
        appId: 'WEBSITE',
        createdAt: '2020-09-26T05:54:08.502Z',
        updatedAt: '2020-09-26T05:54:08.502Z',
        comment: '',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_21__CBUT_reply_16__j4S8C_O3LIyatOUXZjhZqeqQ5whc8glQZy6b-53QrsrO_SFbs__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_21',
        replyId: 'CBUT_reply_16',
        userId: 'j4S8C_O3LIyatOUXZjhZqeqQ5whc8glQZy6b-53QrsrO_SFbs',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2019-04-06T06:45:23.016Z',
        updatedAt: '2019-04-06T06:45:23.016Z',
        appUserId: 'CBUT_user_28',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_22__CBUT_reply_17__j4S8C_LD7dd370hd6B1O-rTkpX9I_n1AhIlTPD_QmDsGA6WoQ__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_22',
        replyId: 'CBUT_reply_17',
        userId: 'j4S8C_LD7dd370hd6B1O-rTkpX9I_n1AhIlTPD_QmDsGA6WoQ',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2018-11-11T23:43:02.406Z',
        updatedAt: '2018-11-11T23:43:02.406Z',
        appUserId: 'CBUT_user_29',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_23__CBUT_reply_18__j4S8C_J-qFf2nzWHvRnx3qA2hEhRhp4ASBnYRkq3IA2i7ngg8__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_23',
        replyId: 'CBUT_reply_18',
        userId: 'j4S8C_J-qFf2nzWHvRnx3qA2hEhRhp4ASBnYRkq3IA2i7ngg8',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2019-01-22T11:01:54.027Z',
        updatedAt: '2019-01-22T11:01:54.027Z',
        appUserId: 'CBUT_user_30',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_24__CBUT_reply_19__qO1C3_86RLd-liEHXfx4M_8H9J7Y0Wb13RIieRp5-2pFxb0Jo__DEVELOPMENT_BACKEND': {
        articleId: 'CBUT_article_24',
        replyId: 'CBUT_reply_19',
        userId: 'qO1C3_86RLd-liEHXfx4M_8H9J7Y0Wb13RIieRp5-2pFxb0Jo',
        appId: 'DEVELOPMENT_BACKEND',
        createdAt: '2019-11-20T22:23:54.285Z',
        updatedAt: '2019-11-20T22:23:54.285Z',
        appUserId: 'CBUT_user_31',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_25__CBUT_reply_20__j4S8C_ZvTJZB57GHkjobdIGZXOtibZ9PYEmQu8pnG-rYXS85A__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_25',
        replyId: 'CBUT_reply_20',
        userId: 'j4S8C_ZvTJZB57GHkjobdIGZXOtibZ9PYEmQu8pnG-rYXS85A',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2018-10-02T04:01:02.366Z',
        updatedAt: '2018-10-02T04:01:02.366Z',
        appUserId: 'CBUT_user_32',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_26__CBUT_reply_21__CBUT_user_16__DEVELOPMENT_FRONTEND': {
        createdAt: '2017-11-22T12:27:20.578Z',
        appId: 'DEVELOPMENT_FRONTEND',
        userId: 'CBUT_user_16',
        updatedAt: '2017-11-22T12:27:20.578Z',
        articleId: 'CBUT_article_26',
        replyId: 'CBUT_reply_21',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_27__CBUT_reply_22__qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo__DEVELOPMENT_BACKEND': {
        createdAt: '2017-08-11T16:55:48.227Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo',
        updatedAt: '2017-08-11T16:55:48.227Z',
        articleId: 'CBUT_article_27',
        replyId: 'CBUT_reply_22',
        appUserId: 'CBUT_user_17',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_28__CBUT_reply_23__qO1C3_wR_HXQWSJ2jY6OA3JRGZrIgVXadixeKTQsX1qxuHsKo__DEVELOPMENT_BACKEND': {
        createdAt: '2018-01-31T13:52:33.249Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_wR_HXQWSJ2jY6OA3JRGZrIgVXadixeKTQsX1qxuHsKo',
        updatedAt: '2018-01-31T13:52:33.249Z',
        articleId: 'CBUT_article_28',
        replyId: 'CBUT_reply_23',
        appUserId: 'CBUT_user_33',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_29__CBUT_reply_24__CBUT_user_23__WEBSITE': {
        articleId: 'CBUT_article_29',
        replyId: 'CBUT_reply_24',
        userId: 'CBUT_user_23',
        appId: 'WEBSITE',
        createdAt: '2020-09-19T17:04:20.167Z',
        updatedAt: '2020-09-19T17:04:20.167Z',
        comment: '',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_5__CBUT_reply_0__CBUT_user_16__DEVELOPMENT_FRONTEND': {
        createdAt: '2017-11-19T09:40:11.292Z',
        appId: 'DEVELOPMENT_FRONTEND',
        userId: 'CBUT_user_16',
        updatedAt: '2017-11-19T09:40:11.292Z',
        articleId: 'CBUT_article_5',
        replyId: 'CBUT_reply_0',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_6__CBUT_reply_1__qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo__DEVELOPMENT_BACKEND': {
        createdAt: '2018-01-11T08:29:22.045Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo',
        updatedAt: '2018-01-11T08:29:22.045Z',
        articleId: 'CBUT_article_6',
        replyId: 'CBUT_reply_1',
        appUserId: 'CBUT_user_17',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_7__CBUT_reply_2__j4S8C_Imj3cjSxE4ieAND7jCqxBcT1kYMCozfzPqI2CM2B4d4__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_7',
        replyId: 'CBUT_reply_2',
        userId: 'j4S8C_Imj3cjSxE4ieAND7jCqxBcT1kYMCozfzPqI2CM2B4d4',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2018-10-29T14:32:04.786Z',
        updatedAt: '2018-10-29T14:32:04.786Z',
        appUserId: 'CBUT_user_18',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_8__CBUT_reply_3__j4S8C_pwhC-K1IePUO5xsLr_3CDzSaH1yk7_OfUJoJeG3lVi4__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_8',
        replyId: 'CBUT_reply_3',
        userId: 'j4S8C_pwhC-K1IePUO5xsLr_3CDzSaH1yk7_OfUJoJeG3lVi4',
        appId: 'RUMORS_LINE_BOT',
        createdAt: '2019-06-25T02:42:31.105Z',
        updatedAt: '2019-06-25T02:42:31.636Z',
        appUserId: 'CBUT_user_19',
      },
      '/articlereplyfeedbacks/doc/CBUT_article_9__CBUT_reply_4__CBUT_user_20__WEBSITE': {
        articleId: 'CBUT_article_9',
        replyId: 'CBUT_reply_4',
        userId: 'CBUT_user_20',
        appId: 'WEBSITE',
        createdAt: '2020-09-10T11:45:55.227Z',
        updatedAt: '2020-09-10T11:45:55.227Z',
        comment: '',
      },
    },
    articles: {
      '/articles/doc/CBUT_article_0': {
        text:
          '轉貼\n\n中華民族崛起的主要敵人 AngloSaxon 民族\n\n兩百年來，鴉片戰爭開始，中國最主要的敵人就是英國，AngloSaxon 民族，現在隨著中國的崛起​...',
        createdAt: '2020-09-08T15:30:44.659Z',
        updatedAt: '2020-09-08T15:30:44.659Z',
        userId: 'j4S8C_Y1PaNolSaA4JExH8wY1ODwOCa4lNbbsPOtxJts305XU',
        appId: 'RUMORS_LINE_BOT',
        references: [
          {
            type: 'LINE',
            createdAt: '2020-09-08T15:30:44.659Z',
            userId: 'j4S8C_Y1PaNolSaA4JExH8wY1ODwOCa4lNbbsPOtxJts305XU',
            appId: 'RUMORS_LINE_BOT',
            appUserId: 'CBUT_user_87',
          },
        ],
        articleReplies: [],
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-09-12T07:19:46.285Z',
            positiveFeedbackCount: 1,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bow',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_0',
            status: 'NORMAL',
            updatedAt: '2020-09-12T07:19:46.285Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        normalArticleReplyCount: 0,
        normalArticleCategoryCount: 1,
        replyRequestCount: 2,
        tags: [],
        hyperlinks: [],
        lastRequestedAt: '2020-09-17T12:44:47.688Z',
        appUserId: 'CBUT_user_87',
      },
      '/articles/doc/CBUT_article_1': {
        references: [
          {
            createdAt: '2020-04-06T04:35:45.919Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_n66M1oaUXCwx_g7G3O5VFjHWL4dbZqq9aiEewD4s8-g',
            appUserId: 'CBUT_user_62',
          },
        ],
        userId: 'j4S8C_n66M1oaUXCwx_g7G3O5VFjHWL4dbZqq9aiEewD4s8-g',
        tags: [],
        createdAt: '2020-04-06T04:35:45.919Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-06T07:42:40.317Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 45,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_50',
            userId: 'CBUT_user_63',
            status: 'NORMAL',
            updatedAt: '2020-04-06T07:42:40.317Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 1,
            createdAt: '2020-05-21T09:08:36.433Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_1',
            status: 'NORMAL',
            updatedAt: '2020-05-21T09:08:36.433Z',
            appUserId: 'CBUT_user_36',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-07-25T03:32:27.942Z',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            userId: 'CBUT_user_15',
            categoryId: 'CBUT_category_10',
            status: 'NORMAL',
            updatedAt: '2020-07-25T03:32:27.942Z',
          },
        ],
        replyRequestCount: 15,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-04-06T07:42:18.890Z',
        text:
          '你有沒有發現今天的記者會、陳時中等所有人都戴上了口罩，這認為意味著什麼？\n\n現在無症狀卻可能有傳染力的隱性感染者，是最讓人驚恐又難以防範的\n真的不要挑戰自己的運...',
        hyperlinks: [],
        updatedAt: '2020-04-06T04:35:45.919Z',
        normalArticleCategoryCount: 2,
        appUserId: 'CBUT_user_62',
      },
      '/articles/doc/CBUT_article_10': {
        references: [
          {
            createdAt: '2020-02-27T03:27:49.869Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_4WVN4mcFb9V8KKu6LT8xXvRud2LWcSa-UjL_qvy0ksw',
            appUserId: 'CBUT_user_76',
          },
        ],
        userId: 'j4S8C_4WVN4mcFb9V8KKu6LT8xXvRud2LWcSa-UjL_qvy0ksw',
        tags: [],
        createdAt: '2020-02-27T03:27:49.869Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-02-27T09:11:33.769Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 4,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_5',
            userId: 'CBUT_user_68',
            status: 'NORMAL',
            updatedAt: '2020-02-27T09:11:33.769Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-24T02:14:20.097Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_0',
            status: 'NORMAL',
            updatedAt: '2020-05-24T02:14:20.097Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 2,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-02-27T07:50:54.261Z',
        text:
          '\n什麼是「中國腳鐐」呢？\n為何全世界會願意套上這個“中國腳鐐”呢？\n一個簡單的答案就是：沒有更好的選擇。\n※※※※※※\n數字是這樣告訴我們的：台灣出口最大的國家...',
        hyperlinks: [],
        updatedAt: '2020-02-27T03:27:49.869Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_76',
      },
      '/articles/doc/CBUT_article_11': {
        references: [
          {
            createdAt: '2017-12-30T01:01:44.438Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_v8peSS0932LsPFMsF61kNZqh7-ZkQ_2QHV22_C3PXf4',
        tags: [],
        createdAt: '2017-12-30T01:01:44.438Z',
        articleReplies: [
          {
            createdAt: '2018-01-01T12:00:04.040Z',
            negativeFeedbackCount: 4,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 10,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_73',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2018-01-01T12:00:04.040Z',
          },
          {
            createdAt: '2018-01-23T15:42:02.682Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 35,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_6',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2018-01-23T15:42:02.682Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T23:26:42.995Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-19T23:26:42.995Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 3,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-12-31T04:18:42.495Z',
        text:
          '舊金山市長李孟賢（Edwin M. Lee）已於12 日凌晨1時11分因心臟病發於舊金山總醫院過世，享年65歲。\n請花幾分鐘閱讀此文：\n\n1. 假設現在是晚上7...',
        hyperlinks: [],
        updatedAt: '2017-12-30T01:01:44.438Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_86',
      },
      '/articles/doc/CBUT_article_12': {
        references: [
          {
            createdAt: '2018-03-30T12:13:57.322Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_4PNVISSWISKdcDP111lIO_cSmpR1jMjSyzDwLoYDvH4',
            appUserId: 'CBUT_user_80',
          },
        ],
        userId: 'j4S8C_4PNVISSWISKdcDP111lIO_cSmpR1jMjSyzDwLoYDvH4',
        tags: [],
        createdAt: '2018-03-30T12:13:57.322Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-03-30T14:19:54.465Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_7',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2018-03-30T14:19:54.465Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T14:05:43.227Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_3',
            status: 'NORMAL',
            updatedAt: '2020-05-19T14:05:43.227Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-03-30T12:13:57.363Z',
        text: '民眾檢舉交通罰單籲7日不用繳費',
        normalArticleReplycount: 0,
        updatedAt: '2018-03-30T12:13:57.322Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_80',
      },
      '/articles/doc/CBUT_article_13': {
        references: [
          {
            createdAt: '2017-01-13T03:49:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-13T03:49:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-07-07T09:39:54.031Z',
            negativeFeedbackCount: 5,
            replyType: 'RUMOR',
            positiveFeedbackCount: 24,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_8',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-07-07T09:39:54.031Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T13:46:40.190Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T13:46:40.190Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 3,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-13T03:49:00.000Z',
        text:
          '太白粉擺上一百年也不長蟲，你相信嗎? \n\n肉羹麵、蚵仔麵線、酸辣湯…勾芡的東西容易引起痛風及糖尿病之慢性病 。 \n喜歡吃燴飯肉羹湯的人請注意！ \n\n烹飪老師是西...',
        hyperlinks: [],
        updatedAt: '2017-01-13T03:49:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_14': {
        references: [
          {
            createdAt: '2017-07-10T11:37:00.087Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_4iGkmTeduiI5SrYlHehwmaS3SX4Nszoyb1oyUWvLMEo',
        tags: [],
        createdAt: '2017-07-10T11:37:00.087Z',
        articleReplies: [
          {
            createdAt: '2017-07-10T14:01:55.451Z',
            negativeFeedbackCount: 2,
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_69',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2017-07-10T14:01:55.451Z',
          },
          {
            negativeFeedbackCount: 1,
            createdAt: '2018-03-11T02:45:59.130Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 5,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_9',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2018-03-11T02:45:59.130Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T05:14:28.507Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-05-21T05:14:28.507Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-07-10T11:37:00.105Z',
        text:
          '轉載：\n以下是一位在臺灣出生在台灣成長的純正台湾人轉來的。\n\n什麼叫「兩蔣時代」？以下就是：\n\n1，是治安良好的時代。\n2，是沒有民粹的時代。\n3，是禮義廉恥、...',
        hyperlinks: [],
        updatedAt: '2017-07-10T11:37:00.087Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_82',
      },
      '/articles/doc/CBUT_article_15': {
        references: [
          {
            createdAt: '2018-01-07T16:35:06.585Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_pGTVQioRDsflknEcedoVAbKom5K10ipYSV4rWSq2G_4',
        tags: [],
        createdAt: '2018-01-07T16:35:06.585Z',
        articleReplies: [
          {
            createdAt: '2018-02-07T17:33:53.027Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_10',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2018-02-07T17:33:53.027Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T22:39:16.144Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_7',
            status: 'NORMAL',
            updatedAt: '2020-05-19T22:39:16.144Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-01-07T16:35:06.599Z',
        text:
          'http://health.businessweekly.com.tw/AArticleM.aspx?ID=ARTL000065544&p=2',
        hyperlinks: [
          {
            summary:
              '7.「支氣管擴張劑」：有些孩子的咳嗽，是因為病毒感染之後呼吸道過度反應而造成，如果發現孩子咳嗽超過一個星期以上，可以在看病時，跟醫師討論看看是否需要使用「支氣管...',
            title:
              '孩子半夜咳不停？問題可能在冷氣！小兒科醫師建議的冷氣溫度這樣調',
            url:
              'http://health.businessweekly.com.tw/AArticleM.aspx?ID=ARTL000065544&p=2',
          },
        ],
        updatedAt: '2018-01-07T16:35:06.585Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_41',
      },
      '/articles/doc/CBUT_article_16': {
        references: [
          {
            createdAt: '2020-03-03T15:37:27.556Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_RW_vDd8lKq4WdfWYxmVh6sJImYM96kix8X4A6FhRJHI',
            appUserId: 'CBUT_user_55',
          },
        ],
        userId: 'j4S8C_RW_vDd8lKq4WdfWYxmVh6sJImYM96kix8X4A6FhRJHI',
        tags: [],
        createdAt: '2020-03-03T15:37:27.556Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-03-04T03:39:58.171Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 60,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_11',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2020-03-04T03:39:58.171Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T04:06:44.784Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-22T04:06:44.784Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 7,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-03-04T03:08:52.875Z',
        text:
          '各位同事，您可能還記得，當我是加利福尼亞大學聖地亞哥分校的病理學教授時，我是世界上最早研究冠狀病毒的分子病毒學家之一（1970年代）。我是第一個證明該病毒包含的...',
        hyperlinks: [],
        updatedAt: '2020-03-03T15:37:27.556Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_55',
      },
      '/articles/doc/CBUT_article_17': {
        references: [
          {
            createdAt: '2020-04-03T01:27:41.969Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_sJtyYDWJm0yBONBKJix3-poQzSELnS-Xyr7WPZdwvvA',
            appUserId: 'CBUT_user_75',
          },
        ],
        userId: 'j4S8C_sJtyYDWJm0yBONBKJix3-poQzSELnS-Xyr7WPZdwvvA',
        tags: [],
        createdAt: '2020-04-03T01:27:41.969Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-03T04:24:14.814Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_12',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2020-04-03T04:24:14.814Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-08T14:05:42.107Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_62',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2020-04-08T14:05:42.107Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-24T01:29:04.833Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-24T01:29:04.833Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-04-03T01:27:41.997Z',
        text:
          '🇮🇹這就是-義大利🇮🇹\n一對*約定今年完婚*的\n情侣.女友感染新冠病毒在醫院搶救.男友探病時断然脱掉防護服⋯他們在3/25日-相繼往生！\n😭看得讓人腸斷心碎😭  ',
        hyperlinks: [],
        updatedAt: '2020-04-03T01:27:41.969Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_75',
      },
      '/articles/doc/CBUT_article_18': {
        references: [
          {
            createdAt: '2017-01-22T04:05:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-22T04:05:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-04-12T11:21:23.420Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 11,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_72',
            userId: 'CBUT_user_85',
            status: 'NORMAL',
            updatedAt: '2017-04-12T11:21:23.420Z',
          },
          {
            createdAt: '2017-04-12T11:25:01.494Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 48,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_13',
            userId: 'CBUT_user_85',
            status: 'NORMAL',
            updatedAt: '2017-04-12T11:25:01.494Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T11:37:33.109Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T11:37:33.109Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 12,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-04-08T08:27:32.840Z',
        text:
          '日本醫學博士說:\n\n防失智, 增強記憶力, 不用吃藥, 可以用食療, 他推薦要吃4種食物 :\n\n1, 番茄煮熟防失智。\n2, 蘋果增強記憶力。\n3, 黑巧克力活...',
        hyperlinks: [],
        updatedAt: '2017-01-22T04:05:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_19': {
        references: [
          {
            createdAt: '2019-11-10T06:18:52.830Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_VSYVIlqF4l_puOdN_Pzu64YnjMc-nM5iu0OWcWTt4xk',
            appUserId: 'CBUT_user_78',
          },
        ],
        userId: 'j4S8C_VSYVIlqF4l_puOdN_Pzu64YnjMc-nM5iu0OWcWTt4xk',
        tags: [],
        createdAt: '2019-11-10T06:18:52.830Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-11-18T07:31:32.991Z',
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_65',
            userId: 'CBUT_user_79',
            status: 'NORMAL',
            updatedAt: '2019-11-18T07:31:32.991Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-11-18T13:31:29.671Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_14',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-11-18T13:31:29.671Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T20:41:58.708Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T20:41:58.708Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2019-11-10T06:18:52.860Z',
        text:
          '电視新聞報導医生檢查女記者的肺部，發現常使用手機的人，肺部迅速老化，那位女記者才二十多歲，肺部老化，相當四十多歲的人。好恐怖!三C產品盡量少用。',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-11-10T06:18:52.830Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_78',
      },
      '/articles/doc/CBUT_article_2': {
        references: [
          {
            createdAt: '2020-04-17T13:11:33.426Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_xhN2O5Ouv6Y6AbEoDULy9Crc1TRdEaQ03ANLtAEJWnY',
            appUserId: 'CBUT_user_59',
          },
        ],
        userId: 'j4S8C_xhN2O5Ouv6Y6AbEoDULy9Crc1TRdEaQ03ANLtAEJWnY',
        tags: [],
        createdAt: '2020-04-17T13:11:33.426Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-28T13:51:19.455Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_48',
            userId: 'CBUT_user_60',
            status: 'NORMAL',
            updatedAt: '2020-04-28T13:51:19.455Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-05T09:41:19.619Z',
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_49',
            userId: 'CBUT_user_61',
            status: 'NORMAL',
            updatedAt: '2020-05-05T09:41:19.619Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 1,
            createdAt: '2020-05-23T16:12:16.897Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_1',
            status: 'NORMAL',
            updatedAt: '2020-05-23T16:12:16.897Z',
            appUserId: 'CBUT_user_36',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-07-25T03:31:27.224Z',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            userId: 'CBUT_user_15',
            categoryId: 'CBUT_category_10',
            status: 'NORMAL',
            updatedAt: '2020-07-25T03:31:27.224Z',
          },
        ],
        replyRequestCount: 3,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-04-25T11:34:30.749Z',
        text:
          '13:23 金克寧 分享轉傳：\n\n兩位地方公共衛生局局長：Grant Colfax和Sara Cody，分別給他們取個中文名字吧：格蘭特和薩拉。\n\n格蘭特是舊金...',
        hyperlinks: [],
        updatedAt: '2020-04-17T13:11:33.426Z',
        normalArticleCategoryCount: 2,
        appUserId: 'CBUT_user_59',
      },
      '/articles/doc/CBUT_article_20': {
        references: [
          {
            createdAt: '2018-01-26T02:28:32.022Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_MKBfQLupvhw4_Pf0ua4s1ag2aNnC7lr88z8Hyx7C31g',
        tags: [],
        createdAt: '2018-01-26T02:28:32.022Z',
        articleReplies: [
          {
            createdAt: '2018-01-26T03:24:02.724Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_15',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2018-01-26T03:24:02.724Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T20:49:05.228Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_11',
            status: 'NORMAL',
            updatedAt: '2020-05-19T20:49:05.228Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-01-26T02:28:32.033Z',
        text:
          '不能只有我拿到！現在來家樂福滴血認親"完成會員認證"，免費拿娘家好禮，完成再抽會員點數一千萬點！\r\nhttps://goo.gl/sNbJrY',
        hyperlinks: [
          {
            summary: null,
            title: null,
            url: 'https://goo.gl/sNbJrY',
          },
        ],
        updatedAt: '2018-01-26T02:28:32.022Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_89',
      },
      '/articles/doc/CBUT_article_21': {
        references: [
          {
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_nEkrBWbE4FfkBTIV9QMn1exDEGReMdemzfznDgx6nA8',
        tags: [],
        createdAt: '2017-03-13T14:12:52.385Z',
        articleReplies: [
          {
            createdAt: '2017-03-21T16:25:34.320Z',
            negativeFeedbackCount: 12,
            replyType: 'RUMOR',
            positiveFeedbackCount: 307,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_16',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-03-21T16:25:34.320Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-08-14T05:27:33.035Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 13,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_57',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2019-08-14T05:27:33.035Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T13:04:27.210Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-20T13:04:27.210Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 27,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-03-21T15:32:51.522Z',
        text:
          '各位朋友請注意，\n\n即日起不可食用泰國製之各類罐頭食品，包含臺灣品牌泰國生産者，尤其是泰國罐裝水果食品。\n泰國200多名愛滋病感染者，在患者的首腦指揮下，湧進泰...',
        hyperlinks: [],
        updatedAt: '2017-03-13T14:12:52.385Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_70',
      },
      '/articles/doc/CBUT_article_22': {
        references: [
          {
            createdAt: '2017-01-16T14:55:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-16T14:55:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-03-27T14:21:52.625Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 10,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_32',
            userId: 'CBUT_user_35',
            status: 'DELETED',
            updatedAt: '2018-11-03T16:59:09.975Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-11-03T16:14:49.213Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_33',
            userId: 'CBUT_user_15',
            status: 'DELETED',
            updatedAt: '2018-11-03T16:17:11.162Z',
          },
          {
            negativeFeedbackCount: 2,
            createdAt: '2018-11-03T16:17:08.188Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 145,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_34',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2018-11-03T16:17:08.188Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-11-07T06:06:02.530Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_35',
            userId: 'CBUT_user_47',
            status: 'DELETED',
            updatedAt: '2018-11-07T06:08:54.149Z',
          },
          {
            negativeFeedbackCount: 1,
            createdAt: '2018-11-07T06:17:00.482Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 22,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_17',
            userId: 'CBUT_user_47',
            status: 'NORMAL',
            updatedAt: '2018-11-07T06:17:00.482Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-11-15T13:54:13.264Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 36,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_36',
            userId: 'CBUT_user_48',
            status: 'NORMAL',
            updatedAt: '2018-11-15T13:54:13.264Z',
          },
        ],
        normalArticleReplyCount: 3,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T12:28:44.104Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_9',
            status: 'NORMAL',
            updatedAt: '2020-05-21T12:28:44.104Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 7,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-16T14:55:00.000Z',
        text:
          '昨天ㄧ位媽媽去地檢署按鈴申告，很多家長去聲援，你看看影片， 她小六大兒子被強迫跟另外一個女生練習自慰， 她抗議給小孩上一些亂七八糟的性解放教育，被霸凌，這會不會...',
        hyperlinks: [],
        updatedAt: '2017-01-16T14:55:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_23': {
        references: [
          {
            createdAt: '2018-12-28T13:18:06.346Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_9V5d6E7GKiGNn1Yv_ifdq_WCxK4GET6BedItZHNN3W0',
            appUserId: 'CBUT_user_72',
          },
        ],
        userId: 'j4S8C_9V5d6E7GKiGNn1Yv_ifdq_WCxK4GET6BedItZHNN3W0',
        tags: [],
        createdAt: '2018-12-28T13:18:06.346Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-12-28T13:27:11.334Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 124,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_18',
            userId: 'CBUT_user_73',
            status: 'NORMAL',
            updatedAt: '2018-12-28T13:27:11.334Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-07-02T04:09:52.096Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 4,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_60',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2019-07-02T04:09:52.096Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-07-23T08:06:47.683Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 15,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_61',
            userId: 'CBUT_user_74',
            status: 'NORMAL',
            updatedAt: '2019-07-23T08:06:47.683Z',
          },
        ],
        normalArticleReplyCount: 3,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T15:00:40.117Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-22T15:00:40.117Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-12-28T13:18:06.372Z',
        text:
          '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效!：https://youtu.be/EI_oExo38fA',
        normalArticleReplycount: 0,
        hyperlinks: [
          {
            summary:
              '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效! 值得收藏！在一次煮玉米粥時，我用叉子在沸騰的粥裡翻攪，想看看粥好了沒有。\n由於想問題，...',
            title:
              '奇蹟！燙到後趕快用它！連紅印和水泡都不會有！每家廚房都有，比沖水更有效!',
            url: 'https://youtu.be/EI_oExo38fA',
          },
        ],
        updatedAt: '2018-12-28T13:18:06.346Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_72',
      },
      '/articles/doc/CBUT_article_24': {
        references: [
          {
            createdAt: '2019-11-13T16:17:15.137Z',
            appId: 'WEBSITE',
            type: 'URL',
            permalink: '',
            userId: 'CBUT_user_35',
          },
        ],
        userId: 'CBUT_user_35',
        tags: [],
        createdAt: '2019-11-13T16:17:15.137Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-11-13T16:18:32.732Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_19',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-11-13T16:18:32.732Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T18:28:42.079Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-05-22T18:28:42.079Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'WEBSITE',
        lastRequestedAt: '2019-11-13T16:17:15.156Z',
        text:
          'My experience with burns is this:\nOnce I was cooking some corn and stuck my fork...',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-11-13T16:17:15.137Z',
        normalArticleCategoryCount: 1,
      },
      '/articles/doc/CBUT_article_25': {
        references: [
          {
            createdAt: '2017-01-16T12:47:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-16T12:47:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-01-09T15:19:00.000Z',
            negativeFeedbackCount: 7,
            replyType: 'RUMOR',
            positiveFeedbackCount: 27,
            appId: 'BOT_LEGACY',
            replyId: 'CBUT_reply_37',
            userId: 'Z0k1e_9jAgBIhrjzs66_4pQrS0Gq4m79N5KCuHY9q8U1xUjYw',
            status: 'DELETED',
            updatedAt: '2017-10-15T16:05:15.495Z',
            appUserId: 'CBUT_user_35',
          },
          {
            createdAt: '2017-10-15T16:05:28.186Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 53,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_38',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-10-15T16:05:28.186Z',
          },
          {
            negativeFeedbackCount: 5,
            createdAt: '2018-10-01T16:02:38.679Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 185,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_20',
            userId: 'CBUT_user_49',
            status: 'NORMAL',
            updatedAt: '2018-10-01T16:02:38.679Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T12:30:16.948Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_3',
            status: 'NORMAL',
            updatedAt: '2020-05-21T12:30:16.948Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 2,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-16T12:47:00.000Z',
        text:
          '今晚開始就實施擴大臨檢\n請各位朋友千萬不要酒後駕車\n警察局擴大臨檢日期：\n10/13：19~23時\n10/16：20~24時\n10/20：21~01時\n10/2...',
        hyperlinks: [],
        updatedAt: '2017-01-16T12:47:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_26': {
        references: [
          {
            createdAt: '2017-11-19T09:35:45.073Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_uURhRzjSSV6s0wz7Patvy8Fb2hBM46AgZeTSyZ0OUfk',
        tags: [],
        createdAt: '2017-11-19T09:35:45.073Z',
        articleReplies: [
          {
            createdAt: '2017-11-19T10:08:18.039Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_21',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-11-19T10:08:18.039Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T14:26:26.065Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-20T14:26:26.065Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-11-19T09:35:45.084Z',
        text: '肺炎疫苗接種，終身受用',
        hyperlinks: [],
        updatedAt: '2017-11-19T09:35:45.073Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_43',
      },
      '/articles/doc/CBUT_article_27': {
        references: [
          {
            createdAt: '2017-06-24T02:25:38.653Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_YWVSKKajD6USiTghIEQzrWT_YKdnVVqVI96-cyS6zWA',
        tags: [],
        createdAt: '2017-06-24T02:25:38.653Z',
        articleReplies: [
          {
            createdAt: '2017-06-24T07:35:54.959Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_68',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2017-06-24T07:35:54.959Z',
          },
          {
            createdAt: '2017-06-27T15:25:17.276Z',
            negativeFeedbackCount: 1,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_22',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2017-06-27T15:25:17.276Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T03:38:53.843Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_7',
            status: 'NORMAL',
            updatedAt: '2020-05-20T03:38:53.843Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-06-24T02:25:38.668Z',
        text:
          '連署增設公幼\n\nhttp://join.gov.tw/idea/detail/6a024fc8-ceb3-45cc-a37a-59cbec2d6bd4\n各位夥伴...',
        hyperlinks: [
          {
            summary:
              '提議內容或建議事項\n\t\t\t\t台灣少子化已是全球第一名，目前總生育率約1.175%，日本為1.4%、韓國1.5%，港澳及新加坡為1.2%。目前全國公幼2465園，...',
            title:
              '公共化幼兒園應在4年內達到1000班之政策目標- 提點子-公共政策網路參與平臺',
            url:
              'http://join.gov.tw/idea/detail/6a024fc8-ceb3-45cc-a37a-59cbec2d6bd4',
          },
        ],
        updatedAt: '2017-06-24T02:25:38.653Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_81',
      },
      '/articles/doc/CBUT_article_28': {
        references: [
          {
            createdAt: '2017-08-29T03:38:35.524Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_igegSRiBLdZh9argcdWuKn3cZzYGaSCes5WLJr-9l2s',
        tags: [],
        createdAt: '2017-08-29T03:38:35.524Z',
        articleReplies: [
          {
            createdAt: '2017-08-29T04:03:40.183Z',
            negativeFeedbackCount: 4,
            replyType: 'RUMOR',
            positiveFeedbackCount: 156,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_23',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-08-29T04:03:40.183Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-12-12T04:33:27.040Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 76,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_46',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2018-12-12T04:33:27.040Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-08-28T02:48:00.678Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 5,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_47',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2019-08-28T02:48:00.678Z',
          },
        ],
        normalArticleReplyCount: 3,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T19:08:49.495Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-20T19:08:49.495Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-08-29T03:38:35.539Z',
        text:
          '台北馬偕醫院傳來訊息：\n\n資深醫生（林清風）傳來：「請大家以後千萬不要再吃生魚片了！」\n因為最近已經發現- 好多病人因為吃了生魚片，胃壁附著《海獸胃腺蟲》，大小...',
        hyperlinks: [],
        updatedAt: '2017-08-29T03:38:35.524Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_58',
      },
      '/articles/doc/CBUT_article_29': {
        references: [
          {
            createdAt: '2019-08-15T02:39:39.532Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_Y1PaNolSaA4JExH8wY1ODwOCa4lNbbsPOtxJts305XU',
            appUserId: 'CBUT_user_87',
          },
        ],
        userId: 'j4S8C_Y1PaNolSaA4JExH8wY1ODwOCa4lNbbsPOtxJts305XU',
        tags: [],
        createdAt: '2019-08-15T02:39:39.532Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-08-21T02:10:43.994Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_24',
            userId: 'CBUT_user_88',
            status: 'NORMAL',
            updatedAt: '2019-08-21T02:10:43.994Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T05:13:17.490Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_0',
            status: 'NORMAL',
            updatedAt: '2020-05-22T05:13:17.490Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2019-08-15T02:39:39.597Z',
        text:
          '運動手錶跟手機互聯，當然手機內所有資料都會進入他們的伺服器（任何品牌都如此）唯獨中國只要發送「詆譭攻擊等圖文」就會鎖定情搜追蹤你。以保護自己及國安角度，供大家參考。',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-08-15T02:39:39.532Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_87',
      },
      '/articles/doc/CBUT_article_3': {
        references: [
          {
            createdAt: '2019-12-24T02:03:16.857Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_nRWu8ne4K6M1m6ibF8HbOq07hUxK3pvrOD4g7cn-okY',
            appUserId: 'CBUT_user_64',
          },
        ],
        userId: 'j4S8C_nRWu8ne4K6M1m6ibF8HbOq07hUxK3pvrOD4g7cn-okY',
        tags: [],
        createdAt: '2019-12-24T02:03:16.857Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-12-24T02:41:34.902Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_51',
            userId: 'CBUT_user_65',
            status: 'DELETED',
            updatedAt: '2019-12-24T02:45:39.069Z',
          },
          {
            negativeFeedbackCount: 1,
            createdAt: '2019-12-24T02:55:42.332Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 44,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_52',
            userId: 'CBUT_user_65',
            status: 'NORMAL',
            updatedAt: '2019-12-24T02:55:42.332Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-12-25T16:46:47.886Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 9,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_53',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2019-12-25T16:46:47.886Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 1,
            createdAt: '2020-05-23T15:39:19.038Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_1',
            status: 'NORMAL',
            updatedAt: '2020-05-23T15:39:19.038Z',
            appUserId: 'CBUT_user_36',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-07-25T03:36:19.070Z',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            userId: 'CBUT_user_15',
            categoryId: 'CBUT_category_4',
            status: 'NORMAL',
            updatedAt: '2020-07-25T03:36:19.070Z',
          },
        ],
        replyRequestCount: 8,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2019-12-24T02:55:11.261Z',
        text:
          '民進黨已經通過了毒品條例了\n\n校園內可以擁有50克的毒品，才被視同販毒。\n\n台灣除了是(同性戀島)\n(愛滋病島)現在又增加了一個(毒品島)了\n校園毒品零容忍遭封...',
        normalArticleReplycount: 0,
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.chinatimes.com/realtimenews/20191217002605-260407',
            summary:
              '立法委員沈智慧在立法院提案，「毒品危害防制條例第四條、第六條及第三十一條之一條文修正草案」，沈智慧曾兩度上台發言，拜託民進黨支持，能讓法案通過，但結果還是被封殺...',
            title: '校園毒品零容忍遭封殺 藍委提嚴刑峻法被綠委阻攔',
            url:
              'https://www.chinatimes.com/realtimenews/20191217002605-260407',
          },
        ],
        updatedAt: '2019-12-24T02:03:16.857Z',
        normalArticleCategoryCount: 2,
        appUserId: 'CBUT_user_64',
      },
      '/articles/doc/CBUT_article_30': {
        references: [
          {
            createdAt: '2019-03-18T15:31:50.099Z',
            appId: 'DEVELOPMENT_BACKEND',
            type: 'URL',
            userId: 'qO1C3_lAgNK9-guL13qEJSGROTsi_UZbF5N5WoQ17vnsOrl6s',
            appUserId: 'CBUT_user_34',
          },
        ],
        userId: 'qO1C3_lAgNK9-guL13qEJSGROTsi_UZbF5N5WoQ17vnsOrl6s',
        tags: [],
        createdAt: '2019-03-18T15:31:50.099Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-03-19T14:57:01.116Z',
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_25',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-03-19T14:57:01.116Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-23T07:28:16.667Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-05-23T07:28:16.667Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'DEVELOPMENT_BACKEND',
        lastRequestedAt: '2019-03-18T15:31:50.146Z',
        text: 'Newsgeist is a horrible event. People are doing nothing here.',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-03-18T15:31:50.099Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_34',
      },
      '/articles/doc/CBUT_article_31': {
        references: [
          {
            createdAt: '2017-01-20T00:58:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-20T00:58:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-09-13T13:44:48.950Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_26',
            userId: 'CBUT_user_38',
            status: 'NORMAL',
            updatedAt: '2017-09-13T13:44:48.950Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T11:42:10.824Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_3',
            status: 'NORMAL',
            updatedAt: '2020-05-21T11:42:10.824Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-20T00:58:00.000Z',
        text:
          '南投新景點～富寮里動物園，佔地18公頃，內有狐獴、侏儒山羊、兔子、鸚鵡、天鵝、紅環、灰冠鶴、驢、騾、小馬、羊駝.......等，1百多種動物(持續增加ing)，...',
        hyperlinks: [
          {
            summary:
              '部落格關閉中\n                                    \n                                \n   ...',
            title: '痞客邦 訊息提示',
            url: 'http://fbuon2881.pixnet.net/blog/post/220868495',
          },
        ],
        updatedAt: '2017-01-20T00:58:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_32': {
        references: [
          {
            createdAt: '2018-06-15T07:34:36.490Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_f0pGq97e0_aH_2-8x0BLaj36xzuu2M1W54ZVcAZ4fM0',
            appUserId: 'CBUT_user_39',
          },
        ],
        userId: 'j4S8C_f0pGq97e0_aH_2-8x0BLaj36xzuu2M1W54ZVcAZ4fM0',
        tags: [],
        createdAt: '2018-06-15T07:34:36.490Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-07-10T16:17:54.867Z',
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_27',
            userId: 'CBUT_user_40',
            status: 'NORMAL',
            updatedAt: '2018-07-10T16:17:54.867Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-23T04:34:30.811Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_4',
            status: 'NORMAL',
            updatedAt: '2020-05-23T04:34:30.811Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-06-15T07:34:36.502Z',
        text:
          '讚！國民黨有史以來打民進黨蔡英文最好的一支廣告.既傳神又生動，大家努力的傳出去https://m.youtube.com/watch?v=VVHf9ni09m4&feature=youtu.be',
        normalArticleReplycount: 0,
        hyperlinks: [
          {
            summary:
              'https://www.facebook.com/events/217719084982022/ 請多多邀請橘綠藍朋友加入或共同參加主辦人行列,要求""媒體自重...',
            title:
              '國民黨馬英九有史以來打民進黨蔡英文最好的一支廣告.既傳神又生動',
            url: 'https://m.youtube.com/watch?v=VVHf9ni09m4&feature=youtu.be',
          },
        ],
        updatedAt: '2018-06-15T07:34:36.490Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_39',
      },
      '/articles/doc/CBUT_article_33': {
        references: [
          {
            createdAt: '2019-05-16T15:09:42.254Z',
            appId: 'WEBSITE',
            type: 'URL',
            permalink:
              'https://www.facebook.com/jnliao/videos/2701707013234650/',
            userId: 'CBUT_user_15',
          },
        ],
        userId: 'CBUT_user_15',
        tags: [],
        createdAt: '2019-05-16T15:09:42.254Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-05-16T15:11:30.728Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_28',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2019-05-16T15:11:30.728Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T23:59:04.380Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_5',
            status: 'NORMAL',
            updatedAt: '2020-05-21T23:59:04.380Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'WEBSITE',
        lastRequestedAt: '2019-05-16T15:09:42.281Z',
        text: '可惡至極的政府亂搞教育政策！這可以秀出來教嗎？這要禍害下一代！',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-05-16T15:09:42.254Z',
        normalArticleCategoryCount: 1,
      },
      '/articles/doc/CBUT_article_34': {
        references: [
          {
            createdAt: '2017-01-18T02:58:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-18T02:58:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-06-12T11:50:12.540Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_29',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-06-12T11:50:12.540Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T12:03:06.153Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_6',
            status: 'NORMAL',
            updatedAt: '2020-05-21T12:03:06.153Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-18T02:58:00.000Z',
        text:
          'l  請大家注意！  l\n\n中壢地區的泰豐輪胎廠方失火，燒出來的煙都是戴奧辛，它非常非常毒，請大家及小朋友們出門可以的話戴口罩，下雨的話也不要淋雨，以上請各位長...',
        hyperlinks: [],
        updatedAt: '2017-01-18T02:58:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_35': {
        references: [
          {
            createdAt: '2017-12-24T23:48:52.794Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_FQn8c8K4i67IISzvXVoOPipzRASqSgeBxPLNjReHVyY',
        tags: [],
        createdAt: '2017-12-24T23:48:52.794Z',
        articleReplies: [
          {
            createdAt: '2017-12-27T14:55:54.650Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_30',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-12-27T14:55:54.650Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T00:00:47.748Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_7',
            status: 'NORMAL',
            updatedAt: '2020-05-20T00:00:47.748Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-12-24T23:48:52.805Z',
        text:
          '支持手遊 www.yxc8.cn 安全服務交易平臺上來交易！瀏覽器打開網頁先上去註冊個屬於自己的帳戶，然後我去掛單你需要的價格的貨物，使用點數卡在線充值選擇貨物完成下單。',
        hyperlinks: [
          {
            summary: null,
            title: null,
            url: 'www.yxc8.cn',
          },
        ],
        updatedAt: '2017-12-24T23:48:52.794Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_42',
      },
      '/articles/doc/CBUT_article_36': {
        references: [
          {
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_pYR1NvuKvLQfho6HCkJWbMqLYFnDzoATlM4SyqsxdEU',
        tags: [],
        createdAt: '2017-03-05T00:13:37.379Z',
        articleReplies: [
          {
            createdAt: '2017-06-19T14:54:52.147Z',
            negativeFeedbackCount: 0,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 28,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_31',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2017-06-19T14:54:52.147Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T13:41:30.327Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_7',
            status: 'NORMAL',
            updatedAt: '2020-05-20T13:41:30.327Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 7,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-06-09T03:12:55.885Z',
        text:
          '(受傷)\n原來汽車機車強制險不是\n只有賠償意外傷亡200萬, \n這個最重要一定要去了解！這篇報導…您不看是您的損失！\n\nhttps://youtu.be/7BV...',
        hyperlinks: [
          {
            summary:
              '觀賞更多影片請至https://goo.gl/rvS1UC  歡迎分享 訂閱\n心靈舒果FB：https://www.facebook.com/anytime99...',
            title:
              '心靈舒果--  您非知道不可的生活常識有汽車 機車強制險的好友們．一定要看一下 要牢記起來',
            url: 'https://youtu.be/7BVazOgwsXk',
          },
        ],
        updatedAt: '2017-03-05T00:13:37.379Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_44',
      },
      '/articles/doc/CBUT_article_37': {
        references: [
          {
            createdAt: '2017-01-11T08:52:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-11T08:52:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-12-14T04:09:00.115Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_39',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-12-14T04:09:00.115Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T15:07:00.163Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T15:07:00.163Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-11T08:52:00.000Z',
        text:
          '小兒用藥安全-小兒用藥的一些基本觀念 https://www.parenting.com.tw/article/5059770-小兒用藥安全-小兒用藥的一些基本觀念/',
        hyperlinks: [
          {
            summary:
              '不管是中藥或是西藥，沒有任何一種藥是沒有副作用的。如果有少數中藥總是強調天然植物提煉，完全不提其副作用，那麼這是一種欺騙的行為。什麼才是正確的用藥觀念呢？讓黃醫...',
            title: '小兒用藥安全-小兒用藥的一些基本觀念/親子天下',
            url:
              'https://www.parenting.com.tw/article/5059770-小兒用藥安全-小兒用藥的一些基本觀念/',
          },
        ],
        updatedAt: '2017-01-11T08:52:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_38': {
        references: [
          {
            createdAt: '2020-02-21T02:02:54.455Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_TthEcGGgLb2we2tKAG98NuJq1KJE6Bd2WbQPK7kqG2M',
            appUserId: 'CBUT_user_56',
          },
        ],
        userId: 'j4S8C_TthEcGGgLb2we2tKAG98NuJq1KJE6Bd2WbQPK7kqG2M',
        tags: [],
        createdAt: '2020-02-21T02:02:54.455Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-02-21T02:49:47.098Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 44,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_44',
            userId: 'CBUT_user_57',
            status: 'NORMAL',
            updatedAt: '2020-02-21T02:49:47.098Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-08T14:19:31.631Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_45',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2020-04-08T14:19:31.631Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T01:28:20.202Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-05-22T01:28:20.202Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-02-21T02:02:55.271Z',
        text:
          '意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨。這回意大利下的石頭雨就讓人見識了大自然的不可預測，不可控制，不可防範的天災，也再次見識了極端天氣之恐怖！\n...',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2020-02-21T02:02:54.455Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_56',
      },
      '/articles/doc/CBUT_article_39': {
        references: [
          {
            createdAt: '2020-04-26T07:32:15.732Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_AunHFdGqCNC2QRUrQ9PSqkcE49Z4JTrkG9mYcgeogm8',
            appUserId: 'CBUT_user_67',
          },
        ],
        userId: 'j4S8C_AunHFdGqCNC2QRUrQ9PSqkcE49Z4JTrkG9mYcgeogm8',
        tags: [],
        createdAt: '2020-04-26T07:32:15.732Z',
        normalArticleCategoryCount: 1,
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-26T15:36:41.569Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_55',
            userId: 'CBUT_user_68',
            status: 'NORMAL',
            updatedAt: '2020-04-26T15:36:41.569Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-27T09:34:49.517Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_56',
            userId: 'CBUT_user_69',
            status: 'NORMAL',
            updatedAt: '2020-04-27T09:34:49.517Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T06:54:21.155Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_7',
            status: 'NORMAL',
            updatedAt: '2020-05-19T06:54:21.155Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 2,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2020-04-27T09:34:01.196Z',
        text:
          'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
        hyperlinks: [
          {
            normalizedUrl:
              'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
            summary:
              'Skip to content\n\n\t\n\t\n\n\t\n\t\t\n\n\t\t\t\n\n\n\t\n\n\t\n\t\n\t\t\n\n\n\n\n朝鲜全体党员、人民军将领以及人民：\n\n\n\n　　朝鲜劳动党中央委员...',
            title: '朝鲜全体党员、人民军将领以及人民：',
            url:
              'https://usacra.wordpress.com/2020/04/23/%e6%9c%9d%e9%b2%9c%e5%85%a8%e4%bd%93%e5%...',
          },
        ],
        updatedAt: '2020-04-26T07:32:15.732Z',
        appUserId: 'CBUT_user_67',
      },
      '/articles/doc/CBUT_article_4': {
        references: [
          {
            createdAt: '2019-05-30T13:03:09.773Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_fqo4vs7i2gmJi3mc1zlp_YgMi2_xyGiHfOR5_eLeYkE',
            appUserId: 'CBUT_user_66',
          },
        ],
        userId: 'j4S8C_fqo4vs7i2gmJi3mc1zlp_YgMi2_xyGiHfOR5_eLeYkE',
        tags: [],
        createdAt: '2019-05-30T13:03:09.773Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-06-02T01:40:59.876Z',
            replyType: 'NOT_ARTICLE',
            positiveFeedbackCount: 2,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_54',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-06-02T01:40:59.876Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 1,
            createdAt: '2020-05-24T00:58:22.690Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_1',
            status: 'NORMAL',
            updatedAt: '2020-05-24T00:58:22.690Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2019-05-30T13:03:09.792Z',
        text:
          '(moon)A，戀人\n\n他是愛你的，你在他眼中你是是完美的戀人，你的優點缺點他都能夠接受。\n情人眼裡出西施，說的就是他，你們在一起是他最為榮幸的，他覺得你是他一...',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-05-30T13:03:09.773Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_66',
      },
      '/articles/doc/CBUT_article_40': {
        references: [
          {
            createdAt: '2017-05-27T04:06:18.243Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo',
        tags: [],
        createdAt: '2017-05-27T04:06:18.243Z',
        articleReplies: [
          {
            createdAt: '2017-06-20T17:29:47.809Z',
            negativeFeedbackCount: 1,
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_58',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2017-06-20T17:29:47.809Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-07-22T03:51:29.633Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_59',
            userId: 'CBUT_user_71',
            status: 'NORMAL',
            updatedAt: '2020-07-22T03:51:29.633Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T05:37:25.077Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-20T05:37:25.077Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 3,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-06-13T13:42:06.389Z',
        text:
          '蜂蜜正確吃法 https://youtu.be/6Mh0EKBfGYg               這是一篇很棒的建議。',
        hyperlinks: [
          {
            summary:
              '選購保健食品，還在看小綠人標章嗎？\n你知道出過問題的味全油品也通過小綠人認證嗎？！\n\n經過人體雙盲研究的保健效果才真正有效！\n\n江守山醫師精選12大保健天然保健...',
            title: '《吃對天然保健食品2天然篇》蜂蜜的神奇保健力',
            url: 'https://youtu.be/6Mh0EKBfGYg',
          },
        ],
        updatedAt: '2017-05-27T04:06:18.243Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_17',
      },
      '/articles/doc/CBUT_article_41': {
        references: [
          {
            createdAt: '2019-04-17T06:56:36.132Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_ppZEoc7CPAXLOlzNOj3zwN_re6FKnwtjIiMmzS0_TFk',
            appUserId: 'CBUT_user_77',
          },
        ],
        userId: 'j4S8C_ppZEoc7CPAXLOlzNOj3zwN_re6FKnwtjIiMmzS0_TFk',
        tags: [],
        createdAt: '2019-04-17T06:56:36.132Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-04-18T11:30:15.508Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_63',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-04-18T11:30:15.508Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T00:11:03.854Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_5',
            status: 'NORMAL',
            updatedAt: '2020-05-22T00:11:03.854Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2019-04-17T06:56:36.153Z',
        text: '日本人砍台灣樹林比台灣人砍的還要多?',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-04-17T06:56:36.132Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_77',
      },
      '/articles/doc/CBUT_article_42': {
        references: [
          {
            createdAt: '2019-04-15T16:38:11.456Z',
            appId: 'WEBSITE',
            type: 'URL',
            permalink:
              'https://www.facebook.com/NeilPeng.tw/photos/a.289826441057121/2633838049989270/?type=3&theater',
            userId: 'CBUT_user_35',
          },
        ],
        userId: 'CBUT_user_35',
        tags: [],
        createdAt: '2019-04-15T16:38:11.456Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-04-15T16:40:37.610Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_64',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2019-04-15T16:40:37.610Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-19T07:47:55.647Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_4',
            status: 'NORMAL',
            updatedAt: '2020-05-19T07:47:55.647Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'WEBSITE',
        lastRequestedAt: '2019-04-15T16:38:11.473Z',
        text:
          '一名網友在「韓國瑜加油團」臉書社團發文指出，經在高雄市政府工作的朋友告知，韓國瑜此行訪美，川普特別透過秘密管道向高市府傳達求見意願，還強調之所以會找上韓國瑜，是...',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2019-04-15T16:38:11.456Z',
        normalArticleCategoryCount: 1,
      },
      '/articles/doc/CBUT_article_43': {
        text:
          '百億離岸風電國產大餅訂單爆肥水外流\n蘋果日報\n離岸風電產業近年因政策利多及多家國際能源大廠來台投資，號稱將帶動八千億產值，其中鋼鐵股王世紀集團被視為民營鋼廠的領...',
        createdAt: '2020-09-26T07:29:51.676Z',
        updatedAt: '2020-09-26T07:29:51.676Z',
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        references: [
          {
            type: 'URL',
            permalink: '',
            createdAt: '2020-09-26T07:29:51.676Z',
            userId: 'CBUT_user_35',
            appId: 'WEBSITE',
          },
        ],
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-09-26T07:30:21.061Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_74',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2020-09-26T07:30:21.061Z',
          },
        ],
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-09-26T15:59:24.667Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bow',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-09-26T15:59:24.667Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        normalArticleReplyCount: 1,
        normalArticleCategoryCount: 1,
        replyRequestCount: 1,
        tags: [],
        hyperlinks: [],
        lastRequestedAt: '2020-09-26T07:29:51.708Z',
      },
      '/articles/doc/CBUT_article_44': {
        text:
          '柯文哲行程曝光，也讓現場媒體相當質疑，特別是先前曝光的國務院、國防部皆被取消，由於當天柯的行程相當鬆散，加上柯訪美期間每天都安排早上受訪，唯獨華盛頓安排中午，也...',
        createdAt: '2020-09-26T09:39:48.802Z',
        updatedAt: '2020-09-26T09:39:48.802Z',
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        references: [
          {
            type: 'URL',
            permalink: '',
            createdAt: '2020-09-26T09:39:48.802Z',
            userId: 'CBUT_user_35',
            appId: 'WEBSITE',
          },
        ],
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-09-26T09:40:02.621Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 1,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_75',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2020-09-26T09:40:02.621Z',
          },
        ],
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-09-26T16:07:22.734Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bow',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_2',
            status: 'NORMAL',
            updatedAt: '2020-09-26T16:07:22.734Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        normalArticleReplyCount: 1,
        normalArticleCategoryCount: 1,
        replyRequestCount: 1,
        tags: [],
        hyperlinks: [],
        lastRequestedAt: '2020-09-26T09:39:48.829Z',
      },
      '/articles/doc/CBUT_article_5': {
        references: [
          {
            createdAt: '2017-11-14T23:42:12.624Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_lQfJ8fARjHNzkXa4s0ZENAkcWbKn_UOARG7VY11af8E',
        tags: [],
        createdAt: '2017-11-14T23:42:12.624Z',
        articleReplies: [
          {
            createdAt: '2017-11-15T05:23:58.758Z',
            negativeFeedbackCount: 1,
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_0',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-11-15T05:23:58.758Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T06:53:00.369Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T06:53:00.369Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-11-14T23:42:12.633Z',
        text:
          '眼睛周圍有一粒一粒的要如何改善？非常多人都有！分享出去功德無量！\nhttp://goez1.com/p97252.asp\n',
        hyperlinks: [
          {
            summary:
              '我們有時候會發現眼睛周圍會突然冒出一些白色顆粒狀的小疙瘩，看起很是讓人困擾，其實這個就是脂肪粒了洗臉的時候對著鏡子總是要嘆氣，別人看著自己的臉時也特別不自信，不...',
            title:
              '眼睛周圍有一粒一粒的要如何改善？非常多人都有！分享出去功德無量！ | EZGOE',
            url: 'http://goez1.com/p97252.asp',
          },
        ],
        updatedAt: '2017-11-14T23:42:12.624Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_50',
      },
      '/articles/doc/CBUT_article_6': {
        references: [
          {
            createdAt: '2017-01-10T01:12:00.000Z',
            type: 'LINE',
          },
        ],
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        tags: [],
        createdAt: '2017-01-10T01:12:00.000Z',
        articleReplies: [
          {
            createdAt: '2017-03-24T16:51:23.378Z',
            negativeFeedbackCount: 1,
            replyType: 'RUMOR',
            positiveFeedbackCount: 40,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_66',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-03-24T16:51:23.378Z',
          },
          {
            createdAt: '2017-10-03T03:37:10.744Z',
            negativeFeedbackCount: 3,
            replyType: 'RUMOR',
            positiveFeedbackCount: 63,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_1',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-10-03T03:37:10.744Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-04-08T13:45:36.418Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 4,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_67',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2020-04-08T13:45:36.418Z',
          },
        ],
        normalArticleReplyCount: 3,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T17:58:18.866Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_3',
            status: 'NORMAL',
            updatedAt: '2020-05-21T17:58:18.866Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 5,
        appId: 'BOT_LEGACY',
        lastRequestedAt: '2017-01-10T01:12:00.000Z',
        text:
          '*轉po*\n昨天年金改革委員會第14次會議，重點如以下幾點：\n1.各項年金請領年齡皆為65歲（未滿65歲無法請領），不分族群職業類別。\n勞保原為60歲即可領，未...',
        hyperlinks: [],
        updatedAt: '2017-01-10T01:12:00.000Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_37',
      },
      '/articles/doc/CBUT_article_7': {
        references: [
          {
            createdAt: '2018-10-23T09:21:35.323Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_rjLUCvhHqmYeT37nsWvYMXt7o0r3Mbniqdr7qU5LXgo',
            appUserId: 'CBUT_user_51',
          },
          {
            createdAt: '2018-10-30T01:50:21.966Z',
            appId: 'RUMORS_LINE_BOT',
            type: 'LINE',
            userId: 'j4S8C_uQip0_ezI3-UENg4tvIubbsdJDOHYWUs8sbOeCIaKN8',
            appUserId: 'CBUT_user_52',
          },
        ],
        userId: 'j4S8C_rjLUCvhHqmYeT37nsWvYMXt7o0r3Mbniqdr7qU5LXgo',
        tags: [],
        createdAt: '2018-10-23T09:21:35.323Z',
        articleReplies: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-10-23T11:51:05.419Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 10,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_40',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2018-10-23T11:51:05.419Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-10-23T17:31:53.669Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 10,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_41',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2018-10-23T17:31:53.669Z',
          },
          {
            negativeFeedbackCount: 6,
            createdAt: '2018-10-24T13:19:03.745Z',
            replyType: 'NOT_RUMOR',
            positiveFeedbackCount: 4,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_42',
            userId: 'CBUT_user_53',
            status: 'NORMAL',
            updatedAt: '2018-10-24T13:19:03.745Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2018-10-25T16:17:06.303Z',
            replyType: 'OPINIONATED',
            positiveFeedbackCount: 10,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_2',
            userId: 'CBUT_user_15',
            status: 'NORMAL',
            updatedAt: '2018-10-25T16:17:06.303Z',
          },
          {
            negativeFeedbackCount: 1,
            createdAt: '2018-11-22T15:55:57.510Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 6,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_43',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2018-11-22T15:55:57.510Z',
          },
        ],
        normalArticleReplyCount: 5,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-22T09:35:53.201Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_1',
            status: 'NORMAL',
            updatedAt: '2020-05-22T09:35:53.201Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 4,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2018-12-05T13:22:22.965Z',
        text:
          '大法官已釋憲，如果年底公投沒達到500萬票，明年五月台灣將成為亞洲第一個同性可合法結婚的國家。\n\n外國同性戀藉結婚來台治療愛滋病，都是健保支付，全民買單。\n\n學...',
        normalArticleReplycount: 0,
        hyperlinks: [],
        updatedAt: '2018-10-30T01:50:21.966Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_51',
      },
      '/articles/doc/CBUT_article_8': {
        references: [
          {
            createdAt: '2017-08-01T08:45:13.720Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_Z8nYqKv190EeosqG5O1H8aeTg64m6xLkRyZOTxSYQ7A',
        tags: [],
        createdAt: '2017-08-01T08:45:13.720Z',
        articleReplies: [
          {
            createdAt: '2017-08-01T13:53:32.747Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 8,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_3',
            userId: 'CBUT_user_45',
            status: 'NORMAL',
            updatedAt: '2017-08-01T13:53:32.747Z',
          },
        ],
        normalArticleReplyCount: 1,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-21T03:24:12.458Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-21T03:24:12.458Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 1,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-08-01T08:45:13.734Z',
        text:
          '天啊! 原來只要這個動作，發黑香蕉瞬間變回新鮮香蕉?!! 實在太神奇了!一定要分享出去!!\nhttp://goez1.com/p46719.asp\n',
        hyperlinks: [
          {
            summary:
              '這是不是太誇張了！我相信你的下巴一定也跟小編一樣掉到地上了，如果你還是不相信，請看完整操作影片！ 現在你再也可以不用因為浪費了香蕉而後悔不已了！ 快將這個神奇的妙招分享出去讓朋友們大開眼界吧！',
            title:
              '天啊! 原來只要這個動作，發黑香蕉瞬間變回新鮮香蕉?!! 實在太神奇了!一定要分享出去!! | EZGOE',
            url: 'http://goez1.com/p46719.asp',
          },
        ],
        updatedAt: '2017-08-01T08:45:13.720Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_46',
      },
      '/articles/doc/CBUT_article_9': {
        references: [
          {
            createdAt: '2017-05-14T13:07:55.057Z',
            type: 'LINE',
          },
        ],
        userId: 'j4S8C_Yk4N-rJ2LtcKZc0Wa2smuJQNB5I7mmo-d2j1k-nyfSo',
        tags: [],
        createdAt: '2017-05-14T13:07:55.057Z',
        articleReplies: [
          {
            createdAt: '2017-07-09T15:02:23.375Z',
            negativeFeedbackCount: 0,
            replyType: 'RUMOR',
            positiveFeedbackCount: 146,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_70',
            userId: 'CBUT_user_35',
            status: 'NORMAL',
            updatedAt: '2017-07-09T15:02:23.375Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2019-12-06T16:05:05.617Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 13,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_4',
            userId: 'CBUT_user_54',
            status: 'NORMAL',
            updatedAt: '2019-12-06T16:05:05.617Z',
          },
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-03-26T06:01:58.473Z',
            replyType: 'RUMOR',
            positiveFeedbackCount: 0,
            appId: 'WEBSITE',
            replyId: 'CBUT_reply_71',
            userId: 'CBUT_user_84',
            status: 'DELETED',
            updatedAt: '2020-03-26T06:05:11.732Z',
          },
        ],
        normalArticleReplyCount: 2,
        articleCategories: [
          {
            negativeFeedbackCount: 0,
            createdAt: '2020-05-20T06:31:05.776Z',
            positiveFeedbackCount: 0,
            appId: 'DEVELOPMENT_BACKEND',
            aiModel: 'bert',
            userId: 'qO1C3_qRN1QoZXuSa3AXBwlee3RiStUngzO50-GUF46nH-pOU',
            aiConfidence: 1,
            categoryId: 'CBUT_category_8',
            status: 'NORMAL',
            updatedAt: '2020-05-20T06:31:05.776Z',
            appUserId: 'CBUT_user_36',
          },
        ],
        replyRequestCount: 2,
        appId: 'RUMORS_LINE_BOT',
        lastRequestedAt: '2017-05-21T04:51:51.057Z',
        text:
          '(牛奶加一物，三高、痛風、癌症都不見了！姐夫82歲了還活力無窮！轉發出去功德無量！)【快來看】│這篇文不錯分享給你看│ →    http://ezvivi.c...',
        hyperlinks: [
          {
            summary:
              '每天一杯牛奶加上它，痛風不見了！還遠離三高，對抗癌細胞！太神奇了！！\n\t\t\t\t\t\n\t\t\t\t\t\n\t\t\t\t\t\n\n\n\n\n\t\t\t\t\t\t\t\n\t\t\n\t\n\n\n\n\t\n\t\t\n\t\t...',
            title:
              '牛奶加一物，三高、痛風、癌症都不見了！姐夫82歲了還活力無窮！轉發出去功德無量！ | Love分享',
            url: 'http://ezvivi.com/article/257739.asp',
          },
          {
            summary:
              '☆COLOR99☆韓衣--持平價奢華路線˙給妳專屬寵愛˙教妳變化多端造型、讓大美人兒也可以穿出自信，讓您有最方便的購物選擇單次消費金額滿1元或累計消費金額滿2元...',
            title: 'Color99-Yahoo!奇摩超級商城',
            url: 'http://goo.gl/mKq5t0',
          },
        ],
        updatedAt: '2017-05-14T13:07:55.057Z',
        normalArticleCategoryCount: 1,
        appUserId: 'CBUT_user_83',
      },
    },
    replies: {
      '/replies/doc/CBUT_reply_0': {
        reference:
          'https://kknews.cc/zh-tw/fashion/lznly52.html\n太早用眼霜會長脂肪粒？別再傳這樣的謠言了',
        createdAt: '2017-11-15T05:23:58.752Z',
        appId: 'WEBSITE',
        text:
          '眼霜並不是導致脂肪粒產生的原因。脂肪粒大多數情況下指的是粟丘疹，它是一種形成於毛囊皮脂腺的表皮囊腫，內部聚集了角質化的殘留物，通常為白色/淡黃色凸起。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '很多妹子們覺得20幾歲還年輕，用不上眼霜。但皺紋的產生是逐步累積的過程，年輕的時候不明顯並不能代表你沒有這個問題！\n...\n脂肪粒的真相\n提到眼霜，就會出現脂肪...',
            title: '太早用眼霜會長脂肪粒？別再傳這樣的謠言了！',
            url: 'https://kknews.cc/zh-tw/fashion/lznly52.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_1': {
        reference:
          'http://www.mygopen.com/2017/10/14line.html\n【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！',
        createdAt: '2017-10-03T03:37:10.740Z',
        appId: 'WEBSITE',
        text:
          '純屬謠言一則，年金改革辦公室也做出澄清了，這完全不是事實，事實上年改會在14次針對年金領取資格會中沒有達成任何共識，勞保方面，根據勞保條例規定，勞保老年年金請領...',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！\n\n\n\n\n\n\n\n\n\n已經傳很久的 LINE 謠言，內容為「昨天年金改革委員會第14次會議，...',
            title:
              '【假新聞】年金改革委員會第14次會議？LINE 傳的都假的，不要採信！ ｜MyGoPen',
            url: 'http://www.mygopen.com/2017/10/14line.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_10': {
        reference:
          'http://sp1.hso.mohw.gov.tw/doctor/Often_question/type_detail.php?PageNo=2&q_type...',
        createdAt: '2018-02-07T17:33:53.022Z',
        appId: 'WEBSITE',
        text:
          '在夜間睡眠時，由於活動量降低，會使得體溫下降，倘保暖不足時，就會出現手腳冰冷的現象，甚至連身體都涼；同時吸入冷空氣刺激支氣管黏膜，而引發咳嗽。\n ',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary: '提醒您，網路醫療諮詢服務並不能代替就醫。',
            title: '',
            url:
              'http://sp1.hso.mohw.gov.tw/doctor/Often_question/type_detail.php?PageNo=2&q_type...',
          },
          {
            summary:
              '▲幼兒沒病卻「咳不停」？半夜踢被＋空氣太冷是主因。（圖／示意圖／達志影像）\n記者嚴云岑／台北報導\n3歲男童，最近幾個月，半夜經常咳不停，雖然睡眠並未因此中斷，就...',
            title:
              '幼兒沒病卻「咳不停」？　半夜踢被＋空氣太冷是主因 | ETtoday健康雲',
            url: 'https://health.ettoday.net/news/614785',
          },
        ],
      },
      '/replies/doc/CBUT_reply_11': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '美國闢謠網站 Snopes 向 James Robb 博士本人查證，博士表示這是他寫的信，但那是給家人與親朋好友的信，並沒有預期到會被公開。\n\n關於其中「鋅錠劑...',
        reference:
          '2020/3/2 Did a Noted Pathologist Write This Viral Coronavirus Advice Letter?\nhtt...',
        createdAt: '2020-03-03T05:10:50.254Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.snopes.com/fact-check/zinc-lozenges-coronavirus/',
            summary:
              'In late February 2020, as a new coronavirus spread worldwide, a letter providing...',
            title:
              'Did a Noted Pathologist Write This Viral Coronavirus Advice Letter?',
            url: 'https://www.snopes.com/fact-check/zinc-lozenges-coronavirus/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_12': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '影片中男主角實為古巴裔美國演員威廉·列維（William Levy）。其中一幕來自墨西哥電視劇（Triunfo del amor ）。\n',
        reference:
          'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml\n義大利女友感染新冠肺炎，男友脫掉防護服見最後一面後雙雙殉情？謠言！\n',
        createdAt: '2020-04-03T04:24:14.807Z',
        hyperlinks: [
          {
            normalizedUrl: 'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml',
            summary:
              '北京市科协主办\n\t\t\t\t\t\t\n\t\t\t\n\t\t\t\n\n\t\t\n\n\n\n\n\n\t\n\n\t\t\n\n\t\t\t\n\n\t\t\t\n\n\t\t\t\n\t\t\t\t\n\t\t\t\t\t首页HOME\n\t\t\t\t\t科技热闻N...',
            title:
              '意大利女友感染新冠肺炎，男友脱掉防护服见最后一面后双双殉情？谣言！',
            url: 'http://ask.kedo.gov.cn/c/2020-04-02/1026409.shtml',
          },
        ],
      },
      '/replies/doc/CBUT_reply_13': {
        reference: 'http://www.tada2002.org.tw/tada_know_02.html',
        createdAt: '2017-04-12T11:25:01.484Z',
        appId: 'WEBSITE',
        text:
          '失智症的成因多元，如有症狀須儘早就醫對症下藥，食療是好的輔助方式，但文中提到「不用吃藥」有誇大之嫌。',
        type: 'RUMOR',
        userId: 'CBUT_user_85',
        hyperlinks: [
          {
            summary:
              '失智人口知多少\n                    台灣失智症人口推估全球失智症人口快速增加依據2017年國際失智症協會（ADI）資料，推估2017年將新增...',
            title: '關於失智症-認識失智症',
            url: 'http://www.tada2002.org.tw/tada_know_02.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_14': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '假的。手機導致的疾病包含器官老化為無稽之談，目前手機釋放的射頻輻射（ RF ）被國際癌症研究機構（ International Agency for Resea...',
        reference:
          'https://www.thenewslens.com/article/91387\nhttps://www.medpartner.club/electromag...',
        createdAt: '2019-11-18T13:31:29.656Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://www.thenewslens.com/article/91387',
            summary:
              '人活著就是要吃東西！對於繁忙的現代人而言，無論是從便利商店購買香噴噴的微波食品，或者是加熱昨晚沒吃完的剩菜剩飯，微波爐似乎是生活當中必不可少的家電之一。    ...',
            title:
              '【圖輯】會致癌？有輻射？各種微波爐謠言一次破解 - The News Lens 關鍵評論網',
            url: 'https://www.thenewslens.com/article/91387',
          },
          {
            normalizedUrl:
              'https://www.medpartner.club/electromagnetic-wave-cancer-myth/',
            summary:
              '手機電磁波真的會致癌嗎？相關的網路傳言一大堆，到底應該相信哪一個？手機真的對我們身體有這麼大的危害嗎？我平常工作就是需要常常使用手機與客戶聯繫，要怎麼辦？其實這...',
            title:
              '手機電磁波致癌！？專家教你如何降低3C危害保護家人健康！ | 美的好朋友',
            url:
              'https://www.medpartner.club/electromagnetic-wave-cancer-myth/',
          },
          {
            normalizedUrl:
              'https://cn.nytimes.com/health/20180205/cell-phones-cancer/zh-hant/',
            summary:
              '手機致癌嗎？儘管已研究多年，這個問題依然沒有明確的答案。但是衛生官員稱，週五公布的兩項分別在大鼠和小鼠身上進行的相關政府研究表明，即便有風險，也是比較小的。多年...',
            title: '手機輻射致癌嗎？科學家研究十年後有了新發現',
            url:
              'https://cn.nytimes.com/health/20180205/cell-phones-cancer/zh-hant/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_15': {
        reference: '',
        createdAt: '2018-01-26T03:24:02.717Z',
        appId: 'WEBSITE',
        text: '這是商業活動廣告 ',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_35',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_16': {
        reference: 'http://www.ettoday.net/news/20130826/261531.htm',
        createdAt: '2017-03-21T16:25:34.315Z',
        appId: 'WEBSITE',
        text:
          '愛滋病毒離開活體後，1個小時以內就無法存活。即使愛滋病毒血液被滴入罐頭，接觸後感染機率非常低。\n另外，就算吃進仍存活的愛滋病毒，如果口腔沒有傷口，也不會因此感染愛滋病。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '▲國際愛滋病研討會發表資料照片。（圖／達志影像／美聯社）\n東森新聞記者陳稚暉、謝宗樺／台中報導\n又是一起網路謠言！已經有人在臉書（Facebook）上看到消息誤...',
            title: '謠言勿再傳！「泰國罐頭遭滴愛滋血」太扯 | ETtoday新聞雲',
            url: 'http://www.ettoday.net/news/20130826/261531.htm',
          },
        ],
      },
      '/replies/doc/CBUT_reply_17': {
        userId: 'CBUT_user_47',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '有關外界網路平台及line群組流傳出現：「現在的國小三年級教保險套、國小四年級教自慰、國小五年級教如何許多人一起性行為…」等類似言論，經教育部及國教院查證國小教...',
        reference:
          '教育部澄清網路群組流傳教科書不實性別平等教育內容之回應\nhttps://www.edu.tw/News_Content.aspx?n=FD56C961F1677...',
        createdAt: '2018-11-07T06:17:00.435Z',
        hyperlinks: [
          {
            summary: '',
            title: '教育部澄清網路群組流傳教科書不實性別平等教育內容之回應',
            url:
              'https://www.edu.tw/News_Content.aspx?n=FD56C961F1677400&sms=E6059C30DDBD5135&s=FC73C38D12A5F849',
          },
        ],
      },
      '/replies/doc/CBUT_reply_18': {
        userId: 'CBUT_user_73',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '覆蓋麵粉不但無法冷卻皮膚，還會阻止皮膚散熱，甚至使傷口感染。 燙傷應立刻以流動清水沖洗冷卻，醬油、牙膏、麵粉都不是在無菌的環境下生產的，可能引起傷口感染，而且增...',
        reference:
          '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤http://news.ltn.com.tw/news/life/paper/1101504\n燙傷撒麵粉能...',
        createdAt: '2018-12-28T13:27:11.319Z',
        hyperlinks: [
          {
            summary:
              '2017-05-11記者陳炳宏／台北報導一名4歲男童嬉戲時，不慎絆倒電熱水壺，造成大面積燙傷，阿公和阿嬤在慌亂中，急忙脫掉他的衣服，並撒麵粉，企圖「降溫」，沒想...',
            title:
              '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤 - 生活 - 自由時報電子報',
            url: 'http://news.ltn.com.tw/news/life/paper/1101504',
          },
          {
            summary:
              '一名4歲男童因被熱水燙傷，爺爺奶奶一時心急，搶在第一時間先脫下男童衣服，還撒上麵粉，以為能藉此降溫，結果導致傷口更難散熱。治療燒燙傷的偏方千奇百怪，除了小護士、...',
            title: '燙傷撒麵粉能降溫？醫生：敷藥前得先刮層皮 - 康健雜誌',
            url:
              'http://www.commonhealth.com.tw/article/article.action?nid=74762',
          },
        ],
      },
      '/replies/doc/CBUT_reply_19': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          'FAKE.\nThe advice it provides runs contrary to all accepted knowledge about how t...',
        reference: 'https://www.snopes.com/fact-check/flour-power/',
        createdAt: '2019-11-13T16:18:32.723Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://www.snopes.com/fact-check/flour-power/',
            summary:
              'Claim:   The application of flour is an effective and recommended treatment for ...',
            title: 'Should You Treat Burns with Flour?',
            url: 'https://www.snopes.com/fact-check/flour-power/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_2': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '「外國同性戀藉結婚來台治療愛滋病，都是健保支付，全民買單」這句話其實並不算錯，但實務上並不會有「拖垮健保」的狀況，也跟同性婚姻關係不大，所以不用太擔心唷。\n\n身...',
        reference:
          '衛福部 - 有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明 \nhttps://www.cdc.gov.tw/in...',
        createdAt: '2018-10-25T16:17:06.241Z',
        hyperlinks: [
          {
            summary:
              '有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明： 外籍人士因工作或其他因素申請在台居留，並合法居留六個月以上，才...',
            title:
              '有關網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔，疾管署特此說明',
            url:
              'https://www.cdc.gov.tw/info.aspx?treeid=45da8e73a81d495d&nowtreeid=1bd193ed6daba...',
          },
          {
            summary:
              '圖／沃草烙哲學提供最近大家應該多少都有看到一些網路資訊，指出同性婚姻合法化會吸引外籍愛滋患者1來台就醫，然後拖垮台灣健保。藉由這些資訊，有些反同人士進一步建立這...',
            title:
              '蔣維倫／愛滋補助是用來拖垮健保的嗎？從效益主義觀看 | 沃草烙哲學 | 鳴人堂',
            url: 'https://opinion.udn.com/opinion/story/6685/2362679',
          },
          {
            summary:
              '最近有太多關於愛滋的謠言，讓我們來告訴你真相究竟是什麼\n製作：台灣同志諮詢熱線協會、愛滋權促會\n設計：開合跳工作室\n　\n\t以下這些流言/謠言，你是不是也聽過呢？...',
            title: '【破除謠言】你所不知道的愛滋真相網頁版(另附錄政府闢謠圖文)',
            url: 'https://hotline.org.tw/blog/1111',
          },
          {
            summary:
              '▲最具影響力的新聞，都在NOWnews今日新聞。（圖／翻攝自網路）\n\n網路流傳外國人染愛滋病來台2年後，所有外國人的愛滋病治療費用是由健保署負擔說法，甚至嘲諷「...',
            title:
              '沒這麼好康！外國人治愛滋　疾管署：須先自掏40萬 | 生活 | NOWnews 今日新聞',
            url: 'https://www.nownews.com/news/20170206/2395108/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_20': {
        userId: 'CBUT_user_49',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '內政部警政署的輿情回應專區，有特別針對這一則消息製作澄清一覽表：網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表,如附件資料。',
        reference:
          'https://www.npa.gov.tw/NPAGip/wSite/public/Attachment/f1531122098570.pdf',
        createdAt: '2018-10-01T16:02:38.646Z',
        hyperlinks: [
          {
            summary: null,
            title: null,
            url:
              'https://www.npa.gov.tw/NPAGip/wSite/public/Attachment/f1531122098570.pdf',
          },
        ],
      },
      '/replies/doc/CBUT_reply_21': {
        reference: '',
        createdAt: '2017-11-19T10:08:18.033Z',
        appId: 'WEBSITE',
        text: '文字長度太短，疑似為使用者手動輸入之查詢語句，不像轉傳文章。',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_35',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_22': {
        reference:
          '比對 join 平台提案內容以及教育部新聞稿 https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB\n',
        createdAt: '2017-06-27T15:25:17.272Z',
        appId: 'WEBSITE',
        text:
          '於「擴大幼兒教保公共化」的政府公開說明中，確實沒有指出如果沒有達成預計數字的話會有什麼作為。\n全教總於提案之中，確實提議將少開的班級加入下一年度班級數，而且在提...',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_15',
        hyperlinks: [
          {
            summary:
              '教育部長潘文忠今(20)日赴行政院報告「擴大幼兒教保公共化」相關規劃，為符膺總統托育政策，提供價格合理、品質有保障的教保服務，減輕家長的經濟負擔，教育部以穩健提...',
            title: '擴大公共化教保服務，教育部規劃增設公共化幼兒園1,000班',
            url:
              'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
          },
        ],
      },
      '/replies/doc/CBUT_reply_23': {
        reference:
          '\nhttp://www.appledaily.com.tw/realtimenews/article/new/20170614/1139982/\n海獸胃腺蟲」鑽胃壁　專家：過去國內僅１例',
        createdAt: '2017-08-29T04:03:40.178Z',
        appId: 'WEBSITE',
        text:
          '過去台灣只有一個海獸胃腺蟲鑽入胃壁的病例，而且僅有一隻蟲。而經查詢，台北馬偕醫院也沒有名為林清風的醫師。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '最近有則LINE訊息在網路上廣傳，指好多人因吃生魚片，「胃壁附著「海獸胃腺蟲」，有的病人甚至胃壁上滿滿都是，無法夾出，驅蟲藥也很難治，魚片中的生鮭魚含蟲量是最可...',
            title: '「海獸胃腺蟲」鑽胃壁　專家：過去國內僅１例 | 蘋果日報',
            url:
              'http://www.appledaily.com.tw/realtimenews/article/new/20170614/1139982/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_24': {
        userId: 'CBUT_user_88',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '無論是電子產品或是網站平台，均在註冊會員或啟用裝置時，會有一份資料蒐集同意書，其中會說明資料蒐集範圍，授權公司，去識別化方式等\n因此「手機內所有資料都會進入他們...',
        reference: ' ',
        createdAt: '2019-08-21T02:10:43.987Z',
      },
      '/replies/doc/CBUT_reply_25': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text: '送出文章的人在嘗試與機器人聊天。',
        reference: '',
        createdAt: '2019-03-19T14:57:01.101Z',
      },
      '/replies/doc/CBUT_reply_26': {
        reference: '',
        createdAt: '2017-09-13T13:44:48.946Z',
        appId: 'WEBSITE',
        text: '內容為個人遊記，不在查證範圍',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_38',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_27': {
        userId: 'CBUT_user_40',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text: '訊息與謠言查證無關。',
        reference: '',
        createdAt: '2018-07-10T16:17:54.862Z',
      },
      '/replies/doc/CBUT_reply_28': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '高中課本裡面只有同志諮詢熱線的電話。影片中手機的網址，是家長自己上網找的同志諮詢熱線官網，教育部審定的課本裡面並沒有網址。\n\n同志諮詢熱線是同志朋友的生命線，2...',
        reference:
          '尤美女委員臉書貼文：台灣同志諮詢熱線是同志朋友的生命線\nhttps://www.facebook.com/yumeinu/posts/1553089634706...',
        createdAt: '2019-05-13T08:50:36.806Z',
        hyperlinks: [
          {
            summary:
              'To personalise content, tailor and measure ads and provide a safer experience, w...',
            title: 'Security Check',
            url: 'https://www.facebook.com/yumeinu/posts/1553089634706916',
          },
          {
            summary:
              '台灣同志諮詢熱線協會對於網路謠傳本會「鼓勵高中生吸毒」影片之回應\n\n近日社群媒體上流傳本會「鼓勵高中生吸毒」的影片，內容為有心人士惡意地針對本會經營的爽歪歪網站...',
            title:
              '20190513台灣同志諮詢熱線協會對於網路謠傳本會「鼓勵高中生吸毒」影片之回應',
            url: 'https://hotline.org.tw/news/1594',
          },
          {
            summary:
              'Skip to content\n\t\n\t\n\t\n\t\t\n\t\t\n\t\t\t\n\n\t\t\t\n\t\t\t\n\t\t\t\t\n\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\tThe Works at Toron...',
            title: 'Harm Reduction Supplies & Locations',
            url: 'https://reurl.cc/WWVMe',
          },
          {
            summary:
              '派對藥物的使用在台灣是既存的事實，而政府單位對於派對藥物的因應方式，長期以來僅僅著重在「恐嚇藥物很可怕」、 「被抓到你就完了」的階段，卻忽略了現實環境中，許多人...',
            title:
              '關於用藥—重要提醒 | 熱線男同志性愉悅性教育網站-爽歪歪 SongYY',
            url: 'https://www.songyy.org.tw/sex/sex-6/sex6-2',
          },
        ],
      },
      '/replies/doc/CBUT_reply_29': {
        reference:
          'http://news.ltn.com.tw/news/life/breakingnews/1950973\n泰豐大火空氣落塵',
        createdAt: '2017-06-12T11:50:12.535Z',
        appId: 'WEBSITE',
        text:
          '衛生局會根據環保局監測結果隨時提醒民眾注意防範事項，呼籲大家不要過度恐慌。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '2017-01-18 11:27〔記者陳昀／桃園報導〕桃園市中壢區昨天發生輪胎廠大火，空氣含有落塵，桃園市政府衛生局提醒民眾，這幾天要儘量避免外出，衛生局也準備...',
            title:
              '泰豐大火空氣落塵嚴重 3萬口罩免費索取 - 生活 - 自由時報電子報',
            url: 'http://news.ltn.com.tw/news/life/breakingnews/1950973',
          },
        ],
      },
      '/replies/doc/CBUT_reply_3': {
        reference: 'https://www.youtube.com/watch?v=RbrCFWkwhPQ',
        createdAt: '2017-08-01T13:53:32.743Z',
        appId: 'WEBSITE',
        text: '此為不實謠言。',
        type: 'RUMOR',
        userId: 'CBUT_user_45',
        hyperlinks: [
          {
            summary:
              '香蕉擺久了會變黑，網路有一段影片，宣稱利用白米、夾鏈袋和吹風機，就可以讓香蕉回春，由黑變黃。記者實際操作，香蕉不僅沒復活，還被理化老師打臉。',
            title: '【中視新聞】米+吹風機 香蕉回春黑轉黃? 實測! 20150618',
            url: 'https://www.youtube.com/watch?v=RbrCFWkwhPQ',
          },
        ],
      },
      '/replies/doc/CBUT_reply_30': {
        reference: '',
        createdAt: '2017-12-27T14:55:54.646Z',
        appId: 'WEBSITE',
        text: '這是商業活動廣告 ',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_35',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_31': {
        reference: 'https://my83.com.tw/blogs?p=1338',
        createdAt: '2017-06-19T14:54:52.139Z',
        appId: 'WEBSITE',
        text:
          '根據保險資訊網站My83表示，只要非犯罪或過失的汽機車事故，事故的受害者不論有無過失，能迅速獲得理賠，不需在急需就醫時還花時間等待理賠金，所以政府強制汽車、機車及機器腳踏車主投保。',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_45',
        hyperlinks: [
          {
            summary:
              '汽車族、機車族及電動腳踏車族們，對於每年按時繳交強制險的規範及保障都有瞭解了嗎？\n\nMY83希望藉由強制險10問答，讓大家清楚強制投保的強制險，到底提供什麼保障...',
            title:
              '強制投保的強制險，有什麼保障？10 問答告訴你！ - 《MY83 保險專欄》',
            url: 'https://my83.com.tw/blogs?p=1338',
          },
        ],
      },
      '/replies/doc/CBUT_reply_32': {
        reference: 'http://news.ltn.com.tw/news/life/paper/1060978',
        createdAt: '2017-03-27T14:21:52.619Z',
        appId: 'WEBSITE',
        text:
          '教育部昨澄清，目前性平教材是早經公聽及專家修訂而無爭議，是提供教師增加知能，盼外界勿再以訛傳訛。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '2016-12-12〔記者吳柏軒、翁聿煌、沈佩瑤／台北報導〕婚姻平權議題沸騰，也牽動校園性平教育備受矚目。近期網路卻屢傳出影片《性教慾》，引用過期資料誣指官方要...',
            title: '性平教材遭抹黑 教部：勿以訛傳訛 - 生活 - 自由時報電子報',
            url: 'http://news.ltn.com.tw/news/life/paper/1060978',
          },
        ],
      },
      '/replies/doc/CBUT_reply_33': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '「小六大兒子被強迫跟另外一個女生練習自慰」是錯的。\n\n影片裡的媽媽在 1:30 的時候說的是：「哥哥在六年級，拿回學習單，哥哥回來問，為什麼學這個東西，要把我和...',
        reference:
          '學習單影本與教材截圖\nhttp://max-everyday.com/2017/01/masturbation/\n\n2017/1/4 中央社 - 議員爆小學課程...',
        createdAt: '2018-11-03T16:14:49.163Z',
        hyperlinks: [
          {
            summary:
              '要不要教小學六年級的同學這個題目，目前爭議很大，有很多家長贊成，很多家長反對。\n北市議員痛批國小課程鼓吹自慰？ 教育局回應：正確的觀念、態度才是最重要的，手冊是...',
            title: '國小六年級學習「什麼叫自慰」 – Max的每一天',
            url: 'http://max-everyday.com/2017/01/masturbation/',
          },
          {
            summary:
              '（中央社記者游凱翔台北4日電）台北市議員汪志冰與多名家長今天抨擊有小學在課堂教導「什麼是自慰」，教育局事後強調，這本教材來自學童常提的420個兩性問題，而自慰只...',
            title:
              '議員爆小學課程教導自慰 真相是這樣 | 地方 | 重點新聞 | 中央社 CNA',
            url: 'https://www.cna.com.tw/news/firstnews/201701040187.aspx',
          },
        ],
      },
      '/replies/doc/CBUT_reply_34': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '訊息與影片標題的「小六大兒子被強迫跟另外一個女生練習自慰」是錯的。\n\n影片裡的媽媽在 1:30 的時候說的是：「哥哥在六年級，拿回學習單，哥哥回來問，為什麼學這...',
        reference:
          '學習單影本與教材截圖\nhttp://max-everyday.com/2017/01/masturbation/\n\n2017/1/4 中央社 - 議員爆小學課程...',
        createdAt: '2018-11-03T16:17:08.155Z',
        hyperlinks: [
          {
            summary:
              '要不要教小學六年級的同學這個題目，目前爭議很大，有很多家長贊成，很多家長反對。\n北市議員痛批國小課程鼓吹自慰？ 教育局回應：正確的觀念、態度才是最重要的，手冊是...',
            title: '國小六年級學習「什麼叫自慰」 – Max的每一天',
            url: 'http://max-everyday.com/2017/01/masturbation/',
          },
          {
            summary:
              '（中央社記者游凱翔台北4日電）台北市議員汪志冰與多名家長今天抨擊有小學在課堂教導「什麼是自慰」，教育局事後強調，這本教材來自學童常提的420個兩性問題，而自慰只...',
            title:
              '議員爆小學課程教導自慰 真相是這樣 | 地方 | 重點新聞 | 中央社 CNA',
            url: 'https://www.cna.com.tw/news/firstnews/201701040187.aspx',
          },
        ],
      },
      '/replies/doc/CBUT_reply_35': {
        reference:
          'http://www.edu.tw/News_Content.aspx?n=9E⋯&sms=169B8E91BB75571F&s=6DDDD36D6874DB8...',
        createdAt: '2017-07-06T06:44:19.192Z',
        appId: 'WEBSITE',
        text:
          '教育部依性別平等教育法等相關規定實施性別平等教育課程，教科書也都經過審定。若家長有具體內容，可提出檢舉，教育部會請教科書業者再做審視及調整。\n目前國小教科書的「...',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '抱歉！請您再重新檢視輸入的網址是否正確。您也可以回到上一頁或首頁。\n\t\t\t\t\n\t\t\t\t\n\t\t\t\t\tSorry. Please check one more ti...',
            title: '教育部全球資訊網',
            url:
              'http://www.edu.tw/News_Content.aspx?n=9E⋯&sms=169B8E91BB75571F&s=6DDDD36D6874DB88',
          },
          {
            summary:
              '隨著同志婚姻議題之各方表述及熱烈討論，「性/性別教育」也一併成為關注焦點。各式各樣關於「性/性別教育」的資訊在電視廣告、LINE、臉書等通訊軟體流傳，但大多數描...',
            title:
              '【圖輯】 性別教育不只是性解放 揭密性別教育的十大真相 - The News Lens 關鍵評論網',
            url: 'https://www.thenewslens.com/article/56138',
          },
          {
            summary:
              '同性戀，亦稱為同性向、同性愛，是以同性為對象建立起親密關係，或以此性傾向做為主要自我認同的行為或現象[1]。同性戀連同雙性戀和異性戀構成了性傾向連續光譜不同程度...',
            title: '同性戀 - 维基百科，自由的百科全书',
            url: 'https://zh.wikipedia.org/wiki/同性戀',
          },
        ],
      },
      '/replies/doc/CBUT_reply_36': {
        userId: 'CBUT_user_48',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '有關外界網路平台及line群組流傳出現：「現在的國小三年級教保險套、國小四年級教自慰、國小五年級教如何許多人一起性行為…」等類似言論，經教育部及國教院查證國小教...',
        reference:
          '教育部性別平等教育 資訊網\nhttps://www.gender.edu.tw/web/index.php/home/index',
        createdAt: '2018-11-15T13:54:13.229Z',
        hyperlinks: [
          {
            summary:
              '教育部期望透過性別平等教育相關的課程與教學機會，教導學生瞭解並尊重不同的性別特質、性別認同與性傾向，清楚地讓學生認知其個體差異並不會影響個人能力及與他人之互動關...',
            title: '教育部告訴您【性別平等教育教什麼】',
            url: 'https://youtu.be/fHSCYj96Fwc',
          },
          {
            summary:
              '國家教育研究院依據課程綱要審定教科書，學校使用期間各界對教科書提出意見或建議，國家教育研究院都會依循相關機制妥善處理，並回應外界意見，期待性別平等教育落實在學生學習。',
            title: '教育部告訴您【性別平等教育教科書疑義處理說明篇】',
            url: 'https://youtu.be/K0I7lfliwOc',
          },
          {
            summary: '',
            title:
              '【教育部告訴您性別平等教育學校教科書選用及課程如何把關說明篇】',
            url: 'https://youtu.be/gy6d_xkgge8',
          },
          {
            summary:
              ':::\n\n\n\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t網站地圖\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\n\t\t\t\t\t\t  \n\t\t\t\t\t\t\t\t線上調查專區\n\t\t\t\t\t\t  \n   ...',
            title: '教育部性別平等全球資訊網',
            url: 'https://www.gender.edu.tw/web/index.php/home/index',
          },
        ],
      },
      '/replies/doc/CBUT_reply_37': {
        reference:
          'http://www.appledaily.com.tw/realtimenews/article/new/20150819/672912/',
        createdAt: '2017-01-10T02:42:00.000Z',
        appId: null,
        text:
          '公路總局說，這些內容都是錯的，麥擱傳啦！\n公路總局監理組組長林福山表示，這則訊息每隔一段時間就會現身在網路\n\n根據道路交通管理處罰條例第60條規定，汽車駕駛人如...',
        type: 'RUMOR',
        hyperlinks: [
          {
            summary:
              '最近不少Line友又開始瘋傳一則已有10年以上年資的網路謠言，內容稱「本月20日起，全國將實施紅燈越線處罰條例，凡是在等待紅燈時，超出白色禁止越線者馬上處以90...',
            title: '本月20日實施紅燈越線處罰條例！假的啦 | 蘋果日報',
            url:
              'http://www.appledaily.com.tw/realtimenews/article/new/20150819/672912/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_38': {
        reference:
          'http://www.traffic.police.ntpc.gov.tw/cp-1084-32629-27.html\n網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表',
        createdAt: '2017-10-15T16:05:28.182Z',
        appId: 'WEBSITE',
        text:
          '假的。近來網路、Line瘋狂流傳，自20日起交通違規將大幅調漲，由於調幅頗大，網友互傳時罵聲不斷，均為錯誤訊息，網路謠言希望能止於智者，不要再以訛傳訛混淆視聽。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary: '',
            title:
              '網路流傳「道路交通法規修法加重處罰」謠言澄清一覽表-新北市政府警察局交通警察大隊',
            url: 'http://www.traffic.police.ntpc.gov.tw/cp-1084-32629-27.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_39': {
        reference:
          'https://www.nownews.com/news/20100519/636359\n藥水比藥錠吸收快？　專業藥師破除網路傳言',
        createdAt: '2017-12-14T04:09:00.110Z',
        appId: 'WEBSITE',
        text:
          '針對特殊族群，如兒童或老人，為了讓藥物更容易服用或是給藥劑量精準，可能會將藥物製成藥水。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '為了提供更好的閱讀內容，我們使用相關網站技術來改善\n使用者體驗，也尊重用戶的隱私權，特別提出說明。 OK',
            title: '找不到與「 News 20100519 636359」相關的文章或頁面',
            url: 'https://www.nownews.com/news/20100519/636359',
          },
        ],
      },
      '/replies/doc/CBUT_reply_4': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '地瓜葉和牛奶的組合含有豐富的營養成分，可以幫助預防骨質疏鬆、延緩血糖上升，但對已罹患的疾病沒有治療效果。傳言所稱「可以治療三高、痛風和癌症」，並不正確。',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/1481',
        createdAt: '2019-12-06T16:05:05.572Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/1481',
            summary:
              '網傳「只要牛奶加上它（地瓜葉），三高、痛風、癌症都不見了」，經查：\n\n一、地瓜葉和牛奶的組合含有豐富的營養成分，可以幫助預防骨質疏鬆、延緩血糖上升，但對已罹患的...',
            title:
              '【錯誤】網傳「只要牛奶加上它（地瓜葉），三高、痛風、癌症都不見了」？',
            url: 'https://tfc-taiwan.org.tw/articles/1481',
          },
        ],
      },
      '/replies/doc/CBUT_reply_40': {
        userId: 'CBUT_user_45',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text: '公投結果並不會影響到大法官的釋憲結果，照常會生效。',
        reference:
          'https://www.storm.mg/article/376727\nhttps://www.chinatimes.com/realtimenews/20180419002190-260407',
        createdAt: '2018-10-23T11:51:05.408Z',
        hyperlinks: [
          {
            summary:
              '立法院甫完成《公投法》的修法，公投門檻大幅降低，「直接民權」顛覆「代議制」的大戰一觸即發。前司法院副院長蘇永欽接受《風傳媒》專訪指出，公投若決定的是法律事項，無...',
            title:
              '反同婚公投過關可推翻釋憲案？蘇永欽：大法官說了算，但恐違反民主理論-風傳媒',
            url: 'https://www.storm.mg/article/376727',
          },
          {
            summary: null,
            title: null,
            url:
              'https://www.chinatimes.com/realtimenews/20180419002190-260407',
          },
        ],
      },
      '/replies/doc/CBUT_reply_41': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '假的。\n1.衛福部回應，外籍人士感染愛滋人數並無增加。婚姻的成立，需要多重步驟，且除了我國，國外許多先進國家，也提供愛滋治療，疫情也已獲控制。因此，入籍台灣享受...',
        reference:
          'http://www.cdc.gov.tw/info.aspx?treeid=4⋯id=1bd193ed6dabaee6&tid=0579D4F36FF0AF9...',
        createdAt: '2018-10-23T17:31:53.634Z',
        hyperlinks: [
          {
            summary: '',
            title: '衛生福利部疾病管制署',
            url:
              'http://www.cdc.gov.tw/info.aspx?treeid=4⋯id=1bd193ed6dabaee6&tid=0579D4F36FF0AF94',
          },
          {
            summary:
              '最近有太多關於愛滋的謠言，讓我們來告訴你真相究竟是什麼\n製作：台灣同志諮詢熱線協會、愛滋權促會\n設計：開合跳工作室\n　\n\t以下這些流言/謠言，你是不是也聽過呢？...',
            title: '【破除謠言】你所不知道的愛滋真相網頁版(另附錄政府闢謠圖文)',
            url: 'https://hotline.org.tw/blog/1111',
          },
          {
            summary:
              'udn.com 提供即時新聞以及豐富的政治、社會、地方、兩岸、國際、財經、數位、運動、NBA、娛樂、生活、健康、旅遊新聞，以最即時、多元的內容，滿足行動世代的需求。',
            title: '',
            url: 'https://udn.com/news/story/6656/2484493',
          },
          {
            summary:
              '12\n05月\n\t\t\t\t\t\t\t\t\t\n\n\t\t\t\t\t\t\t\t\t\n\n\t\t\t\t\t\t\t\t\t\t落實校園性別平等教育，保護孩子遠離性侵害（新聞稿）\n\n\t\t\t\t\t\t\t\t\t\t 201...',
            title: '教育部性別平等全球資訊網',
            url: 'https://www.gender.edu.tw/web/index.php/m1/m1_03_01?sid=408',
          },
          {
            summary: null,
            title: null,
            url: 'https://www.tgeea.org.tw/issue/3396/',
          },
          {
            summary:
              '婚姻平權議題持續延燒，反同團體在同婚修法之外，另外再開性別平等教育戰場，以家長之名在全國縣市議會串連議員發動提案、開座談會，近日甚至將矛頭對準校園，動員外校家長...',
            title:
              '性別光譜不是你想的性別混淆　高市印宣傳摺頁破除性平謠言--上報',
            url: 'https://www.upmedia.mg/news_info.php?SerialNo=18179',
          },
          {
            summary: null,
            title: null,
            url: 'https://www.awakening.org.tw/topic/2460',
          },
          {
            summary:
              '下一代幸福聯盟（以下簡稱下福盟）發起「反同志教育公投案」，內容是：「你是否同意在國民教育階段內（國中及國小），教育部及各級學校不應對學生實施性別平等教育法施行細...',
            title:
              '反同志教育公投一旦通過，同志的無奈人生只會無止盡地延續下去 - The News Lens 關鍵評論網',
            url: 'https://www.thenewslens.com/article/103774',
          },
          {
            summary:
              'udn.com 提供即時新聞以及豐富的政治、社會、地方、兩岸、國際、財經、數位、運動、NBA、娛樂、生活、健康、旅遊新聞，以最即時、多元的內容，滿足行動世代的需求。',
            title: '',
            url: 'https://udn.com/news/story/10966/2483385',
          },
          {
            summary:
              '喧騰多時的同性婚姻爭議終於有解，大法官今釋憲宣告《民法》禁止同性結婚違憲。而根據法務部統計資料顯示，目前世界共有23國承認同性婚姻並以法律保障，其中南非、英國另...',
            title: '【同婚釋憲】除了台灣　全球23國承認同婚 | 蘋果日報',
            url:
              'http://www.appledaily.com.tw/realtimenews/article/new/20170524/1124709/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_42': {
        userId: 'CBUT_user_53',
        appId: 'WEBSITE',
        type: 'NOT_RUMOR',
        text:
          '1. 大法官748釋憲確實為「亞洲第1同婚國家」打開了「潘朵拉」盒子！為何呢？因為探究婚姻的「真義」( https://bit.ly/2O2sRAH )，顯然與...',
        reference:
          'https://tw.news.yahoo.com/%E5%90%8C%E6%80%A7%E5%A9%9A%E5%A7%BB%E9%87%8B%E6%86%B2...',
        createdAt: '2018-10-24T13:19:03.710Z',
      },
      '/replies/doc/CBUT_reply_43': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 2017年度使用台灣健保之外籍愛滋病患，僅有 25 人，花費僅佔台灣健保資源之百萬分之五點二。對台灣健保之影響微乎其微。\n2. 我國不禁止外籍愛滋病患入境...',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/260',
        createdAt: '2018-11-22T15:55:57.477Z',
        hyperlinks: [
          {
            summary:
              '通訊軟體流傳多則「外國愛滋病患濫用台灣健保」之謠言，聲稱「全球愛滋病患感謝台灣健保、愛滋病治療將會弄垮臺灣的健保」。\n經查：\n1. 2017年度使用台灣健保之外...',
            title:
              '【錯誤】網傳「全球愛滋病患感謝台灣健保、外國愛滋病患者濫用台灣健保…愛滋病治療將會弄垮臺灣的健保」？',
            url: 'https://tfc-taiwan.org.tw/articles/260',
          },
        ],
      },
      '/replies/doc/CBUT_reply_44': {
        userId: 'CBUT_user_57',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '下附影片為羅馬尼亞於2019/5/28拍攝的，可在Youtube上找到，並非義大利\n再根據另一個網頁查證，當日為羅馬尼亞的冰雹',
        reference:
          '羅馬尼亞冰雹查證資料出處：https://www.mygopen.com/2019/07/Romania-hail.html\n羅馬尼亞冰雹影片 https://...',
        createdAt: '2020-02-21T02:49:47.068Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://www.mygopen.com/2019/07/Romania-hail.html',
            summary: '',
            title: '',
            url: 'https://www.mygopen.com/2019/07/Romania-hail.html',
          },
          {
            normalizedUrl: 'https://www.youtube.com/watch?v=IDogwFIaE58',
            summary: 'Stone rain\nSubscribe,Like and share.\nFor more videos',
            title: 'Stone rain in Romania',
            url: 'https://www.youtube.com/watch?v=IDogwFIaE58',
          },
        ],
      },
      '/replies/doc/CBUT_reply_45': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 網傳影片出處是2019年5月28日羅馬尼亞北部札勒烏（Zalau）市下冰雹的景象，不是義大利羅馬下石頭雨。\n2. 台灣事實查核中心檢索義大利媒體及國際通訊社，並無義大利羅馬下石頭雨的事件。',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3477',
        createdAt: '2020-04-08T14:19:31.619Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3477',
            summary:
              '網傳照片文字宣稱「意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨...」，經查：\n\n一、網傳影片出處是2019年5月28日羅馬尼亞北部札勒烏（Zalau）...',
            title:
              '【錯誤】網傳影片搭配文字宣稱「意大利羅馬周三，下起了從來沒見過，也沒聽說過的石頭雨......」？',
            url: 'https://tfc-taiwan.org.tw/articles/3477',
          },
        ],
      },
      '/replies/doc/CBUT_reply_46': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 馬偕醫院沒有「林清風」醫師。\n2. 這謠言連寄生蟲名字都寫錯，正確名稱是「海獸胃線蟲」\n3. 鮭魚被歸類為淡水魚，不容易遭海獸胃腺蟲寄生，反倒白帶魚、青魚...',
        reference:
          '《泛科學》有文章闢謠：\nhttps://pansci.asia/archives/122243',
        createdAt: '2018-12-12T04:33:27.009Z',
        hyperlinks: [
          {
            summary:
              '作者 / 林慶順教授    編輯 / 吳培安、雷雅淇\n\n《緊急通知》\n台北馬偕醫院傳來訊息：\n資深醫生（林清風）傳來：「請大家以後千萬不要再吃生魚片了！」\n因為...',
            title:
              '吃生魚片容易感染「海獸胃線蟲」，尤其鮭魚的含蟲量最高？ - PanSci 泛科學',
            url: 'https://pansci.asia/archives/122243',
          },
        ],
      },
      '/replies/doc/CBUT_reply_47': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '台灣事實查核中心已經證實為【錯誤】訊息！\n一、馬偕醫院2017年已於官網澄清，本訊息為網路謠言。\n二、專家指出，海獸胃線蟲寄生在海洋生物體內，人類並非宿主，目前...',
        reference: 'https://tfc-taiwan.org.tw/articles/772',
        createdAt: '2019-08-28T02:48:00.667Z',
        hyperlinks: [
          {
            summary:
              '近來網傳「台北馬偕醫院傳來訊息，指出好多病人因吃生魚片，胃壁附著『海獸胃腺蟲』，甚至胃壁上滿滿都是無法夾出來，驅蟲藥也很難根治」，並且搭配病患眼中夾出寄生蟲的影...',
            title:
              '【錯誤】網傳「台北馬偕醫院傳來訊息，好多人因為吃了生魚片，胃壁附著『海獸胃腺蟲』」？',
            url: 'https://tfc-taiwan.org.tw/articles/772',
          },
        ],
      },
      '/replies/doc/CBUT_reply_48': {
        userId: 'CBUT_user_60',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '■對不同城市及其官員對疫情處置的評價，屬個人評論；亦有評論從貧困人口、企業靈活度等不同角度分析舊金山與紐約的疫情處置差異。\n\n■文中提及人物資訊、事件時序大抵正...',
        reference:
          '【疫情處置評論】\n\n布魯金斯學會-Coronavirus lessons from New York and San Francisco\nhttps://www...',
        createdAt: '2020-04-28T13:51:19.413Z',
      },
      '/replies/doc/CBUT_reply_49': {
        userId: 'CBUT_user_61',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text:
          '此訊息需要更多數據，不過難以統計，其他關於政策的訂定時間與描述很清楚。',
        reference: '',
        createdAt: '2020-05-05T09:41:19.615Z',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_5': {
        userId: 'CBUT_user_68',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '1. 本文出自商周雜誌〈冰封中國關鍵報告〉。\n2. 中國有「世界最大勞動力人口國」的優勢，爆發大型流行病情，固然會影響各產業及國際貿易，若疫情發生在任何一個大型...',
        reference:
          '【商周】冰封中國關鍵報告\nhttps://www.businessweekly.com.tw/magazine/article_page.aspx?id=600...',
        createdAt: '2020-02-27T09:11:33.760Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.businessweekly.com.tw/magazine/article_page.aspx?id=6001194',
            summary:
              '一個疑似從武漢海鮮市場開始的病毒，現在，造成中國經濟停擺，全世界都地震！當貨出不去，也進不來，中國的冰封效應，正如何在台灣快速擴散？商周兵分5路，從一個乳液的瓶...',
            title: '冰封中國關鍵報告  - 封面摘要 - 商業周刊',
            url:
              'https://www.businessweekly.com.tw/magazine/article_page.aspx?id=6001194',
          },
          {
            normalizedUrl:
              'https://www.theverge.com/2020/2/12/21127754/mwc-2020-canceled-coronavirus-trade-...',
            summary:
              'The world’s biggest phone show, Mobile World Congress, is no longer taking place...',
            title:
              'The world’s biggest phone show has been canceled due to coronavirus concerns',
            url:
              'https://www.theverge.com/2020/2/12/21127754/mwc-2020-canceled-coronavirus-trade-...',
          },
          {
            normalizedUrl:
              'https://www.reuters.com/article/us-china-health-fda/fda-identified-20-drugs-with...',
            summary:
              'The ultrastructural morphology exhibited by the 2019 Novel Coronavirus (2019-nCo...',
            title:
              'FDA identified 20 drugs with shortage risks due to coronavirus outbreak',
            url:
              'https://www.reuters.com/article/us-china-health-fda/fda-identified-20-drugs-with...',
          },
          {
            normalizedUrl:
              'https://www.spglobal.com/platts/en/market-insights/latest-news/oil/022620-oil-fu...',
            summary:
              'HighlightsCrude lowest since January 2019\n  More countries report COVID-19 cases...',
            title:
              'Oil extends losses as coronavirus outbreak accelerates outside China',
            url:
              'https://www.spglobal.com/platts/en/market-insights/latest-news/oil/022620-oil-fu...',
          },
        ],
      },
      '/replies/doc/CBUT_reply_50': {
        userId: 'CBUT_user_63',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '一個場合當中誰做了什麼、意味什麼，這都是沒有直接證據的推論。此訊息當中可以留意的是“無症狀卻可能有傳染力的隱性感染者”造成的疫情傳染風險。由於本次新型冠狀肺炎疫...',
        reference:
          '口罩用途可以參考文章\nhttp://www-o.ntust.edu.tw/h1n1/page16.htm',
        createdAt: '2020-04-06T07:42:40.309Z',
        hyperlinks: [
          {
            normalizedUrl: 'http://www-o.ntust.edu.tw/h1n1/page16.htm',
            summary:
              '一、為什麼戴口罩可以預防傳染病? \n\t\t\n\t\t          \n\t\t1.避免把病毒或細菌傳染給別人。\n\t\t\n\t\t            \n\t\t因：咳嗽及打噴...',
            title: '口罩知多少',
            url: 'http://www-o.ntust.edu.tw/h1n1/page16.htm',
          },
        ],
      },
      '/replies/doc/CBUT_reply_51': {
        userId: 'CBUT_user_65',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '是修法更嚴執行：法務部在毒品危害防制條例三讀修正通過的當天也發布新聞稿，說明這次的修法重點，包括重懲毒販，提高製造、販賣、運輸毒品的刑度及罰金；將持有第3級、第...',
        reference: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx\n',
        createdAt: '2019-12-24T02:41:34.864Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
            summary:
              '立法院會17日三讀修正通過毒品危害防制條例部分條文，持有第三級、第四級毒品純質淨重5公克以上者可處有期徒刑。圖為第四級毒品5-MeO-DIPT，俗名火狐狸。（示...',
            title:
              '網傳校園攜毒免罰 綠委：修法更嚴勿混淆視聽 | 社會 | 重點新聞 | 中央社 CNA',
            url: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
          },
        ],
      },
      '/replies/doc/CBUT_reply_52': {
        userId: 'CBUT_user_65',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '而有臉書粉絲專頁po文指出，國民黨團當時提出修正動議，持有第3級、第4級毒品的純質淨重，1公克以上就要課予刑責，但被民進黨表決封殺。此外，專頁上還附上一張圖卡，...',
        reference: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
        createdAt: '2019-12-24T02:55:42.325Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
            summary:
              '立法院會17日三讀修正通過毒品危害防制條例部分條文，持有第三級、第四級毒品純質淨重5公克以上者可處有期徒刑。圖為第四級毒品5-MeO-DIPT，俗名火狐狸。（示...',
            title:
              '網傳校園攜毒免罰 綠委：修法更嚴勿混淆視聽 | 社會 | 重點新聞 | 中央社 CNA',
            url: 'https://www.cna.com.tw/news/firstnews/201912205004.aspx',
          },
        ],
      },
      '/replies/doc/CBUT_reply_53': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 12月17日立法院三讀通過《毒品危害防制條例》部分條文修正草案，是把處罰標準加重，而非放寬。\n2. 根據毒品危害防制條例規定，有「販賣」毒品意圖就處以刑罰...',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/1670',
        createdAt: '2019-12-25T16:46:47.879Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/1670',
            summary: '',
            title:
              '【錯誤】網傳「民進黨已經通過了毒品條例了，校園內可以擁有5克的毒品，才被視同販毒。台灣除了是(同性戀島)(愛滋病島)現在又增加了一個(毒品島)了」？ | 台灣事實查核中心',
            url: 'https://tfc-taiwan.org.tw/articles/1670',
          },
        ],
      },
      '/replies/doc/CBUT_reply_54': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text: '訊息與謠言查證無關。',
        reference: '',
        createdAt: '2019-06-02T01:40:59.872Z',
      },
      '/replies/doc/CBUT_reply_55': {
        userId: 'CBUT_user_68',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '此為假冒北韓官方的訊息。北韓官媒《朝鮮中央通訊社》4月23日仍有報導金正恩回電感謝敘利亞阿薩德的新聞，南韓青瓦台4月23日也表示「沒有具體證據顯示金正恩的健康狀...',
        reference:
          '命危消息傳出後 北韓官媒曝金正恩最新動態\nhttps://news.ltn.com.tw/news/world/breakingnews/3142735\n\nTh...',
        createdAt: '2020-04-26T15:36:41.541Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://news.ltn.com.tw/news/world/breakingnews/3142735',
            summary:
              '北韓領導人金正恩身體狀況引發外界關注。（美聯社）\n    \n\n\n    2020-04-23 07:36:23〔即時新聞／綜合報導〕北韓領導人金正恩缺席祖父金日...',
            title:
              '命危消息傳出後 北韓官媒曝金正恩最新動態 - 國際 - 自由時報電子報',
            url: 'https://news.ltn.com.tw/news/world/breakingnews/3142735',
          },
          {
            normalizedUrl:
              'https://www.foxnews.com/world/kim-jong-un-mystery-north-korea-dictator-health',
            summary:
              'What’s the story with Kim Jong Un?The North Korean leader missed a high-profile ...',
            title:
              "The mystery surrounding Kim Jong Un: What's going on with the North Korean dictator's health?",
            url:
              'https://www.foxnews.com/world/kim-jong-un-mystery-north-korea-dictator-health',
          },
          {
            normalizedUrl:
              'http://world.kbs.co.kr/service/news_view.htm?lang=k&Seq_Code=353396',
            summary:
              'Photo : YONHAP News\n                \n\n                \n                         ...',
            title: '靑 NSC',
            url:
              'http://world.kbs.co.kr/service/news_view.htm?lang=k&Seq_Code=353396',
          },
          {
            normalizedUrl:
              'http://www.uriminzokkiri.com/index.php?ptype=cgisas&mtype=view&no=1190981',
            summary: '',
            title: '',
            url:
              'http://www.uriminzokkiri.com/index.php?ptype=cgisas&mtype=view&no=1190981',
          },
        ],
      },
      '/replies/doc/CBUT_reply_56': {
        userId: 'CBUT_user_69',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '此為改編自金正日2011年的訃聞，只有名字和時間有改動，其餘皆相同。\n"朝鲜劳动党中央委员会和朝鲜劳动党中央军事委员会、朝鲜民主主义人民共和国国防委员会和最高人...',
        reference: 'http://world.people.com.cn/GB/16651924.html',
        createdAt: '2020-04-27T09:34:49.512Z',
        hyperlinks: [
          {
            normalizedUrl: 'http://world.people.com.cn/GB/16651924.html',
            summary:
              '12月19日，北京，朝鲜大使馆降半旗悼念金正日。 　　据中央电视台报道，朝鲜中央电视台19日播出该国最高领导人金正日逝世讣告。　　讣告如下：　　朝鲜全体党员、人...',
            title: '朝鲜官方媒体播出金正日讣告--国际--人民网',
            url: 'http://world.people.com.cn/GB/16651924.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_57': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '台灣事實查核中心已經證實為【錯誤】訊息囉！\n一、食藥署、疾管署，還有泰國的食藥署早已證實此訊息為網路謠言誤。\n二、專家指出，愛滋病毒離開人體後，短時間內便會失去...',
        reference: 'https://tfc-taiwan.org.tw/articles/740',
        createdAt: '2019-08-14T05:27:33.030Z',
        hyperlinks: [
          {
            summary:
              '【錯誤】「泰國水果罐頭被愛滋病患者血液汙染」謠言再起 \n      \n         \n\n網路、社群媒體近期流傳「泰國水果罐頭遭愛滋病毒汙染」訊息，經查：\n\n...',
            title: '【錯誤】「泰國水果罐頭被愛滋病患者血液汙染」謠言再起',
            url: 'https://tfc-taiwan.org.tw/articles/740',
          },
        ],
      },
      '/replies/doc/CBUT_reply_58': {
        reference:
          'http://www.appledaily.com.tw/appledaily/article/headline/20151103/36878044/',
        createdAt: '2017-06-20T17:29:47.805Z',
        appId: 'WEBSITE',
        text:
          '江守山醫師畢業自陽明大學醫學系，曾任台北榮總腎臟科總醫師、內科主治醫師；新光醫院腎臟科主治醫師、腎臟科主任，專長為腎病、腎衰竭治療。對醫療保健應該有一定專業。但...',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_45',
        hyperlinks: [
          {
            summary:
              '【綜合報導】腎臟科名醫江守山日前撰文，影射他任職的台北市新光醫院營運出狀況、醫療設備長期不更換、用藥較差等，恐吹熄燈號。昨院方反擊，指兩年前就查獲江涉Ａ健保，讓...',
            title: '名醫江守山A健保 醫院開除 | 蘋果日報',
            url:
              'http://www.appledaily.com.tw/appledaily/article/headline/20151103/36878044/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_59': {
        userId: 'CBUT_user_71',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '影片含有許多不實訊息，例如 2:09 聲稱蜂蜜是二級燙傷首選藥物，有嚴重誤導人之嫌，已有其他急診醫師說醫療級才可，正確做法還是立刻沖水降溫，嚴重時趕快送醫。',
        reference: 'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
        createdAt: '2020-07-22T03:51:29.624Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
            summary:
              '【本報台北訊】居家治療燒燙傷偏方不少，牙膏、蛋清等五花八門，多數遭專家否定，不止沒療效還恐增加感染風險，實證研究顯示，「蜂蜜」具消炎殺菌功效，有助照護燒燙傷口。...',
            title: '燙傷可抹蜂蜜？醫師：醫療級才可 | 醫藥 | 人間福報',
            url: 'https://www.merit-times.com.tw/NewsPage.aspx?unid=436129',
          },
        ],
      },
      '/replies/doc/CBUT_reply_6': {
        reference:
          'http://news.ltn.com.tw/news/life/paper/111309\n心肌梗塞突襲 咳嗽不能自救 快就醫才對',
        createdAt: '2018-01-23T15:42:02.674Z',
        appId: 'WEBSITE',
        text:
          '假的。\n這項處置主要是用於當醫護人員觀測到患者心跳出現異常，為了延長其意識清醒時間，由專業人員指導患者咳嗽；而非一般心臟病患者自救。\n台北榮總急診部主治醫師高偉...',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '2007-01-13記者洪素卿／台北報導「現在是下午5點50分，忙忙碌碌的你上了一整天的班，正在開車回家的路上。突然！你感到胸口有一股劇痛，並且開始蔓延到手臂和...',
            title:
              '心肌梗塞突襲 咳嗽不能自救 快就醫才對 - 生活 - 自由時報電子報',
            url: 'http://news.ltn.com.tw/news/life/paper/111309',
          },
        ],
      },
      '/replies/doc/CBUT_reply_60': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 燙傷的正確處理方式應是「沖、脫、泡、蓋、送」\n2. 奇美醫院急診部副主任許建清則表示，醬油、牙膏、麵粉都不是在無菌的環境下生產的，可能引起傷口感染，而且增...',
        reference:
          'https://www.commonhealth.com.tw/article/article.action?nid=74762',
        createdAt: '2019-07-02T04:09:52.081Z',
        hyperlinks: [
          {
            summary: null,
            title: null,
            url:
              'https://www.commonhealth.com.tw/article/article.action?nid=74762',
          },
        ],
      },
      '/replies/doc/CBUT_reply_61': {
        userId: 'CBUT_user_74',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '至於坊間流傳的撒麵粉、鹽巴、太白粉、沙拉油、豬油等偏方，不但效果差，而且到醫院醫師還要先花時間清潔才能評估傷口，反而延遲救治時間。\n陽光基金會倡導組專案經理莊麗...',
        reference: 'https://news.ltn.com.tw/news/life/paper/1101504',
        createdAt: '2019-07-23T08:06:47.678Z',
        hyperlinks: [
          {
            summary:
              '2017-05-11記者陳炳宏／台北報導一名4歲男童嬉戲時，不慎絆倒電熱水壺，造成大面積燙傷，阿公和阿嬤在慌亂中，急忙脫掉他的衣服，並撒麵粉，企圖「降溫」，沒想...',
            title:
              '謠言終結站》 燙傷灑麵粉可降溫？ 醫：延誤救治更易留疤 - 生活 - 自由時報電子報',
            url: 'https://news.ltn.com.tw/news/life/paper/1101504',
          },
        ],
      },
      '/replies/doc/CBUT_reply_62': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text: '網傳影片為2010年墨西哥電視劇《愛的勝利》片段。',
        reference: 'https://tfc-taiwan.org.tw/articles/3542',
        createdAt: '2020-04-08T14:05:42.091Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3542',
            summary:
              '【錯誤】網傳影片「一對意大利情侣，本来約定今年完婚，女友感染新冠肺炎在搶救，男友見最后一面，果断脱掉防護服並親吻女友」？\n      \n         \n\n網...',
            title:
              '【錯誤】網傳影片「一對意大利情侣，本来約定今年完婚，女友感染新冠肺炎在搶救，男友見最后一面，果断脱掉防護服並親吻女友」？',
            url: 'https://tfc-taiwan.org.tw/articles/3542',
          },
        ],
      },
      '/replies/doc/CBUT_reply_63': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '日本人砍伐台灣森林總面積：1萬8432公頃 \n國民政府與民間砍伐總面積：34萬4千多公頃 ',
        reference: 'https://www.douban.com/group/topic/60942708/',
        createdAt: '2019-04-18T11:30:15.497Z',
        hyperlinks: [
          {
            summary:
              '来自: 凱恩司(叫我凱拔!!)\n                2014-08-26 10:32:17\n            \n\n            \n ...',
            title: '台灣歷史課本不提的森林史真相　(轉　作者：黃鈺翔)',
            url: 'https://www.douban.com/group/topic/60942708/',
          },
        ],
      },
      '/replies/doc/CBUT_reply_64': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '假的。華府智庫戰略暨國際研究中心（CSIS）亞洲資深顧問葛來儀（Bonnie Glaser）說，有些人在捏造事件，假新聞非常普遍。',
        reference:
          'https://news.ltn.com.tw/news/politics/breakingnews/2754266\n\nhttps://www.setn.com...',
        createdAt: '2019-04-15T16:40:37.582Z',
        hyperlinks: [
          {
            summary:
              '高雄市長韓國瑜9日啟程訪美。（資料照）\n\n\n2019-04-10 13:15〔即時新聞／綜合報導〕高雄市長韓國瑜昨（9日）啟程訪美，臉書社團「韓國瑜加油團」日前...',
            title:
              '川普求見韓國瑜？ 葛來儀一句話狠打臉.... - 政治 - 自由時報電子報',
            url: 'https://news.ltn.com.tw/news/politics/breakingnews/2754266',
          },
          {
            summary:
              '政治中心／台北報導高雄市長韓國瑜即將出發訪美，如今卻有韓粉在臉書社團爆料，說美國總統川普透過秘密管道傳達訊息給高雄市政府，希望韓國瑜能充當美中貿易戰之間的和事佬...',
            title:
              '說故事大師！韓粉爆川普求見韓國瑜　網酸：不是說好取代習 | 政治 | 三立新聞網  SETN.COM',
            url: 'https://www.setn.com/News.aspx?NewsID=524453',
          },
          {
            summary: null,
            title: null,
            url:
              'https://www.storm.mg/article/1159024?srcid=gAAAAABctLPDyIyvBIeudem6SegNC_3_JhLhr...',
          },
        ],
      },
      '/replies/doc/CBUT_reply_65': {
        userId: 'CBUT_user_79',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text: '文字長度太短，疑似為使用者手動輸入之查詢語句，不像轉傳文章。',
        reference: '',
        createdAt: '2019-11-18T07:31:32.985Z',
      },
      '/replies/doc/CBUT_reply_66': {
        reference:
          'http://pension.president.gov.tw/cp.aspx?n=03E38CC0FF75CFBE&s=FB4868255F8CFFAD',
        createdAt: '2017-03-24T16:51:23.373Z',
        appId: 'WEBSITE',
        text: '年改會第14次會議重點引述非事實，請參閱資料來源。',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary: '',
            title:
              '總統府國家年金改革委員會-【澄清稿】年改會第14次會議重點引述非事實',
            url:
              'http://pension.president.gov.tw/cp.aspx?n=03E38CC0FF75CFBE&s=FB4868255F8CFFAD',
          },
        ],
      },
      '/replies/doc/CBUT_reply_67': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1. 年金改革委員會第14次會議早在2016年9月29日就已舉行，會議為蒐集各方意見，並未做成任何決議。\n2. 軍公教人員退休年齡規定，依年資各有不同，並非一律...',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3573',
        createdAt: '2020-04-08T13:45:36.390Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3573',
            summary:
              '網傳「昨天年金改革委員會第14次會議...各項年金請領年齡皆為65歲...軍人終身俸不再發放...80歲以後到死1塊錢不給...」，經查：\n\n一、年金改革委員會...',
            title:
              '【錯誤】網傳「昨天年金改革委員會第14次會議...各項年金請領年齡皆為65歲...軍人終身俸不再發放...80歲以後到死1塊錢不給...」？',
            url: 'https://tfc-taiwan.org.tw/articles/3573',
          },
        ],
      },
      '/replies/doc/CBUT_reply_68': {
        reference:
          'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
        createdAt: '2017-06-24T07:35:54.955Z',
        appId: 'WEBSITE',
        text:
          '目前平台之連署議案僅針對公共化幼稚園，要求政府在特定年度未完成之目標數量，因累加至下一年，確保最終仍為1000間。\n並如原訊息中沒有要求沒開成的非營利幼兒園一定...',
        type: 'RUMOR',
        userId: 'CBUT_user_45',
        hyperlinks: [
          {
            summary:
              '教育部長潘文忠今(20)日赴行政院報告「擴大幼兒教保公共化」相關規劃，為符膺總統托育政策，提供價格合理、品質有保障的教保服務，減輕家長的經濟負擔，教育部以穩健提...',
            title: '擴大公共化教保服務，教育部規劃增設公共化幼兒園1,000班',
            url:
              'https://www.edu.tw/News_Content.aspx?n=9E7AC85F1954DDA8&s=9BD8BE46F97CACFB',
          },
        ],
      },
      '/replies/doc/CBUT_reply_69': {
        reference: '',
        createdAt: '2017-07-10T14:01:55.446Z',
        appId: 'WEBSITE',
        text: '此應該是個人心得。',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_45',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_7': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '假的。其實是「違規行為終了日」到「檢舉日期」超過七天，才不予舉發，跟開單日期一點關係都沒有。\n\n根據道路交通管理處罰條例第 7-1 條的解釋：對於違反本條例之行...',
        reference:
          'https://www.mygopen.com/2018/03/line_27.html\n【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言',
        createdAt: '2018-03-30T14:19:54.455Z',
        hyperlinks: [
          {
            summary:
              '【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言\n\n\n\n\n\n\n\n\n\n最近 LINE 收到「如果遇到檢舉罰單請注意檢舉日期跟開單日期，很多朋友都不知道...',
            title:
              '【假LINE】罰單注意檢舉跟開單日期？超過七日的申訴罰單謠言 ｜MyGoPen',
            url: 'https://www.mygopen.com/2018/03/line_27.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_70': {
        reference:
          'http://www.mygopen.com/2017/06/blog-post_26.html\n神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！',
        createdAt: '2017-07-09T15:02:23.370Z',
        appId: 'WEBSITE',
        text:
          '基本上地瓜葉其實單吃就有效果，牛奶、豆漿可加可不加，民眾不用迷信。 三高或是糖尿病的患者還是要搭配醫生的治療，不能單靠吃地瓜葉就想將身體調養好，民眾千萬要注意。...',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！\n\n\n\n\n\n\n\n\n\n有些食物固然有效，但網路謠言常會過於誇大，像內容為「神...',
            title:
              '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！ ｜MyGoPen',
            url: 'http://www.mygopen.com/2017/06/blog-post_26.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_71': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '基本上地瓜葉其實單吃就有效果，牛奶、豆漿可加可不加，民眾不用迷信。 三高或是糖尿病的患者還是要搭配醫生的治療，不能單靠吃地瓜葉就想將身體調養好，民眾千萬要注意。...',
        reference:
          'http://www.mygopen.com/2017/06/blog-post_26.html\n神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！',
        createdAt: '2018-06-10T13:21:16.828Z',
        hyperlinks: [
          {
            summary:
              '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！\n\n\n\n\n\n\n\n\n\n有些食物固然有效，但網路謠言常會過於誇大，像內容為「神...',
            title:
              '【假養生】神奇的地瓜葉？搭配鮮奶遠離三高？謠言讓醫生說：有痛風或是腎臟不好的人要小心！ ｜MyGoPen',
            url: 'http://www.mygopen.com/2017/06/blog-post_26.html',
          },
        ],
      },
      '/replies/doc/CBUT_reply_72': {
        reference:
          'https://www.everydayhealth.com.tw/article/857\nhttps://www.amazon.co.jp/%E8%AA%8D...',
        createdAt: '2017-04-12T11:21:23.414Z',
        appId: 'WEBSITE',
        text:
          '文中的日本醫學博士是安田和人，有針對失智症食療出書，內容確實包含文中四項食物。',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_85',
        hyperlinks: [
          {
            summary:
              '【早安健康／孫于琛】每個人都有可能得到失智症，除了多運動並多訓練腦力外，其實日常飲食對我們身體才有最大的影響。不是只有吃藥才能治好，有很多種食物其實對預防甚至改...',
            title: '早安健康｜防失智不用吃藥！日本醫學博士推薦4食材',
            url: 'https://www.everydayhealth.com.tw/article/857',
          },
          {
            summary:
              'Prime Video                                                                     ...',
            title:
              '認知症 治った! 助かった! この方法 | 安田 和人 |本 | 通販 | Amazon',
            url:
              'https://www.amazon.co.jp/%E8%AA%8D%E7%9F%A5%E7%97%87-%E6%B2%BB%E3%81%A3%E3%81%9F...',
          },
        ],
      },
      '/replies/doc/CBUT_reply_73': {
        reference: 'https://zh.wikipedia.org/wiki/李孟贤',
        createdAt: '2018-01-01T12:00:04.036Z',
        appId: 'WEBSITE',
        text:
          '舊金山前市長確實因為心臟病去世，但其他部分應該只是想要提醒大家注意心臟疾病。',
        type: 'NOT_RUMOR',
        userId: 'CBUT_user_45',
        hyperlinks: [
          {
            summary:
              '李孟贤Edwin Mah Lee\n\n第43任舊金山市長\n任期2011年1月11日－2017年12月12日前任\n加文·纽森继任\n伦敦·布里德（代理）个人资料出生\n...',
            title: '李孟贤 - 维基百科，自由的百科全书',
            url: 'https://zh.wikipedia.org/wiki/李孟贤',
          },
        ],
      },
      '/replies/doc/CBUT_reply_74': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '有關報載世紀鋼廠房建置時程落後之情形，世紀鋼公開表示，內容未經查核，顯與事實有違，該公司為國內率先響應離岸風電國產化的本土公司，除了觀音廠廠房擴建產能，投入生產...',
        reference:
          'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
        createdAt: '2020-09-26T07:30:21.054Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
            summary:
              '有關媒體報載「百億離岸風電國產大餅訂單爆肥水外流」一文，指總統蔡英文去(108)年在台北港水下基礎基樁對接儀式中，為世紀集團對水下基礎製造技術、廠房等投資背書，...',
            title: '逐步推動離岸風電產業政策 開發商與在地產業攜手共創雙贏',
            url:
              'https://www.moea.gov.tw/MNS/populace/news/News.aspx?kind=9&menu_id=22333&news_id=90907',
          },
        ],
      },
      '/replies/doc/CBUT_reply_75': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '據外交部瞭解，拜會美國行政部門的相關行程，均由臺北市政府自行洽請美國在台協會（AIT)協助安排，並非由外交部或駐處洽排。相關安排情形外交部均不便評論，宜由臺北市政府自行説明。',
        reference:
          'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
        createdAt: '2020-09-26T09:40:02.610Z',
        hyperlinks: [
          {
            normalizedUrl:
              'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
            summary: '',
            title: '',
            url:
              'https://www.mofa.gov.tw/News_Content_M_2.aspx?n=D6D997396B647A7C&sms=9C41168DD8E...',
          },
        ],
      },
      '/replies/doc/CBUT_reply_76': {
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text:
          '陸委會指出，習近平近日再定義「九二共識」是「兩岸同屬一個中國、共同謀求國家統一」，更倡議探索「一國兩制」台灣方案，踐踏民主程序，透過花瓶式的「政治協商」來分化台...',
        reference:
          'https://www.cna.com.tw/news/aipl/201901090206.aspx\n\n\nhttps://www.rti.org.tw/news/view/id/2007762',
        createdAt: '2019-01-10T07:11:59.378Z',
        hyperlinks: [
          {
            summary:
              '中國國民黨9日在中央黨部舉行中常會，國民黨主席吳敦義（圖）主持會議。中央社記者裴禛攝  108年1月9日（中央社記者余祥台北9日電）國民黨主席吳敦義今天向總統蔡...',
            title: '吳敦義向總統喊話  重新正確認知九二共識 | 政治 | 中央社 CNA',
            url: 'https://www.cna.com.tw/news/aipl/201901090206.aspx',
          },
          {
            summary:
              '時間：2019-01-09 21:24\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t新聞引據：中央社\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t撰稿編輯：陳子華\n\t\t\t\t\t\t\t\t\n\n...',
            title: '國民黨續提九二共識 陸委會：無異附和中共',
            url: 'https://www.rti.org.tw/news/view/id/2007762',
          },
        ],
      },
      '/replies/doc/CBUT_reply_77': {
        userId: 'CBUT_user_54',
        appId: 'WEBSITE',
        type: 'RUMOR',
        text:
          '1.新北市的「老人健保補助」，必須符合年紀和設籍滿一年的要件，社會局會主動寄專函通知符合資格的民眾，並非「帶身分證至區公所辦理」。\n\n2.65歲以上領3224元...',
        reference: '台灣事實查核中心\nhttps://tfc-taiwan.org.tw/articles/3737',
        createdAt: '2020-04-28T05:56:52.104Z',
        hyperlinks: [
          {
            normalizedUrl: 'https://tfc-taiwan.org.tw/articles/3737',
            summary:
              '「只要戶籍在新北市，年滿65歲可到區公所帶身份證存摺辦理健保補助金，三千二百二十四元，半年領一次...」，經查：\n\n一、新北市確實有「老人健保補助」，但必須是符...',
            title:
              '【錯誤】網傳「只要戶籍在新北市，年滿65歲可到區公所帶身份證存摺辦理健保補助金，三千二百二十四元，半年領一次...」？',
            url: 'https://tfc-taiwan.org.tw/articles/3737',
          },
        ],
      },
      '/replies/doc/CBUT_reply_78': {
        userId: 'CBUT_user_90',
        appId: 'WEBSITE',
        type: 'NOT_ARTICLE',
        text: '訊息與謠言查證無關。',
        reference: '',
        createdAt: '2020-07-31T08:25:23.003Z',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_79': {
        createdAt: '2017-12-04T15:18:29.041Z',
        appId: 'DEVELOPMENT_FRONTEND',
        text: '聊天',
        type: 'NOT_ARTICLE',
        userId: 'CBUT_user_16',
        hyperlinks: [],
      },
      '/replies/doc/CBUT_reply_8': {
        reference:
          'https://www.fda.gov.tw/tc/newsContent.aspx?cid=5049&id=22092\n會中毒，所以要少吃勾芡料理？',
        createdAt: '2017-07-07T09:39:54.019Z',
        appId: 'WEBSITE',
        text:
          '太白粉的樹薯塊根含有微量具有毒性之氰酸，食用後會引發噁心、嘔吐、下痢、昏眩，甚至會呼吸衰竭而死亡。\n樹薯粉是將樹薯經過浸水、脫汁、壓榨、加熱和乾燥等加工過程後抽...',
        type: 'RUMOR',
        userId: 'CBUT_user_35',
        hyperlinks: [
          {
            summary:
              '聽說吃樹薯粉會中毒，所以要少吃勾芡料理，這是真的嗎？\n                【發布日期：2017-05-02】 \n                發布單位：企劃及科技管理組',
            title:
              '聽說吃樹薯粉會中毒，所以要少吃勾芡料理，這是真的嗎？-衛生福利部食品藥物管理署',
            url: 'https://www.fda.gov.tw/tc/newsContent.aspx?cid=5049&id=22092',
          },
        ],
      },
      '/replies/doc/CBUT_reply_80': {
        userId: 'CBUT_user_45',
        appId: 'DEVELOPMENT_FRONTEND',
        type: 'RUMOR',
        text: '微信支付自2018年起即有收費。',
        reference: 'http://www.sohu.com/a/243987392_561670',
        createdAt: '2019-11-18T18:36:40.797Z',
        hyperlinks: [
          {
            normalizedUrl: 'http://www.sohu.com/a/243987392_561670',
            summary:
              '由于您未安装flash播放器视频无法播放，点击这里安装\n          \n          \n            \n            微信收费升...',
            title: '8月1日起微信又要多收一笔钱，上亿用户将受影响',
            url: 'http://www.sohu.com/a/243987392_561670',
          },
        ],
      },
      '/replies/doc/CBUT_reply_9': {
        userId: 'CBUT_user_15',
        appId: 'WEBSITE',
        type: 'OPINIONATED',
        text: '每個人對於兩蔣時代的生命經歷不同，因此有各自不同的觀點。',
        reference:
          '網友逐條回應\nhttps://m.facebook.com/morningdusts/posts/645289662344314\n\nhttps://m.face...',
        createdAt: '2018-03-11T02:45:59.101Z',
        hyperlinks: [
          {
            summary:
              '1.因為白色恐怖加特務，你會被說讀禁書就死了 2.因為白色恐怖加特務，你會被說你民粹就死了 3.因為這些事情以外的部分，都不敢上報入書或寫成歌，不然報紙就被禁了...',
            title: '晨塵',
            url: 'https://m.facebook.com/morningdusts/posts/645289662344314',
          },
          {
            summary:
              '以下˙是我的臉友 Pasoa Yang 對26條 “兩蔣時代是布拉布拉的美好時代” 的回應 :   "我很不幸, 能夠親身感受那個悲惨時代。 這是「兩蔣時代」。...',
            title: '段震宇',
            url:
              'https://m.facebook.com/story.php?story_fbid=1728483420523821&id=100000865406669',
          },
        ],
      },
    },
    replyrequests: {
      '/replyrequests/doc/CBUT_article_35__j4S8C_FQn8c8K4i67IISzvXVoOPipzRASqSgeBxPLNjReHVyY__RUMORS_LINE_BOT': {
        createdAt: '2017-12-24T23:48:52.805Z',
        appId: 'RUMORS_LINE_BOT',
        userId: 'j4S8C_FQn8c8K4i67IISzvXVoOPipzRASqSgeBxPLNjReHVyY',
        updatedAt: '2017-12-24T23:48:52.805Z',
        articleId: 'CBUT_article_35',
        appUserId: 'CBUT_user_42',
      },
      '/replyrequests/doc/CBUT_article_36__qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo__DEVELOPMENT_BACKEND': {
        createdAt: '2017-05-10T13:36:53.795Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo',
        updatedAt: '2017-05-10T13:36:53.795Z',
        articleId: 'CBUT_article_36',
        appUserId: 'CBUT_user_17',
      },
      '/replyrequests/doc/CBUT_article_37__Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU__BOT_LEGACY': {
        createdAt: '2017-01-11T08:52:00.000Z',
        appId: 'BOT_LEGACY',
        userId: 'Z0k1e_3tpHtWzO6L-HfjoQpFDln3K3YQHkvLIUzCBUhDznwqU',
        articleId: 'CBUT_article_37',
        appUserId: 'CBUT_user_37',
      },
      '/replyrequests/doc/CBUT_article_38__j4S8C_TthEcGGgLb2we2tKAG98NuJq1KJE6Bd2WbQPK7kqG2M__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_38',
        userId: 'j4S8C_TthEcGGgLb2we2tKAG98NuJq1KJE6Bd2WbQPK7kqG2M',
        appId: 'RUMORS_LINE_BOT',
        reason: '找不到相關訊息  繁簡夾雜',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2020-02-21T02:02:55.271Z',
        updatedAt: '2020-02-21T02:02:55.271Z',
        appUserId: 'CBUT_user_56',
      },
      '/replyrequests/doc/CBUT_article_39__CBUT_user_69__WEBSITE': {
        articleId: 'CBUT_article_39',
        userId: 'CBUT_user_69',
        appId: 'WEBSITE',
        reason:
          '此為改編自金正日2011年的訃聞，只有名字和時間有改動，其餘皆相同。\n\n"　　朝鲜劳动党中央委员会和朝鲜劳动党中央军事委员会、朝鲜民主主义人民共和国国防委员会和...',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2020-04-27T09:34:01.196Z',
        updatedAt: '2020-04-27T09:34:01.196Z',
      },
      '/replyrequests/doc/CBUT_article_40__qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo__DEVELOPMENT_BACKEND': {
        createdAt: '2017-05-30T15:36:48.738Z',
        appId: 'DEVELOPMENT_BACKEND',
        userId: 'qO1C3_qmqlhUR3yeWRZcuwPI8CP1A7XREZrNc5h3JtIy4q_Lo',
        updatedAt: '2017-05-30T15:36:48.738Z',
        articleId: 'CBUT_article_40',
        appUserId: 'CBUT_user_17',
      },
      '/replyrequests/doc/CBUT_article_41__j4S8C_ppZEoc7CPAXLOlzNOj3zwN_re6FKnwtjIiMmzS0_TFk__RUMORS_LINE_BOT': {
        articleId: 'CBUT_article_41',
        userId: 'j4S8C_ppZEoc7CPAXLOlzNOj3zwN_re6FKnwtjIiMmzS0_TFk',
        appId: 'RUMORS_LINE_BOT',
        reason:
          '我用關鍵字找，發現有不一樣的說法 https://m.douban.com/group/topic/60942708/',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2019-04-17T06:56:36.153Z',
        updatedAt: '2019-04-17T06:56:36.153Z',
        appUserId: 'CBUT_user_77',
      },
      '/replyrequests/doc/CBUT_article_42__CBUT_user_35__WEBSITE': {
        articleId: 'CBUT_article_42',
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        reason: '',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2019-04-15T16:38:11.473Z',
        updatedAt: '2019-04-15T16:38:11.473Z',
      },
      '/replyrequests/doc/CBUT_article_43__CBUT_user_35__WEBSITE': {
        articleId: 'CBUT_article_43',
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        reason: '',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2020-09-26T07:29:51.708Z',
        updatedAt: '2020-09-26T07:29:51.708Z',
      },
      '/replyrequests/doc/CBUT_article_44__CBUT_user_35__WEBSITE': {
        articleId: 'CBUT_article_44',
        userId: 'CBUT_user_35',
        appId: 'WEBSITE',
        reason: '',
        feedbacks: [],
        positiveFeedbackCount: 0,
        negativeFeedbackCount: 0,
        createdAt: '2020-09-26T09:39:48.829Z',
        updatedAt: '2020-09-26T09:39:48.829Z',
      },
    },
    analytics: {}
  },
};
for (const key in fixtures.fixturesToLoad) {
  const [, index, _, id] = key.split('/');
  if (index === 'analytics') {
    const value = fixtures.fixturesToLoad[key];
    const docIndex = value.type.replace('y', 'ies').replace('cle', 'cles');
    const doc = fixtures.expectedResults[docIndex][`/${docIndex}/doc/${value.docId}`];
    fixtures.expectedResults.analytics[key] = {
      ...value,
      docUserId: doc.userId,
      docAppId: doc.appId
    }
  }
}

console.log(fixtures.expectedResults.analytics);