...
export const loginRouter = Router()
  .use((ctx, next) => {
    ...
    if (ctx.query.redirect_to) {
      const allowedUrls = (process.env.ALLOWED_CALLBACK_URLS || '')
        .split(',')
        .map((u) => u.trim())
        .filter(Boolean);
      if (!allowedUrls.includes(ctx.query.redirect_to)) {
        const err = new Error(
...
export const authRouter = Router()
  .use(async (ctx, next) => {
    ...
    //
    if ((!ctx.session.redirect || !ctx.session.appId) && !ctx.session.redirectTo) {
      const err = new Error(
